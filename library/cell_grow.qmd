...
|---|---|---|
| `balance` | Size emerges from a synthesis/degradation balance, with no size sensor anywhere. | `k_syn` (1.0), `k_deg` (1.0) |
| `sizer` | Growth rate falls with the cell's own size: small cells grow faster, large ones slower. | `size_gain` (1.0), `f_max` (4.0) |
| `timer` | Grow at whatever rate lands the cell on its target size after `cycle_frames` frames. | `cycle_frames` (100.0) |
