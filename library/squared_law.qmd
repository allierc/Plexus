...
|---|---|---|
| `mesh` | Same law, solved on a grid instead of pair by pair: deposit the masses cloud-in-cell, solve Poisson in k-space, take the gradient in k-space, gather back with the same weights. reachable. | `n_grid` (256) |
| `warp` | Same law, as one Warp kernel: still O(N^2) arithmetic, but O(N) memory, because each thread accumulates its own sum and no [N, N] array is ever allocated. | &ndash; |
