...
|---|---|---|
| `adder` | G1 ends when the cell has ADDED `g1_delta` x `v_ref` since birth -- the adder, as a G1 rule. | `g1_delta` (1.0) |
| `doubler` | G1 ends at `g1_factor` x THIS CELL'S OWN birth volume -- the relative rule, the null that corrects nothing (Ginzberg, Kafri & Kirschner 2015), as a G1 rule. | `g1_factor` (2.0) |
| `inhibitor_dilution` | The size checkpoint with a MECHANISM under it: an inhibitor, diluted by growth, crossing a threshold. | `inhib_thresh` (0.6) |
| `timer` | Every phase on a clock, G1 included: the NULL against which a checkpoint has to be an improvement. | &ndash; |
| `transition_probability` | Smith & Martin: G1 is not a duration, it is a CONSTANT HAZARD of starting. | `p_g1` (1.0 / max(self.t[0], 1.0) |
