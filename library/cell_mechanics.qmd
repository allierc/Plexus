...
|---|---|---|
| `apicobasal` | R3 OF THE APICO-BASAL PROMOTION: the energy, written on the doubled degree-of-freedom set. | `k_v` (4.0), `kappa_s` (0.2), `kappa_h` (0.0), `sep_mu` (1.0), `surface` ("apical"), `sep_block` ("sep"), `gamma` (0.0), `rest_calibration` ("force_balance"), `mono_k` (None) |
| `marinari` | The work function of Marinari et al., Nature 484:542 (2012), Supplementary p.1: | `K` (160.0), `crowding` (1.0) |
| `monolayer` | The MONOLAYER implementation of the cell_mechanics contract (plexus2 sec. 5: same biological operator -- the mechanical force that shapes the epithelial vesicle -- different NUMERICS). | `k_v` (4.0), `kappa_s` (0.2), `h0` (0.4), `gamma` (0.0), `rest_calibration` ("force_balance"), `mono_k` (None) |
...
|---|---|---|
| `warp` | `cell_mechanics` with the shape-energy gradient in warp -- WHICH IS THE DEFAULT. | &ndash; |
