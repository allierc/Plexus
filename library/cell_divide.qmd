...
|---|---|---|
| `adder` | Divide once the cell has ADDED a constant volume since birth: `v >= v_birth + delta*jit*v_ref`. | `delta` (1.0) |
| `concerted` | Size AND time together: divide when `w*(v/(factor*jit*v_ref)) + (1-w)*(age/(cycle*jit)) >= 1`. | `size_weight` (0.5), `cycle` (8.0) |
| `cycle` | Divide when `cell_cycle` says the cell is in M. The trigger reads a phase and nothing else. | &ndash; |
| `doubler` | Divide at `factor` x THIS CELL'S OWN BIRTH VOLUME -- the rule that was the default until kept because it is the null the absolute-size rule has to beat, and because a large part of this project's recorded results were measured under it. | &ndash; |
| `timer` | Divide on the CLOCK: `age >= cycle * jit` division-calls since birth, size ignored entirely. | `cycle` (8.0) |
