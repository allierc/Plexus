...
::: {.op-head}
![](../figures/icons/die.png){.op-logo}

[`die`]{.op-badge} [`acts on: vertex`]{.op-badge} [`prediction: none`]{.op-badge}
...
One of these goes in the spec as `model:`; each is a different claim about the tissue, with its own reference. Omitting `model:` gives the operator described above, which is the default and is not repeated here.

| model | what it claims | its own parameters |
|---|---|---|
| `chem_low` | `chem_low` model of cell_die: below an absolute activator threshold, so cells die BETWEEN the spots of a pattern. | &ndash; |
| `competition` | `competition` model of cell_die: a cell growing more slowly than its neighbours is out-proliferated and removed -- the Myc-style loser of cell competition. | &ndash; |
| `crowded` | `crowded` model of cell_die: a cell with more neighbours than a trivalent sheet should give it -- density-driven extrusion. | &ndash; |
| `dimmer` | `dimmer` model of cell_die: a cell carrying less activator than its neighbours -- the LOCAL form of `chem_low`, which compares against the whole tissue instead. | &ndash; |
| `field` | `field` model of cell_die: death keyed to a per-cell quantity SOME OTHER operator published, so a new measurement needs no new branch here. | &ndash; |
| `lonely` | `lonely` model of cell_die: a cell with fewer neighbours than a sheet should give it, so the tissue closes over a gap. | &ndash; |
| `older` | `older` model of cell_die: a cell that has gone longer without dividing than its neighbours -- one that has stopped cycling while the tissue around it has not. | &ndash; |
| `prescribed` | `prescribed` model of cell_die: the EXPERIMENTER ablates a patch, and the tissue did not choose it. | &ndash; |
| `small` | `small` model of cell_die: below an ABSOLUTE volume threshold, not a relative one -- the one selection that is genuinely about the cell's own state and not about where it sits. | &ndash; |
| `smaller` | `smaller` model of cell_die: a cell smaller than its neighbours is squeezed out by a tissue that can no longer accommodate it. | &ndash; |
| `stalled` | `stalled` model of cell_die: below an absolute GROWTH-RATE threshold -- a cell that has stopped growing, judged against a number rather than against its neighbours. | &ndash; |
