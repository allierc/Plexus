...
def _where(mech_id) -> str:
    return (f"YOUR WORKING COPY: `atlas_cc3d/_work/{mech_id}.yaml` -- this file holds this "
            f"ONE mechanism entry and nothing else. EDIT THAT FILE. Do not open or edit "
            f"`atlas_cc3d/atlas_record.yaml`: the driver merges your entry back into it "
            f"after validating it, and an edit there is reverted.\n"
            f"YOUR NOTE: `atlas_cc3d/campaign/notes/{mech_id}.md` -- create or append; the "
...
2. Write the oracle script under `atlas_cc3d/_oracle/scripts/<id>.py`; run it with
   `cd atlas_cc3d && python oracle.py run _oracle/scripts/<id>.py --name diff_<id>`. It must write a `.npz` and
   a `summary.json` into its run directory.
3. Write the matching Plexus spec under `config/atlas_cc3d/<id>.yaml` and RUN IT THROUGH THE ENGINE
   with `python atlas_cc3d/run_spec.py <id>`, which writes a real evidence folder at
   `log/atlas_cc3d/<id>/` in the same shape as `log/okuda/coral_fixed_ball/`:
...
3. Write a spec in `config/atlas_cc3d/` that composes it with at least one operator Plexus already
   had, and run it with `run_spec.py` so it leaves an evidence folder under `log/atlas_cc3d/`. A
