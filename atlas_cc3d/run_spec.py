...
auto-scaled to its own contents.
    ...
    python run_spec.py <name>              # config/atlas_cc3d/<name>.yaml
...
CONFIG_DIR = os.path.join(PLEXUS, "config", "atlas_cc3d")
LOG_DIR = os.path.join(PLEXUS, "log", "atlas_cc3d")
...
def main():
    ...
    ap.add_argument("name", help="config/atlas_cc3d/<name>.yaml, or a path to a spec")
    ...
    t0 = time.time()
    data_dir, out = data_generate(sim, "atlas_cc3d", device=a.device, erase=True)
    wall = time.time() - t0
    ...
    if not a.no_movie:
        ...
        plot_dataset(sim, "atlas_cc3d", movie=True)
