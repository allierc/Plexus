"""Property tests for the `adhere:soft_sphere` operator (jax-morph harmonic SoftSphere potential).

These assert properties statable WITHOUT the reference -- they come from the calculus of the
conservative harmonic energy U(r) = (eps/2)(1 - r/sigma)^2, not from the oracle's numbers:

* PURELY REPULSIVE, ANALYTIC FORCE -- two overlapping cells push apart with magnitude exactly
  f(r) = (eps/sigma)(1 - r/sigma). This is -dU/dr done by hand; matching it verifies that autodiff
  of the energy WITH the 1/2 prefactor yields a unit-per-sigma force coefficient (the whole point
  of the 1/2 = 1/exponent), and that the force points away from the neighbour.
...
* C1 (HARMONIC) SOFTNESS -- the force is exactly LINEAR in the separation r inside contact, so it
  reaches 0 at contact with a FINITE, nonzero slope (equal steps in r give equal drops in force).
  This is the signature of the exponent-2 harmonic core and the distinction from `hertzian`
  (5/2 exponent), whose force slope ALSO vanishes at contact (C2).
...
import plexus.operators.candidates.jax_morph_soft_sphere  # noqa: F401  (registers adhere:soft_sphere)
...
def _op(params=None):
    return get_operator("adhere", "soft_sphere")((params or {}), "cpu")


def _soft_force(r, sigma, eps):
    """The analytic radial force magnitude f(r) = (eps/sigma)(1 - r/sigma), 0 beyond contact.
    Derived by hand from -dU/dr with U = (eps/2)(1 - r/sigma)^2 -- NOT read from the source."""
    if r >= sigma:
        return 0.0
    return (eps / sigma) * (1.0 - r / sigma)


def test_overlap_is_repulsive_with_analytic_magnitude():
    """Two cells overlapping within contact push APART, with the exact harmonic force magnitude."""
    eps, r_cell = 1.0, 0.5
    ...
    f = _soft_force(s, sigma, eps)                           # (1/1)(1 - 0.5) = 0.5
...
def test_force_is_linear_in_separation_c1_at_contact():
    """C1 harmonic signature: the force is EXACTLY LINEAR in r inside contact, so equal steps in r
    give equal drops in force and it reaches 0 at contact with a FINITE nonzero slope (contrast the
    C2 hertzian, whose slope also vanishes)."""
    eps, r_cell = 1.0, 0.5
    sigma = 1.0
    seps = [0.2, 0.4, 0.6, 0.8]                              # equally spaced separations (step 0.2)
    mags = []
    for s in seps:
        H, lvl = _world([[0.0, 0.0], [s, 0.0]], radius=r_cell)
        out = _op({"epsilon": eps})(H, lvl.active)["cell"]
        mags.append(float(out[1].norm()))
        assert math.isclose(mags[-1], _soft_force(s, sigma, eps), rel_tol=1e-4)
    # linear force -> constant first difference; the slope is -eps/sigma^2 = -1 over a step 0.2.
    diffs = [mags[k + 1] - mags[k] for k in range(len(mags) - 1)]
    assert all(math.isclose(dd, diffs[0], rel_tol=1e-4) for dd in diffs)   # equal decrements
    assert math.isclose(diffs[0], -(eps / sigma ** 2) * 0.2, rel_tol=1e-4)  # finite nonzero slope at contact
...
def test_size_consistent_contact_distance():
    ...
    assert math.isclose(fc, _soft_force(0.9, 1.5, eps), rel_tol=1e-4)
...
def test_dead_cells_are_masked_externally():
    ...
    f = _soft_force(s, 2 * r_cell, eps)
    assert torch.allclose(out[0], torch.tensor([-f, 0.0]), atol=1e-5)
    assert torch.allclose(out[1], torch.tensor([+f, 0.0]), atol=1e-5)


def test_per_cell_epsilon_mixes_by_arithmetic_mean():
    """A per-cell strength field is combined per pair by the arithmetic mean 0.5*(eps_i + eps_j),
    ...
    f = _soft_force(s, sigma, 3.0)                                    # mean(2,4) = 3
    assert torch.allclose(out[1], torch.tensor([+f, 0.0]), atol=1e-5)
...
def test_force_is_differentiable_wrt_positions():
    ...
    assert torch.isfinite(lvl.state.grad).all()                      # no NaN from the double-where guard
