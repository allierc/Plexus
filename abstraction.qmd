...
dispatched by the relation it acts on. Four operators move *state* along a fixed
structure, one per relation the container admits, and two *structural* operators
change a set's membership. The four state operators:
...
**Two structural operators: Divide and Die.** The four above move *state* while
the sets, edges, and maps stay fixed. Living matter also rewrites its own
structure: cells divide and die, so the cardinality $|S_k|$ itself grows and
shrinks. Two operators do exactly this — **Divide** $\mathcal{B}: x \mapsto
\{x',x''\}$ (proliferation) and **Die** $\mathcal{D}: x \mapsto \varnothing$
(apoptosis / efflux) — the only operators that change a set's cardinality. For a
*living* system they are as primordial as the other four; everything else
assumes the cells it acts on already exist.

![The six operators. The first four move state along a fixed structure; *Divide* and *Die* are structural — they change a set's membership (cell division and death), carried on a fixed buffer by a per-element occupancy mass $w$.](figures/fig_ops.png){width=100%}
...
Sets, fields, and these six operators are the entire vocabulary. The wager of
...
| make an element | structural: division | $\mathcal{B}:x\!\mapsto\!\{x',x''\}$ | `Divide` |
| remove an element | structural: death | $\mathcal{D}:x\!\mapsto\!\varnothing$ | `Die` |
