...
\begin{scope}[shift={(128mm,-5.5mm)},local bounding box=RW]
  \node[n] (rc){};
...
\end{scope}
\node[anchor=north,align=center] at (DV |- 0,-15mm){\textbf{Divide}\\$w\!\to\!\tfrac{w}{2},\tfrac{w}{2}$};
\begin{scope}[shift={(190mm,-5.5mm)},local bounding box=DE]
  \node[n,densely dashed,fill=cset!3] (z){};
  \node[right=8mm of z] (nul) {$\varnothing$};
  \draw[a] (z)--(nul);
\end{scope}
\node[anchor=north,align=center] at (DE |- 0,-15mm){\textbf{Die}\\$w\!\to\!0$};
\end{tikzpicture}
\end{document}
