...
That is not a numerical convenience. At this scale viscous drag beats inertia by about ten orders of
magnitude, so $\gamma\dot{\mathbf x}=\mathbf F$ is the correct equation of motion, and a sheet given a
mass instead would overshoot and oscillate --- behaviour we used to observe and tune around, which does
not exist here. And what is called adhesion here is a tether to a surface, not a
