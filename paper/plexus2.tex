...
\begin{quote}
\itshape
Il semble que la perfection soit atteinte non quand il n'y a plus rien \`a ajouter,
mais quand il n'y a plus rien \`a retrancher.\\[2pt]
(It seems that perfection is attained not when there is nothing more to add,
but when there is nothing more to take away.)
\upshape\\[2pt]
\footnotesize Antoine de Saint-Exup\'ery, \emph{Terre des hommes} (1939), ch.~III
\end{quote}
...
\upshape\\[2pt]
\footnotesize Richard P.~Feynman, blackboard at Caltech, February 1988
\end{quote}
...
laws of physics.
\upshape\\[2pt]
\footnotesize Richard P.~Feynman, \emph{The Feynman Lectures on Physics}, Vol.~I (1963), ch.~1
\end{quote}
...
Activities are therefore typed morphisms between kinds of entities. What an activity does
\emph{not} carry is the relation it acts through. Maps define the relations a mechanism
requires --- which particle belongs to which cell, which synapses connect which neurons, how
cells relate to a continuous field --- and each is declared once, with the structure that
carries it: the containment map $\pi$ of a set, or the two legs of a relation given entity
status (Section~\ref{sec:hierarchy}). An activity reaches the relations it needs by traversing
those declarations at run time, through the gather and scatter primitives the language
provides, rather than by restating them in its own signature.

The separation is what makes activities reusable across organizations. One adhesion acts over
a neighbour graph, a mesh half-edge table or a contact list without being rewritten, and a
Rewire that changes the relation changes which entities participate without touching the
activity at all. It also keeps the two declarations from drifting apart, which they do as soon
as both exist: a signature that names the maps it walks is a second copy of a fact the
structure already states, and the copy is only as good as the discipline maintaining it.
