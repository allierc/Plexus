...
without which an agent elaborates operators to explain an artifact. A fourth responsibility keeps the
process honest once the metrics are met: \emph{perfecting}. A satisfied threshold is not the same as
qualitative fidelity to the reference, and the master must notice discrepancies that \emph{no current
metric captures} --- cells that are triangular where the reference's are round, a boundary that is ragged
where it should be smooth --- and treat each as a prompt to author a \emph{new} metric and refine toward
it. This closes the loop between qualitative observation and quantitative objective: the eye (or the
reference figure) flags a gap, the master turns the gap into a measurable target, and the loops close it.
Perfecting is therefore not obsessive polishing but the mechanism by which the goal itself is progressively
sharpened as the model approaches the reference. It is also a deliberate counterweight to the characteristic
failure mode of a fast operator, human or agent: advancing to the next stage before the current one is
genuinely settled. The master holds each stage open until its metric is met and refuses the tempting
shortcut of ``good enough to move on''; the discipline it supplies --- finish the step, verify it, only
then proceed --- is precisely the one a hurried operator most often skips. The master thus keeps a multi-round,
