...
The chain $\pi_k:S_k\to S_{k+1}$ is the right intuition --- and the place to start
--- but read literally it over-constrains in two ways: it assumes each scale has
\emph{one} child type, and it treats every entity as an element of one big set,
told apart only by a label. A real cell is neither: it is a container of many
\emph{different kinds} of set at once --- a membrane, a cytoplasm, a nucleus,
organelles, molecules --- each with its own state space and its own operators. So
we make two moves: from one set to a \emph{family} of typed sets, and from a chain
to a graph.
