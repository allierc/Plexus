...
Existing simulation frameworks therefore become alternative backends
implementing the same operator contracts, separating biological semantics from
numerical realization.

The Plexus research program has three objectives. First, to define an algebra of
biological operators that can be composed, compared, differentiated, replaced,
and ultimately discovered. Second, to decompose the existing corpus of
simulation models into this common language and identify a compact reusable
vocabulary of biological mechanisms. Third, to construct differentiable
implementations of these operator compositions so that mechanistic models can be
directly inferred from biological imaging.

Plexus therefore proposes a common language in which simulation, inverse
modelling and mechanistic discovery become successive stages of constructing,
testing and extending executable biological theories.
...
These frameworks have enabled major advances within their respective domains,
yet they remain largely incompatible. Combining electrical signalling,
mechanics, metabolism and development often requires coupling multiple
specialized simulators whose underlying abstractions differ fundamentally.

This fragmentation is not primarily a numerical problem but a language problem.
Each simulator introduces its own representation of biological mechanisms,
making models difficult to compare, compose and extend. Consequently, inverse
modelling typically operates within a fixed simulator, optimizing parameters of
an assumed mechanism rather than reasoning about alternative mechanistic
descriptions.

We propose a different perspective. Instead of treating simulation frameworks as
fundamental, we treat biological mechanisms as the primary computational
objects. Plexus introduces a typed intermediate representation whose central
elements are biological operators. Operators describe mechanisms such as
adhesion, diffusion, division, chemotaxis, electrical propagation or active
stress independently of their numerical implementation. Different simulation
frameworks then become alternative implementations of these operators rather
than independent modelling paradigms.

This viewpoint defines a broader research program.

First, Plexus seeks to construct an algebra of biological operators whose
elements can be composed into complex dynamical systems, compared through their
observable consequences, differentiated for inverse modelling, and expanded as
new mechanisms are discovered.

Second, Plexus aims to extract this operator algebra from the existing corpus of
biological simulation models. Rather than viewing current frameworks as
competing simulators, we regard them as repositories of reusable mechanisms. By
decomposing these models into a common language, we seek to identify a compact
vocabulary capable of expressing a wide range of biological dynamics across
scales.

Third, Plexus constructs differentiable counterparts of these operator
compositions. Differentiability is therefore not the objective but the enabling
technology allowing mechanistic models to be fitted directly to biological
imaging, residuals to be interpreted mechanistically, and missing operators

In this perspective, biological modelling becomes analogous to compiler design.
Just as modern compilers separate programming languages from their hardware
implementations through a typed intermediate representation, Plexus separates
biological mechanisms from their numerical realizations. Existing simulators
become alternative backends implementing the same operator contracts, while the
language itself becomes the primary object of scientific investigation. Because
mechanisms are represented in a formal language rather than embedded in
simulator-specific code, they become directly accessible to agentic systems,
which can assemble, compare, modify and extend mechanistic hypotheses
automatically. Mechanistic model discovery is therefore recast as the
progressive construction, evaluation, and extension of an executable language
of biological mechanisms.

\section{A typed operator algebra}

Plexus is built around three fundamental concepts: \emph{sets},
\emph{states}, and \emph{operators}. Among them, operators are the primary
objects of the language. A biological model is therefore not defined by a
simulator, but by a composition of biological operators.

An operator represents one biological mechanism independently of its numerical
implementation. Examples include adhesion, chemotaxis, diffusion, electrical
propagation, active stress, metabolism, growth, and cell division. Operators
can be composed into larger dynamical systems, compared through their
observable consequences, differentiated for inverse modelling, replaced by
alternative numerical implementations, and ultimately discovered from
experimental observations.

Every operator is defined by a \emph{typed signature} and one or more
implementations.

The typed signature specifies

\begin{itemize}
\item the input sets,
\item the output sets,
\item the maps relating these sets,
\item the state variables consumed and produced.
\end{itemize}

The implementation specifies how this transformation is numerically realized.
Different numerical methods may therefore implement the same operator without
...
\texttt{chemotaxis}
:
(\mathrm{Cell},\mathrm{ConcentrationField})
\xrightarrow{\mathrm{sample}}
\mathrm{Cell}
\]

states that cells interact with a concentration field through a
\texttt{sample} map. Likewise,
...
reduces particle information onto their parent cells through the
\texttt{parent} map, while

\[
\texttt{synaptic\_transmission}
:
(\mathrm{Neuron},\mathrm{Synapse})
\xrightarrow{\mathrm{pre},\,\mathrm{post}}
\mathrm{Neuron}
\]

requires the \texttt{pre} and \texttt{post} maps relating synapses to their
presynaptic and postsynaptic neurons.

Maps are therefore not independent primitives of the language. They are part of
the operator signature, defining how biological entities are related whenever a
mechanism is applied. Operators become typed morphisms between biological sets,
while implementations become interchangeable numerical realizations of the same
