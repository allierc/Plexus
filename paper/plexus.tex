...
a common abstraction of living systems. Plexus is constructed by decomposing the
large existing corpus of simulation models into a common representation. This
includes neural computation (Hodgkin--Huxley, connectome and neural-network
models), multicellular dynamics (vertex, Cellular Potts and active-matter
models), transport processes (diffusion, chemotaxis and reaction--diffusion),
and developmental dynamics (embryogenesis and morphogenesis).

Within Plexus, every simulation model is expressed through five primitives:
\emph{sets} of biological entities, the \emph{states} carried by each entity,
continuous spatial \emph{fields}, \emph{maps} describing relationships between
entities and representations, and \emph{operators} implementing biological
mechanisms. Operators are further organized by the hierarchy of interactions
they implement (set--set, set--field, field--field, and parent--child). Every
primitive is paired with a differentiable counterpart: operators become
differentiable functions, maps become differentiable transformations, and
fields may be represented explicitly or as neural implicit representations.
Models originating from different simulation paradigms can therefore be
expressed, composed, compared, and optimized within a single formalism without
sacrificing mechanistic interpretability.

This representation enables mechanistic analysis that goes beyond forward modelisation.
Competing models can be fitted against biological observations, their residuals
compared, and persistent discrepancies interpreted as evidence for missing
mechanisms rather than suboptimal parameters. Plexus provides a common language 
in which simulation, inverse modelling, mechanistic analysis, and hypothesis generation 
become successive stages of mechanistic model discovery.
...
changes exactly one of the things the container holds
(Fig.~\ref{fig:ops}): the \textbf{state} on the nodes, the \textbf{field} itself, the
\textbf{relations} between them, or the \textbf{entities} themselves. \textbf{Four dynamics
operators} move \emph{state} along a fixed structure, one per relation the container admits,
each returning a time-derivative contribution. \textbf{Field} operators are a field's own
dynamics --- diffusion, decay, reaction --- mapping field to field. \textbf{Rewire} changes the
\emph{relations} --- the edge set $E$, which nodes interact. \textbf{Structural} operators change
the \emph{entities} --- the node set $|S_k|$, which nodes exist: \textbf{Divide} and \textbf{Die}
are its two members. \textbf{Seed} writes the initial state once, at $t=0$. All but the first four
return no state delta. Eight kinds in all, and the implementation
(\code{plexus.models.base.KINDS}) recognises exactly these. The four dynamics operators:
\begin{align*}
...
\paragraph{Structural: Divide and Die (entities).} The dynamics operators move \emph{state}
...
\end{scope}
\node[below=1mm of FD,align=center]{\textbf{Field}\\$\partial_t c$};
...
\end{scope}
\node[below=1mm of DV,align=center] (DVL) {\textbf{Divide}\\$w\!\to\!\tfrac{w}{2},\tfrac{w}{2}$};
\begin{scope}[shift={(222mm,0)},local bounding box=DE]
  \node[n,densely dashed,fill=cset!3] (z){};
  \node[right=8mm of z] (nul) {$\varnothing$};
  \draw[a] (z)--(nul);
\end{scope}
\node[below=1mm of DE,align=center] (DEL) {\textbf{Die}\\$w\!\to\!0$};
% ONE KIND, TWO OPERATORS: `structural` is the registry's name for the kind that changes the node
% set, and Divide and Die are its members -- drawn inside it rather than beside it.
...
\end{scope}
\node[below=1mm of SD,align=center]{\textbf{Seed}\\$t=0$};
\end{tikzpicture}}
\caption{The eight operator kinds, grouped by what they change. The first four are
\emph{dynamics}: they move a set's \emph{state} along a fixed structure --- \emph{Lateral}
...
both Exchange). \emph{Field} is a field's own dynamics --- diffusion, decay, reaction ---
field to field, returning no set delta. The rest return no state delta either: \emph{Rewire}
changes the \emph{relations} --- the edge set $E$, which nodes interact (a radius graph rebuilt
each tick, dashed cutoff); \emph{Structural} changes the \emph{entities} --- the node set
$|S_k|$, and its two operators are \emph{Divide} and \emph{Die} (dashed box), cell division and
death on a fixed buffer with per-element occupancy $w$; \emph{Seed} writes the initial state
once, at $t=0$, and never again.}
\label{fig:ops}
\end{figure}
...
Aggregate, Broadcast, Exchange, Field, Rewire, Structural (Divide and Die), and
Seed are the primitive
