...
over-constrains: it assumes each scale has \emph{one} child type. A real cell does
not --- it is a container of many sets at once: a membrane, a cytoplasm, a nucleus,
organelles, receptors, molecules, each with its own state, operators, and parent
map. So we lift the chain to a graph.

Containment is a parent map between \emph{any} two sets, declared by naming the
parent:
\[
  \pi_{A\to B} : S_A \longrightarrow S_B,
\]
assigning every entity of type $A$ to a parent of type $B$. A parent set may have
many children, so a cell is not one fibre but a \textbf{bundle of fibres},
\[
  \mathrm{Cell}(c)=\Big\{\, \pi_{A\to\mathrm{cell}}^{-1}(c) \;:\; A\in\mathrm{children}(\mathrm{cell}) \,\Big\}.
\]
The container is therefore a \emph{typed containment graph} --- a \textbf{DAG}
(directed acyclic graph: the sets are nodes, the parent maps are directed edges,
and containment never cycles), shown in Fig.~\ref{fig:graph} --- not a fixed scale
ladder. A DAG rather than a tree because a parent has \emph{many} children. The
shift is from
``\emph{level $k$ contains level $k{-}1$}'' to ``\emph{set $A$ is contained by set
$B$}'', and it is what keeps the code clean: the engine needs only
\code{Level.parent} (the index map) and \code{Level.parent\_name} (the edge); new
biological structure is a \emph{new set and a parent map}, never an engine change.

This also keeps operators \emph{typed}: \code{membrane\_tension} acts on
...
membrane\,$\leftrightarrow$\,extracellular field). The alternative --- one set of
generic particles carrying a \code{role} flag, driven by one mega-operator that
branches on the role --- is precisely the spaghetti a typed graph avoids.
