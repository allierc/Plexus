...
\code{disk\_ic}              & structural & --- & frame-0 IC: rotating self-gravitating disk (near-circular orbits + central mass) \\
\code{cohesion},\,\code{alignment},\,\code{separation} & lateral & acceleration & the three boids rules; their deltas sum to flocking \\
\code{cruise}                & lateral & acceleration & Vicsek self-propulsion toward cruising speed $v_0$ \\
...
\code{heading\_align},\,\code{flow\_align} & exchange & --- & align \code{heading} to neighbours / to solved MPM flow \\
\code{pulse\_to\_contraction} & exchange & mpm\_accel. & activation-gradient body force (cardiac contraction) \\
\code{pulse\_to\_active\_stress} & exchange & --- & activation $\to$ active-stress tensor (\code{H.active\_stress}) \\
\code{apply\_material\_map}  & exchange & --- & image field $\to$ per-particle material (\code{youngs}) \\
\code{agent\_to\_mpm},\,\code{mpm\_to\_agent},\,\code{agent\_remodel} & exchange & --- / vel. & two-way agent\,$\leftrightarrow$\,material coupling \\
...
\code{mpm\_strain},\,\code{p2g},\,\code{mpm\_grid\_update},\,\code{g2p} & lat./exch./field & --- & the decomposed MLS-MPM cycle (strain\,$\to$\,P2G\,$\to$\,grid solve\,$\to$\,G2P) \\
\code{mls\_mpm\_mechanics}   & exchange & --- & the same cycle as one fenced \emph{transitional} oracle (Section~\ref{sec:roadmap}) \\
\midrule
\multicolumn{4}{@{}l}{\emph{Fields --- a field's own dynamics (\code{kind=field}; write the grid, return \{\})}}\\
...
\midrule
\multicolumn{4}{@{}l}{\emph{Relations --- change the edge set $E$ (\code{kind=rewire})}}\\
\code{radius\_graph}         & rewire & --- & neighbour graph: live pairs within a cutoff, rebuilt each tick \\
\midrule
\multicolumn{4}{@{}l}{\emph{Entities --- change the node set $|S|$ (\code{kind=structural})}}\\
...
\item ``moves by boids / flock'' $\to$ \code{cohesion}$+$\code{alignment}$+$\code{separation}; ``wanders / motile'' $\to$ \code{glide}.
...
Adding the \code{sense} operator (a Physarum/Jones three-sensor rule: each ant
samples pheromone ahead-left / ahead / ahead-right and turns toward the strongest,
while \code{glide} drives it forward and \code{deposit} lays pheromone) yields
...
\caption{Ten ant variants, each the same \code{sense}$+$\code{glide}$+$\code{deposit}
...
it is assigned to a family --- motion/migration, chemical signalling, field sampling,
continuum (MPM) mechanics, growth, activation, coupling, measurement/scorecard, or
visualization. An operator enters the core only when its family is clear. This keeps
the registry a small set of reusable physical primitives rather than a flat pile of
one-off biological events.
...
\code{pulse\_to\_contraction} emits an \code{mpm\_acceleration}, whereas
\code{pulse\_to\_active\_stress} writes an active stress and emits nothing --- those
