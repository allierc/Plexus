...
\begin{abstract}
Living systems are simultaneously mechanical, chemical, electrical, and
developmental, yet existing computational frameworks each specialize in one aspect
--- particle mechanics, reaction--diffusion, graph neural networks, or multicellular
dynamics. Plexus starts from the observation that these are not different simulators
but different \emph{operators} acting on the same biological objects.

We introduce Plexus, a formal language whose objects are five primitives:
\emph{sets} of discrete entities across scales; the \emph{state} each element
carries; \emph{fields} of continuous quantities over space; \emph{maps}, index
relations between sets (containment and incidence alike); and \emph{operators} that
transform state along maps. State is what makes the sets interchangeable: a particle
carrying $\{\code{pos},\code{vel}\}$ and a neuron carrying
$\{\code{voltage},\code{calcium}\}$ are the \emph{same} object under different
schemas. An operator is a \emph{contract} --- \code{neuron\_voltage} names ``membrane
state $\to$ its derivative,'' which Hodgkin--Huxley, Izhikevich, a Neural ODE, or an
MLP all satisfy interchangeably --- so choosing biophysical or learned dynamics is
choosing an implementation, never editing the engine.

Plexus is therefore not another simulator but a typed \emph{intermediate
representation} for biological computation: models written for \textsc{Neuron},
Jaxley, or a connectome GNN compile to the same sets, states, maps, fields, and
operators, and existing simulators become \emph{backends} that implement one family
of operator contracts --- Plexus is to a biophysics simulator what \textsc{LLVM} is
to a C compiler. This single representation unifies mechanics, signalling, transport,
growth, and neural computation, and is also a foundation for automated discovery.
Because mechanisms
are explicit, composable operators, inverse modelling can search not only parameter
values but competing mechanistic explanations, and a persistent residual is read as
evidence for a \emph{missing} operator --- so the language itself expands as new
mechanisms are found. Simulation, inference, and hypothesis generation become stages
of constructing an increasingly expressive language for living systems.
\end{abstract}
