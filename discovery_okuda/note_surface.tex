...
S5 & the same on a curved, dividing, re-meshed surface & $<10^{-6}$ & \pass{1.2e-7} max,
     \pass{1.5e-8} median\\
S6 & the penalty does not run away: penetration bounded & $<2$ cells & \pass{1.47} (03b),
     \pass{0.82} (04), \pass{0.73} (04d) --- a NUMERICAL bound; U1 asks the physical one\\
...
\multicolumn{4}{@{}l}{\textit{is the failure rate-limited?} (\op{03e\_press\_rate}, press over $8\times$
at matched travel)}\\
S17& the answer converges as the press slows & change between the two slowest $<0.05$ &
     \pass{0.011} ($19.49\to14.95\to15.44\to15.61\%$)\\
S18& the $k$-dependence weakens when the rate falls & spread $<0.10$ (it is $0.119$ at nominal) &
     \fail{0.136} ($14.19/15.61/16.31\%$) --- it does NOT\\
S19& friction changes the slip on a CURVED, moving surface & $>2\times$, as on the flat patch &
     \fail{1.73$\times$} ($5.22$ vs $3.02\times10^{-2}$, \op{04e})\\
\multicolumn{4}{@{}l}{\textit{the scale the note is quoted in} (\op{0u\_units})}\\
U1& penetration on the spheroid, against a cell diameter & $[0, 10]$ um & \fail{15.02 um}\\
U2& penetration on the flat rig & $[0, 30]$ um & \pass{26.93 um}\\
U3& spheroid radius at the last frame & $[50, 400]$ um & \pass{175.8 um}\\
U4& the gel around it & $[200, 2000]$ um & \pass{468.9 um}\\
U5& the speed the surface advances & $[0.5, 20]$ um/h & \pass{3.869 um/h}\\
...
\textbf{S6 and U1 are one measurement against two thresholds, and that is the point of declaring
units.} S6 asks whether the penalty runs away and answers no, in grid cells; U1 asks whether the
surface is a surface and answers no, in micrometres. A threshold in grid cells is a threshold in the
currency of the discretisation, which is the currency that makes it easiest to pass --- and the same
$0.82$ that reads as a comfortable improvement on $1.47$ reads as $15\,\mu$m, nearly two cell
diameters, against a basement membrane of $0.1\,\mu$m. Neither row is wrong; the failure was having
only the first. \textbf{The rule this yields is general: a gate's threshold belongs in the unit of the
phenomenon, not of the mesh.}
...
\textbf{S13--S15 are the finding of this note, and S17--S18 are what became of the explanation.} A penalty permits a depth
...
$13\%$; halving the time step moves it again, $14.95\to18.50\%$.

\textbf{The obvious explanation was that the contact is rate-limited}, and \op{03e} rejected it. The
surface is prescribed, so it advances $v\Delta t$ per step whatever the matrix does; if the penalty
cannot accelerate a particle out of the way within a step, the depth is set by the advance rather than
by the force, and all three sweeps become sweeps of how much advance happens per step. That is
testable, because it predicts two things. The first holds: pressing at $2$, $1$, $\tfrac12$ and
$\tfrac14$ of the nominal speed at matched total travel, the compression converges --- $19.49\to14.95
\to15.44\to15.61\%$, the last two within $1.1\%$ of each other (S17), so the nominal press is already
converged to $4\%$ and the time-step failure S15 is a resolution question rather than a mechanism.
\textbf{The second fails.} At the slowest press the $k$-dependence does not weaken; it is
$13.6\%$ over a $16\times$ change in $k$ against $11.9\%$ at the nominal (S18). A rate-limited contact
must lose its $k$-dependence when the rate falls, and this one does not --- so the compliance is
intrinsic to the penalty and not an artefact of pressing too fast, and the answer is the barrier
formulation (BFEMP \cite{li2022}) rather than a slower press. That is a prediction made before the run
and refuted by it, which is the only reason to have made it.

\textbf{S19 is the same lesson in the other direction.} Friction changes the slip four- to fivefold on
the flat patch, and only $1.73\times$ on the spheroid --- because the spheroid's loading is very nearly
pure normal expansion, so there is little tangential relative motion for a friction law to act on. The
threshold was set on the flat rig and does not transfer: what \op{04e} shows is not that the friction
law is broken but that the curved run does not exercise it, and a tangential gate for the spheroid
needs a loading with shear in it.

What survives all of this is S12, and S16 with it: the reaction is exact at $1.5$--$1.8\times10^{-7}$
in every variant of every sweep --- four press speeds, three stiffnesses, two time steps, two grids,
two friction coefficients --- because Eq.~\eqref{eq:reaction} is an identity on $\mathbf w$ and not a
balance that has to converge. And refining the grid at fixed particle count moves the compression by
$12\%$ and the penetration in box units by $7\%$, which is the material's response and not the
interface's: the one place the scheme's own claim, that particle-to-surface contact never consults the
grid, is tested and holds.
