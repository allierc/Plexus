...
\code{grow\_3d}, \code{divide\_3d}  & Divide          & \code{cell}     & volume growth and cytokinesis & cells $200\to6076$, radius $\times3.56$\\
\code{shape\_energy\_3d}            & Lateral         & \code{junction} & vertex-model force balance & aspect $1.021$, stays round\\
\code{reconnect\_t1\_3d}            & Rewire          & \code{junction} & neighbour exchange & $0.0089$ T1 per cell per frame\\\midrule
\multicolumn{5}{@{}l}{\textit{epithelium $\to$ basement membrane}}\\
\code{bm\_secrete}                  & Divide          & \code{bm}       & polarised deposition of laminin, collagen~IV, nidogen, perlecan & areal density $\rho$ held constant as $A(t)$ grows\\
\code{bm\_assemble}                 & Rewire          & \code{bm}       & laminin polymerises where integrin and dystroglycan nucleate it & no sheet forms where the receptor is absent\\
\code{bm\_degrade}                  & Die             & \code{bm}       & MMP proteolysis, local and MMP-independent breach & a hole opens only where protease is present\\\midrule
\multicolumn{5}{@{}l}{\textit{basement membrane $\leftrightarrow$ epithelium}}\\
\code{plaque\_seed}                 & Rewire          & \code{plaque}   & hemidesmosomes form at density $\Sigma^{-2}$, $\Sigma\approx7T$ & plaque count and spacing\\
\code{plaque\_pull}                 & Lateral         & \code{plaque}   & Eq.~\eqref{eq:adh}: normal spring at rest length $\ell_0$, tangential friction $\xi$, \emph{and its reaction} & standoff $=\ell_0$; momentum conserved to machine precision\\
\code{plaque\_rupture}              & Rewire          & \code{plaque}   & the bond fails past a load & fraction bound vs.\ applied stress\\
\code{bm\_stiffness}                & Lateral         & \code{bm}       & Eq.~\eqref{eq:bmstiff}: tangent modulus rising with stretch & $E$ $0.4\to3$ MPa across the run\\\midrule
\multicolumn{5}{@{}l}{\textit{basement membrane $\leftrightarrow$ matrix}}\\
\code{fibril\_seed}, \code{fibril\_pull} & Rewire, Lateral & \code{fibril} & collagen~VII anchoring; the stroma is loaded \emph{through} the sheet & stroma displacement with the sheet present vs.\ absent\\
\code{mpm\_scatter}/\code{gather}   & Exchange        & \code{bm}, \code{ecm} & both bodies share one momentum field & contact without an explicit contact law\\
\code{bm\_barrier}                  & Exchange        & \code{bm}, fields & pores $10$--$130$ nm pass solutes, stop cells & solute flux across an intact vs.\ breached sheet\\\midrule
\multicolumn{5}{@{}l}{\textit{matrix}}\\
\code{ecm\_align}, \code{ecm\_crosslink} & Lateral    & \code{ecm}      & fibre reorientation under load; LOX stiffening & alignment angle vs.\ radial direction\\
\bottomrule
\end{tabular}
\end{center}
