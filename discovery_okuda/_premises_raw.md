...
- constrains: `cell_grow.rho (the baseline floor in growth = rate*(rho + hill(a))), and the presence/absence of a body-wide growth operator such as cell_grow in the schedule`
- check: `rho > 0, OR a body-wide growth operator (cell_grow/uniform_ramp) is present in `schedule`. Equivalently, measured: mean over cells of d(v_eq)/dt > 0 at every recorded frame, and total tissue volume V_tot(t_end) > V_tot(t_growth_start).`
- if violated: "rho = 0 with no uniform growth operator means the body adds no material at all: growth = rate*hill(a) is exactly zero for every cell whose activator is below a_sw, and the tissue can only redistribute the volume it was seeded with. Is that a deliberate ablation? If so, declare it and predict what the protrusion is made of; if not, set rho > 0 or add cell_grow to the schedule."
...
- constrains: `the cell_chem_react -> cell_grow `gate` connection in _discovery.structure.connections, jointly with cell_grow.rho and cell_grow.a_sw`
...
- constrains: `cell_divide.min_cycle and cell_divide.vcap jointly with cell_grow.rate, .rho and .vth_frac, and general.dt`
...
- constrains: `the recorded time series of total tissue volume V_tot and mean cell volume, against cell_divide.max_div_frac / .max_div / .min_cycle and cell_grow.rho`
...
- constrains: `cell_grow.conserve_amount (true = divide cell.chem by the volume growth ratio each tick) jointly with cell_chem_react.rate, .mu_a, .mu_h and cell_grow.rate`
- check: `conserve_amount must be true whenever cell volume changes (otherwise concentration is silently created out of nothing), AND the dilution rate must be small against the reaction: rate*(rho + 1) << min(mu_a, mu_h)*cell_chem_react.rate. In ref_okuda_route: dilution <= 0.01 vs mu_a*rate = 1.0, a ratio of 100 — comfortable — but the check must be stated, because at rho = 1 and rate = 0.03 with a slower reaction it is not.`
...
- constrains: `cell_chem_react.a0 (Gierer-Meinhardt basal activator production, currently 0.01) jointly with cell_grow.conserve_amount and cell_grow.a_sw`
...
- constrains: `cell_chem_diffuse.d_a, cell_chem_diffuse.d_h, cell_chem_diffuse.chi, and the measured spot/peak count versus tissue linear size`
...
- constrains: `cell_divide implementation (hertwig vs orient_iface), cell_divide.factor (2.0) and cell_divide.reset_noise`
...
- constrains: `cell_divide.min_cycle, .max_div_frac and .max_div — currently a pure clock with a global throttle and no mechanical input`
...
- constrains: `cell_mechanics.p0 (default 3.90; vocabulary range 3.4-4.2) jointly with cell_grow.rho and with any protrusion/aspect metric used as a success criterion`
...
- constrains: `edge_flip.l_th_frac, .max_flips, .every — the operator flips on a pure length threshold with no energy acceptance test`
...
- if violated: "edge_flip flips every edge below l_th_frac with no energy test and a cap of 300 flips per frame. Is the flip count saturating that cap? If so the topology is being driven by the cap rather than by mechanics — declare it, or add the energy-decrease acceptance test."
...
- constrains: `cell_mechanics.p0 and cell_divide.p0 (config/okuda/*.yaml; searchable range in discovery/composition_space.py is (3.4, 4.2, 3.90)). mesh_ops sets P0 = p0 * sqrt(A0), so p0 IS the dimensionless 2D shape index.`
...
- constrains: `voronoi_tension_3d.s0 and voronoi_tension_shell.s0 (default 5.4, prototype/Turing_vertex) versus cell_mechanics.p0 (default 3.9, discovery_okuda/ops) -- and any claim that the okuda/Tyssue recipe is a "3D vertex model".`
...
- constrains: `cell_mechanics.p0 / voronoi_tension_*.s0 versus the phenotype the recipe claims: a bud/tube that persists requires the rigid side; convergent extension or flow requires the fluid side.`
...
- constrains: `tube_analysis.frame_metrics outputs shape_idx_mean / shape_idx_med / shape_idx_p95 versus the configured cell_mechanics.p0.`
...
- constrains: `cell_grow (rate, rho, conserve_amount), which drives the A0/P0 targets; cell_mechanics's per-face target V0f, which in mesh_ops is the wedge volume from the shell centre to the face (a partition of the lumen), not the cell's own volume; h0 in the monolayer implementation is the only thickness variable.`
...
- constrains: `edge_flip.l_th / edge_flip.l_th_frac (threshold = l_th_frac * mean edge length in t1_ops). config/okuda/round_44_base.yaml uses l_th_frac = 0.28; discovery_okuda/ops scripts use 0.35.`
...
- constrains: `edge_flip -- specifically the accept/refuse test in t1_ops.t1_flip_3d, which currently tests only geometric validity (non-degenerate, outward-facing, simple faces) and never the energy change.`
...
- constrains: `edge_flip: `new_len` is set equal to the flip threshold `thr`, so a freshly created junction sits exactly at the threshold and a single relaxation step can push it back below; also edge_flip.every and max_flips (300/frame in round_44_base).`
...
- constrains: `topo_record output (E_srce/E_trgt/E_face, nF, Nv) and the validity tests in t1_ops (_local_manifold_ok, _polygon_simple_2d); the broken_n / broken_frac measurable in tube_analysis.frame_metrics.`
...
- constrains: `topo_record (nF, Nv, half-edge tables) and edge_flip (asserted in its docstring to keep V, E, F and Euler = 2 fixed); the self-intersection/inversion guard cell_mechanics.antiinv (0.0 in round_44_base).`
...
- source: Euler's formula plus trivalency (textbook: 3V = 2E gives E = 3(F-2) and mean sides 2E/F = 6 - 12/F); the invariance of V, E, F under a T1 is asserted in /workspace/Plexus/discovery_okuda/ops/t1_ops.py. The biological statement that epithelial cells average six neighbours is UNCITED in these four papers.
...
- constrains: `the choice between the re-tessellated Voronoi route (voronoi_tension_3d / voronoi_graph_3d, where vertices are always shared by exactly three cells) and the explicit half-edge mesh (cell_mechanics + edge_flip); edge_flip refuses any flip whose endpoints are not trivalent.`
...
- constrains: `cell_mechanics.relax_iters (searchable range (10, 90), default 30), eta, cap_frac and the global dt; against cell_grow.rate, cell_divide.max_div_frac / max_div, and edge_flip.every (all every=1 in round_44_base).`
...
- constrains: `cell_divide.factor (2.0) x cell_divide.cycle_cv (0.4) — the trigger is vf >= factor*djit*Vbirth with djit ~ N(1, cycle_cv) clipped to [0.4, 1.8] (mesh_ops.py _fresh_djit)`
...
- constrains: `the pairing of cell_grow (rate, rho, cap, vth_frac, after_frame) with cell_divide (vcap, min_cycle, max_div_frac, after_frame). With rho = 0 the operator takes the `else` branch and clamps only the LINEAR scale s at cap (default 2.5), i.e. a volume scale of 15.6x; the per-cell v_eq cap at vth_frac*v_ref is gated behind `if self.rho > 0` and is therefore inactive in every ref_okuda_route / round_44 config.`
...
- if violated: "rho = 0 disables the v_eq cap, leaving only cap = 2.5 on the linear scale — a licence for one cell to reach 15x its neighbours' volume. Only cell_divide.vcap = 1.5 is holding cell size down, and vcap is a solver-side force-divide, not a growth law. Is hypertrophy the intended specimen? If not, report d(log N)/d(log V) and the cell-volume distribution, and state which mechanism is supposed to bound cell size."
...
- constrains: `cell_chem_diffuse.implementation. Omitting the key (as config/okuda/ref_okuda_route.yaml and round_44_base.yaml both do) resolves to `graph_laplacian`, an unweighted neighbour average that reads neither shared area nor volume; the Okuda finite-volume form is `interface_weighted`.`
...
- constrains: `cell_grow.conserve_amount (must stay true; the operator rescales c <- c * (s_prev/s)^3 when v_eq grows as s^3)`
- check: `with cell_chem_react and cell_chem_diffuse disabled, sum_j c_j * v_j must be exactly constant under pure growth; per step, a volume scaling of s^3 must be matched by a concentration scaling of 1/s^3.`
- if violated: "With conserve_amount off, growth creates activator out of nothing precisely where the activator is already highest, because growth is gated on the activator — the tip becomes a self-feeding source and every bud or tube is a mass leak, not a Turing structure. Is volume-proportional production being declared as a kinetic term? If so put it in cell_chem_react where it can be seen; if not, report the drift in sum_j c_j v_j over the run."
...
- constrains: `cell_mechanics.relax_iters (fixed 30), .eta (0.08), .cap_frac (0.12) — a fixed-iteration unrolled relaxation with no residual test anywhere`
...
- constrains: `edge_flip.l_th_frac (0.28), .every (1, i.e. every mechanical frame), .max_flips (300)`
...
- constrains: `cell_divide.min_cycle (16 frames) and cell_grow.rate (0.01 per frame) versus the mechanical relaxation delivered by cell_mechanics.relax_iters * eta per frame`
...
- constrains: `cell_mechanics.K_V (6.0) relative to cell_mechanics.kappa_s (0.2) — the volume term is a constraint penalty, not a soft spring`
...
- constrains: `cell_grow.a_sw (1.5) relative to the fixed points of cell_chem_react: gierer_meinhardt with gm_rho = 1, mu_a = 1, mu_h = 1, a0 = 0.01`
...
- constrains: `cell_chem_diffuse.d_h and .chi (which set the domain size, ~ chi^(1/4) phi^(1/2)) together with mesh_seed.n_cells (500 in ref_okuda_route)`
...
- constrains: `cell_chem_diffuse.vol_floor (0.05) and .w_cap (4.0) — guards that silently convert a collapsed or inverted wedge volume into a merely small one; also cell_mechanics.antiinv`
...
- constrains: `cell_divide.max_div (30), .max_div_frac (0.0075), .vcap (1.5, a force-divide override); edge_flip.max_flips (300); cell_mechanics.cap_frac (0.12)`
...
- constrains: `cell_divide (all implementations) via cell_divide.cell_set — the chem state written to the two fresh daughters`
...
- constrains: `general.boundary (free), general.world ([80, 80, 80]), and the friction model in cell_mechanics (.eta, .mu) — whether drag is taken relative to the local tissue velocity field or to the lab frame`
...
- constrains: `cell_divide.cycle_cv (0.4) and cell_divide.reset_noise (0.12)`
...
- constrains: `cell_mechanics.p0 (3.9), .K_P, .Gamma, .Lambda; cell_divide.p0 (default 3.72)`
...
## from: CODE (read in full): /workspace/Plexus/discovery_okuda/ops/mesh_ops.py; /workspace/Plexus/discovery_okuda/ops/ty
...
- constrains: `cell_mechanics.K_R (and the m["R0"] it is measured against). Hard-wired to 0.02 in translate._emit_shape_energy; it is absent from OPERATORS["cell_mechanics"]["params"], so it is not theta, not in comp_hash, not perturbable by sample_params, and not removable by any legal one-edit.`
...
- if violated: "Every vertex is on a spring to |x| = R0 about the WORLD ORIGIN, and cell_grow updates A0/P0/V0f but never R0 -- so in the okuda_route composition (local growth, no cell_grow) R0 stays frozen at the seed radius for the whole run. At r = 3*R0 that spring pulls each vertex inward with force 2*K_R*(r-R0) = 0.40, versus 0.20 for the Lambda line tension and ~0.26 for a 10% cell-volume error at K_V = 6. Is there a vitelline membrane in this experiment? If not, declare K_R = 0 and re-run every claim about whether growth alone can tubulate. If yes, expose K_R as theta so the confinement can be ablated like `extrude` is -- and note that `=cell_mechanics:monolayer` currently removes K_R and Lambda silently along with the volume definition, so that 'one edit' is three."
...
- constrains: `cell_divide's trigger `vf >= factor*djit*Vbirth`, its `Vbirth`/`vcap`/`v_ref`, and cell_grow's `vth_frac` -- all computed from face_geometry_3d's wedge volume v_f = (1/3)(cen_f . N_f), which is proportional to the distance from the world origin (v ~ r*A/3).`
...
- if violated: "A cell riding a bud outward from r = 5 to r = 10 doubles its wedge volume with no change of shape, so it crosses the 2 x V_birth threshold having grown nothing -- a tube would proliferate at its tip at growth rate zero, and K_V would then 'defend' the inflated volume. Is division meant to be driven by radial position? If not, use a translation-invariant volume (A_mid * h, as cell_mechanics:monolayer already computes) for both Vbirth and the trigger. If the wedge is kept, run the rate = 0 control and report how many divisions it produces before any tube claim is recorded."
...
- scope: closed epithelial vesicle / 3D vertex (apical-surface) model / any run with cell_divide present
...
- constrains: `mesh_seed's V0f / V0 / v_ref and cell_mechanics.K_V in the default implementation -- the per-cell wedge volumes v_f are pyramids from the shell centre, so sum_f v_f is EXACTLY the enclosed cavity, i.e. the 'cell volumes' tile the lumen.`
...
- constrains: `cell_grow's locked scaling A0 = A0_init*s^2, P0 = P0_init*s, V0f = V0f_init*s^3; cell_divide's `iso` rule (a0d = A0/V0f^(2/3) * half^(2/3)) under g1_ramp; and monolayer_ops's h_cell = full(h0), a single global constant ('v1: uniform fixed thickness').`
...
- scope: growing epithelial vesicle with reaction-diffusion patterning / morphogenesis timescales (hours-days) / any composition containing both cell_chem_react and a growth operator
...
- constrains: `cell_grow.cap (default 2.5 on the LINEAR scale = 15.6x in volume; not emitted by translate and not present in the search vocabulary at all, so it is invisible), cell_grow.vth_frac (translate emits 1.5; Okuda's is 4/3), and cell_divide.vcap / max_div / max_div_frac.`
...
- constrains: `cell_chem_diffuse.d_a / d_h / chi (both implementations measure length in CELL HOPS, so the diffusion length comes out directly in cells) together with cell_chem_react.mu_h / mu_a.`
...
- constrains: `the entire vertex mechanics (cell_mechanics, rd_interface_tension, cell_divide's local relax): there is no contact, collision or steric term anywhere in discovery_okuda/ops, and it also constrains every shape metric computed on the result (run_one.frame_metrics `protr` = r95/rmed, r95, and the tube_len/tube_diam family).`
...
- constrains: `cell_divide's rebuilt half-edge table (E_srce/E_trgt/E_face), and every consumer of ShapeEnergy3D._twin_faces -- whose documented behaviour is 'fallback to self (no penalty) if no twin' -- including the K_bend dihedral term, cell_chem_diffuse:interface_weighted (`shared = (twin != ef)`) and rd_interface_tension's interface ring.`
- check: `After every topology-changing operator: V - E + F == 2, every undirected edge has exactly two incident faces, every ring has >= 3 distinct vertices. topology_ops._check_closed already computes exactly this; edge_flip calls it, cell_divide does not, and no campaign metric in discovery/ records it (grep for 'euler' over discovery/*.py returns nothing).`
...
- constrains: `cell_mechanics.relax_iters (box 10-90) with eta and cap_frac (hard-wired 0.08 / 0.12 by translate), against the growth increment applied per frame by cell_grow.rate and the division budget max_div / max_div_frac.`
...
- constrains: `cell_divide.cycle_cv (vocabulary box 0.05-0.5, default 0.40; clamped in _fresh_djit to [0.4, 1.8]) and mesh_seed.vseed_cv (default 0.15).`
...
- constrains: `the mesh produced by mesh_seed, cell_divide (divide_face_3d adds a vertex to each of the two neighbours whose shared edge is split) and edge_flip; and, downstream, cell_neighbours, whose neighbour graph is the RD's entire notion of space.`
...
- constrains: `the integrated cell.chem block (engine.py:624, chem += dt*d) fed by cell_chem_diffuse and cell_chem_react, and read by cell_grow's Hill term a**hill / (a_sw**hill + a**hill + 1e-12) and by cell_divide's orient_asw / rd_interface_tension's a_sw thresholds.`
...
- scope: cell-discrete reaction-diffusion on an epithelial cell set / any composition with cell_chem_react + cell_grow

_miner notes: WHAT I DELIBERATELY DID NOT ASSERT.\n\n1. Shape index / rigidity transition. I drafted a premise that a confluent epithelium sits near p* = P/sqrt(A) ~ 3.81 and that cell_mechanics.p0 (box 3.4-4.2, default 3.90) should be checked against the REALISED shape index rather than merely set. I dropped it. p* = 3.81 is a result for a FLAT 2D vertex model with no volume term; on a closed curved shell wit_
...
- constrains: `cell_mechanics K_bend — hard-coded to 0.0 in discovery/translate.py:114 for the `default` implementation (the `monolayer` implementation instead generates bending emergently from h0)`
- check: `K_bend > 0, OR cell_mechanics.implementation == 'monolayer' with h0 > 0. And: the measured buckle/bud wavelength must be many cell diameters, not 1–2 (a mesh-scale wavelength is the signature of zero bending rigidity).`
...
- constrains: `cell_mechanics K_A (area stiffness, emitted as 1.0) and K_P, against cell_grow.rate and cell_grow.rate; and the run's mean-cell-area trajectory`
...
- constrains: `edge_flip l_th (0.01–0.12, default 0.04) and the run's T1-event count per frame`
...
- constrains: `cell_divide max_div (4–480, default 30 per frame) and max_div_frac (default 0.0075 per frame) — the per-frame division budget; and cell_divide vcap (0.0–3.0, default 1.5), where 0 disables the hard size cap`
...
- constrains: `the operator vocabulary itself — cell_divide exists but no apoptosis / cell-removal operator does (`extrude` is a radial push force, K_extrude, not a removal)`
...
- constrains: `cell_grow rate / rho / a_sw (the growth gate reads the activator only) and cell_divide (no pressure or density input at all)`
...
- constrains: `cell_mechanics p0 — searchable box currently (3.4, 4.2), default 3.90; the engine sets P0 = p0 * sqrt(A0) (mesh_ops.py:202, 581), i.e. the 2D convention`
...
- constrains: `voronoi_tension_3d s0 (default 5.4, prototype/Turing_vertex/vertex3d_ops.py:367) versus cell_mechanics p0 (default 3.90) — two different fields, two different conventions`
...
- constrains: `mesh_seed output topology (fibonacci_sphere, n_cells 150–2000) and every mesh-integrity metric read off that mesh`
...
- constrains: `cell_mechanics p0 jointly with mesh_seed n_cells (150–2000) — and specifically the planned 10x-reservoir re-run at ~18,000 cells`
...
- constrains: `cell_mechanics relax_iters (10–90, default 30) against cell_divide min_cycle (2–64 frames, default 16) and max_div_frac`
...
- constrains: `cell_mechanics Lambda (edge line tension, 0.0–0.3, default 0.20) and any reported holed / labyrinth / porous morphology`
...
- constrains: `the bud-count and bud-spacing readouts, together with cell_mechanics K_bend (currently 0.0, see premise 1)`
...
- constrains: `the tube aspect-ratio metric (the campaign's "aspect 7.5 -> 3.2") jointly with cell_mechanics p0 (fluid vs solid) and extrude K_extrude (0.0–14.0, default 4.0)`
...
- constrains: `cell_chem_react mu_h (inhibitor decay, 0.2–2.0) and the hard-coded mu_a = 1.0 in the Gierer-Meinhardt emit at discovery/translate.py:141, against cell_chem_diffuse d_a and d_h`
...
- constrains: `cell_chem_diffuse implementation — `graph_laplacian` couples every neighbour equally and `interface_weighted` by shared area; both are symmetric and carry no apical-basal axis`
...
- constrains: `mesh_seed (a single cell population, no fate field anywhere in the vocabulary) and any target morphology labelled a gland bud, duct, or branch`
