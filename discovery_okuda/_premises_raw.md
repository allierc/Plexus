...
- constrains: `grow_3d.rho (the baseline floor in growth = rate*(rho + hill(a))), and the presence/absence of a body-wide growth operator such as grow_3d in the schedule`
- check: `rho > 0, OR a body-wide growth operator (grow_3d/uniform_ramp) is present in `schedule`. Equivalently, measured: mean over cells of d(v_eq)/dt > 0 at every recorded frame, and total tissue volume V_tot(t_end) > V_tot(t_growth_start).`
...
- constrains: `the cell_react -> grow_3d `gate` connection in _discovery.structure.connections, jointly with grow_3d.rho and grow_3d.a_sw`
...
- constrains: `divide_3d.min_cycle and divide_3d.vcap jointly with grow_3d.rate, .rho and .vth_frac, and general.dt`
...
- constrains: `the recorded time series of total tissue volume V_tot and mean cell volume, against divide_3d.max_div_frac / .max_div / .min_cycle and grow_3d.rho`
...
- constrains: `grow_3d.conserve_amount (true = divide cell.chem by the volume growth ratio each tick) jointly with cell_react.rate, .mu_a, .mu_h and grow_3d.rate`
...
- constrains: `cell_react.a0 (Gierer-Meinhardt basal activator production, currently 0.01) jointly with grow_3d.conserve_amount and grow_3d.a_sw`
...
- constrains: `shape_energy_3d.p0 (default 3.90; vocabulary range 3.4-4.2) jointly with grow_3d.rho and with any protrusion/aspect metric used as a success criterion`
...
- constrains: `grow_3d (rate, rho, conserve_amount), which drives the A0/P0 targets; shape_energy_3d's per-face target V0f, which in tyssue_ops3d is the wedge volume from the shell centre to the face (a partition of the lumen), not the cell's own volume; h0 in the monolayer implementation is the only thickness variable.`
...
- constrains: `shape_energy_3d.relax_iters (searchable range (10, 90), default 30), eta, cap_frac and the global dt; against grow_3d.rate, divide_3d.max_div_frac / max_div, and reconnect_t1_3d.every (all every=1 in round_44_base).`
...
- constrains: `the pairing of grow_3d (rate, rho, cap, vth_frac, after_frame) with divide_3d (vcap, min_cycle, max_div_frac, after_frame). With rho = 0 the operator takes the `else` branch and clamps only the LINEAR scale s at cap (default 2.5), i.e. a volume scale of 15.6x; the per-cell v_eq cap at vth_frac*v_ref is gated behind `if self.rho > 0` and is therefore inactive in every ref_okuda_route / round_44 config.`
...
- constrains: `grow_3d.conserve_amount (must stay true; the operator rescales c <- c * (s_prev/s)^3 when v_eq grows as s^3)`
...
- constrains: `divide_3d.min_cycle (16 frames) and grow_3d.rate (0.01 per frame) versus the mechanical relaxation delivered by shape_energy_3d.relax_iters * eta per frame`
...
- constrains: `grow_3d.a_sw (1.5) relative to the fixed points of cell_react: gierer_meinhardt with gm_rho = 1, mu_a = 1, mu_h = 1, a0 = 0.01`
...
- constrains: `divide_3d's trigger `vf >= factor*djit*Vbirth`, its `Vbirth`/`vcap`/`v_ref`, and grow_3d's `vth_frac` -- all computed from face_geometry_3d's wedge volume v_f = (1/3)(cen_f . N_f), which is proportional to the distance from the world origin (v ~ r*A/3).`
...
- constrains: `grow_3d's locked scaling A0 = A0_init*s^2, P0 = P0_init*s, V0f = V0f_init*s^3; divide_3d's `iso` rule (a0d = A0/V0f^(2/3) * half^(2/3)) under g1_ramp; and tyssue_monolayer's h_cell = full(h0), a single global constant ('v1: uniform fixed thickness').`
...
- constrains: `grow_3d.cap (default 2.5 on the LINEAR scale = 15.6x in volume; not emitted by translate and not present in the search vocabulary at all, so it is invisible), grow_3d.vth_frac (translate emits 1.5; Okuda's is 4/3), and divide_3d.vcap / max_div / max_div_frac.`
...
- constrains: `shape_energy_3d.relax_iters (box 10-90) with eta and cap_frac (hard-wired 0.08 / 0.12 by translate), against the growth increment applied per frame by grow_3d.rate and the division budget max_div / max_div_frac.`
...
- constrains: `the integrated cell.chem block (engine.py:624, chem += dt*d) fed by cell_diffuse and cell_react, and read by grow_3d's Hill term a**hill / (a_sw**hill + a**hill + 1e-12) and by divide_3d's orient_asw / rd_interface_tension's a_sw thresholds.`
...
- scope: cell-discrete reaction-diffusion on an epithelial cell set / any composition with cell_react + grow_3d
...
- constrains: `shape_energy_3d K_A (area stiffness, emitted as 1.0) and K_P, against grow_3d.rate and grow_3d.rate; and the run's mean-cell-area trajectory`
...
- constrains: `grow_3d rate / rho / a_sw (the growth gate reads the activator only) and divide_3d (no pressure or density input at all)`
