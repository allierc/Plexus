...
`cell_chem_from_shape.beta` is inert, and they missed it the same way -- none of them measured the
operator's effect on the trajectory:
  ...
  * `instrument.install()` asks "did it act?" as `bool(out) or fingerprint moved`, and
    `cell_chem_from_shape` returns a full-size tensor every call, so `bool(out)` is True even when that
...
during __init__ and forward. `cell_chem_seed.amp` has sat in every spec this campaign has ever
written and is not read by any branch of the operator.

RUN:  python op_probe.py <spec.yaml> --op cell_chem_from_shape --param beta --values -2 -4
...
def run_spec(spec_dict, frames, device="cpu", tag=""):
    ...
    import mesh_ops, chem_ops, t1_ops, monolayer_ops  # noqa: F401
...
# frames before `cell_grow` even switches on, and it starts from a SPHERE -- where the
...
STRUCTURAL = ("cell_divide", "edge_flip", "cell_extrude_3d")
...
def warm_spec(spec_dict, ckpt_path, frames=50, freeze_topology=False):
    ...
    So a GATED `cell_chem_seed` is dropped (its job is done, it is in the checkpoint), while an
    ...
    #
    # A THOUSAND-FOLD difference in the perturbation moved the distance by 7%. `cell_divide` runs
    ...
    #
    # Sensitivity needs a fixed-dimensional state space. It also needs a tape, and `cell_divide` /
    # `edge_flip` are both DIFFERENTIABLE = False, so with them in the chain NO parameter
    ...
    for o in d.get("operators", []):
        if freeze_topology and o.get("op") in STRUCTURAL:
            continue
        if o.get("op") == "mesh_seed":
            ops.append({"op": "load_mesh_3d", "at": o.get("at", "vertex"),
                        "cell_set": o.get("cell_set", "cell"), "ckpt": ckpt_path,
                        "before_frame": 1})
            continue
        if o.get("op") == "cell_chem_seed" and any(k in o for k in ("before_frame", "every")):
    ...
    # rewriting only the first raises "schedule step 'mesh_seed' is not a declared operator" --
    ...
    if "schedule" in d:
        ...
        d["schedule"] = [s for s in
                         ["load_mesh_3d" if s == "mesh_seed" else s for s in d["schedule"]]
                         if s in kept]
    return d
...
def selftest(fixture_spec, ckpt, ops, frames=50, device="cpu"):
    ...
    each script?"* -- yes, and the ORDER is the point. The existing `cell_chem_from_shape` self-test does
