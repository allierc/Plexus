"""chem_ops -- live Turing reaction-diffusion ON the cell set (Goal 2). Forked from the
...
Operators:
  cell_neighbours (rewire)     -- build the cell-cell neighbour graph from the half-edge table
  cell_chem_seed   (structural) -- Gray-Scott initial condition (substrate=1, a central activator spot)
  cell_chem_diffuse   (lateral)    -- graph-Laplacian diffusion of chem on the cell adjacency (EMIT=velocity)
  cell_chem_react     (lateral)    -- Gray-Scott autocatalysis (EMIT=velocity)
  cell_geometry (aggregate)-- vertices -> per-cell centroid/area
  cell_grow (growth)-- grow each cell's target volume by a Hill fn of its activator -> BUDDING
"""
...
class CellDiffuseInterfaceWeighted(Lateral):
    ...
    def forward(self, H, mask=None):
        from mesh_ops import face_geometry_3d, ShapeEnergy3D
...
class Grow3D(Structural):
    ...
    def __init__(self, params, device="cpu"):
        ...
        from mesh_ops import _engine_owns_clock
