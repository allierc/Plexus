...
THE CHAIN THIS TESTS, one link at a time, with a number on each:
    ...
    2  a growing tissue therefore meets more material in that direction and the matrix pushes back
       harder there -- `ecm_from_cell` records the contact pressure by direction, so this is measured,
       not assumed
    3  `ecm_gate_growth` slows the CELL CYCLE where that pressure is high: a cell whose target
       volume grows more slowly reaches `cell_divide`'s doubling threshold later and divides less often
