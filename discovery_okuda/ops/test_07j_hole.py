...
class Rig07j(H.Rig07d, Rig05m):
    ...
    def _respot(self):
        """`spot_field`'s cap, PER CELL rather than per surface triangle.

        `spot_field` sizes `mt1` to `F_epi`, which on `Rig05m` is the epithelium's faces and IS the
        cell list. On 07c it is not: the surface is one wedge per half-edge, 1,188 of them under 200
        cells, while `_mt1_on_faces` and `_cell_to_face` index by `ct_face`, which is the CELL. A
        1,188-long field read at cell indices 0..199 does not raise -- it reads a sixth of the cap and
        calls it the source, which is the same silent permutation that made 05h1's fields random when
        the driver changed. The angle is therefore taken of each CELL's own centroid direction, and
        the inhibitor and proMMP-2 sources are made uniform at their means, as `spot_field` does.
        """
        theta, off = self._spot
        cen = self.x_epi[self._cen_idx()] - self.c
        u = cen / cen.norm(dim=1, keepdim=True).clamp_min(1e-30)
        ...
        dt_ = dt_ / dt_.norm()
        ang = torch.arccos((u @ dt_).clamp(-1.0, 1.0))
