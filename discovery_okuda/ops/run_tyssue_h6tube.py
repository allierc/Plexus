...
def make(p):
    ...
    ops = [{"op": "mesh_seed", "at": "vertex", "n_cells": n, "radius": R, "jitter": J, "p0": 3.90, "seed": SEED, "before_frame": 1, "vseed_cv": 0.15},
           ...
           {"op": "cell_grow", "at": "vertex", "cell_set": "cell", "rate": p["rate"], "a_sw": p["a_sw"], "hill": 4.0, "rho": p["rho"], "vth_frac": p["vth"]},
           # p0>3.81 FLUID (tubes need T1 flow) + modest surface tension for rounder cells; radial ~off so tubes leave the sphere
           {"op": "cell_mechanics", "at": "vertex", "p0": 3.90, "K_A": 1.0, "K_P": 1.0, "Gamma": p.get("Gamma", 0.2), "Lambda": p.get("Lambda", 0.6), "K_V": p.get("K_V", 4.0), "K_R": 0.02, "mu": 1.0, "dt": 0.02, "relax_iters": 26, "eta": 0.08, "cap_frac": 0.12,
            "K_bend": p.get("K_bend", 0.0), "K_lumen": p.get("K_lumen", 0.0), "antiinv": p.get("antiinv", 0.0)},   # balloon-hollow stabilisers (one per variant)
           {"op": "edge_flip", "at": "vertex", "l_th_frac": 0.28, "every": 1, "max_flips": max(60, n // 8)},
           {"op": "cell_divide", "at": "vertex", "factor": 2.0, "reset_noise": 0.12, "cycle_cv": p.get("cyc_cv", 0.15), "p0": 3.90, "every": 2, "max_div": 60, "max_div_frac": 0.02, "cell_set": "cell", "min_cycle": 4, "max_cycle": p.get("max_cyc", 30)},
           {"op": "topo_record", "at": "vertex", "every": 1}]
    sched = ["mesh_seed", "cell_geometry", "cell_neighbours", "cell_chem_seed", "cell_chem_diffuse", "cell_chem_react",
             "cell_grow", "cell_mechanics", "edge_flip", "cell_divide", "topo_record"]
