...
`cell_mechanics`'s line-tension term `Lambda * sum_e l_e` -- one number for the whole tissue, fixed in
...
THE HARD PART IS NOT THE DYNAMICS, IT IS TOPOLOGY. `edge_flip` (kind `rewire`) rewires the
half-edge relation; `cell_divide` (kind `structural`) adds cells and edges; cell death removes them. In
...
nothing and takes `myo_new`, and an edge that is gone simply stops being asked. `edge_flip`,
`cell_divide` and any future death operator need NO edits and no knowledge that myosin exists.
...
That is what `junction_sync` (below) exists to undo: myosin is state of the junction edge-set, so
...
exists -- reached without teaching `cell_divide` or `edge_flip` that myosin exists.

WHAT WENT WRONG WITHOUT IT, MEASURED. `junction_myosin` writes `m["myo"]` sized to the half-edge buffer
at its own schedule slot (before `cell_mechanics`); `edge_flip` and `cell_divide` then rewire and
lengthen those arrays later in the same frame, and `topo_record` records the new edges beside the
