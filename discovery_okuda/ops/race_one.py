...
kb = float(sys.argv[3]) if len(sys.argv) > 3 else 2.0e5
...
cfg.update(membrane=npz, membrane_particles=45000, membrane_cutoff=0.008, membrane_break=0.35,
           membrane_bond_k=kb, membrane_adhesion=4.0e4, membrane_tau=60.0, membrane_jitter=0.35,
...
out = dict(rate=rate, k_bond=kb, n_alive_end=int(al.sum()),
...
import ecm_ops
_vm = np.asarray(ecm_ops.STRESS_RAW, np.float32)
out["ecm_p99"] = float(np.percentile(_vm[-1], 99)) if len(_vm) else None
out["unstable"] = bool(out["ecm_p99"] and out["ecm_p99"] > 100.0)
