...
## Energy — new op `monolayer_energy_3d` (or a mode of `cell_mechanics`)
```
U = Σ_j [ ½ k_v (v_j − v_eq_j)²  +  κ_s · s_j ]        # Okuda Eq. 3, verbatim
force = −∂U/∂x_i                                        # autograd, same relax loop as cell_mechanics
```
Emit velocity, bounded-Euler, `relax_iters` — identical plumbing to the current `cell_mechanics`.
...
`cell_grow` already raises `v_eq_j` via `Hill(activator)`. With the monolayer volume it now inflates
...
| `cell_mechanics` volume | swap wedge-volume → prism `v_j`; add `κ_s s_j`; targets optional |
| new state | one cell channel `h_j` |
| `cell_grow` | none (already grows `v_eq`) |
