#!/usr/bin/env python
"""Archive: HOMOGENISED vesicle (vh_K4_cv15_d4 recipe) + Brusselator RD, NO activator->growth coupling
(a_sw=50 -> drive=rho=1 uniform). Shows the Turing coral pattern riding a uniform growing/dividing shell
-- the sanity that RD is pure colouring on the homogenised recipe, before we couple it. Renders top row
coloured by ACTIVATOR (white->red Turing pattern), bottom row cross-section; reports area_cv AND per-cell
volume_cv (the cleaner uniformity metric with future tubes)."""
...
R, J, SEED, FR = 5.0, 0.18, 0, 400
NAME = "vh_K4_cv15_d4_rd"
...
def make():
    ...
    ops = [{"op": "mesh_seed", "at": "vertex", "n_cells": 150, "radius": R, "jitter": J, "p0": 3.72, "seed": SEED, "before_frame": 1, "vseed_cv": 0.15},
           {"op": "cell_geometry", "at": "cell"}, {"op": "cell_neighbours", "at": "cell"},
           {"op": "cell_chem_seed", "at": "cell", "seed": SEED, "before_frame": 3, "mode": "noise", "A": 1.0, "B": 3.0, "noise": 0.04},
           {"op": "cell_chem_diffuse", "at": "cell", "d_a": 0.05, "d_h": 0.7, "chi": 4.0},
           {"op": "cell_chem_react", "at": "cell", "model": "brusselator", "gamma": 2.0, "A": 1.0, "B": 3.0},
           {"op": "cell_grow", "at": "vertex", "cell_set": "cell", "rate": 0.03, "a_sw": 50.0, "hill": 4.0, "rho": 1.0, "vth_frac": 1.4},
           {"op": "cell_mechanics", "at": "vertex", "p0": 3.72, "K_A": 1.0, "K_P": 1.0, "Gamma": 0.1, "Lambda": 0.5, "K_V": 4.0, "K_R": 0.4, "mu": 1.0, "dt": 0.02, "relax_iters": 26, "eta": 0.08, "cap_frac": 0.12},
           {"op": "edge_flip", "at": "vertex", "l_th_frac": 0.35, "every": 2, "max_flips": 30},
           {"op": "cell_divide", "at": "vertex", "factor": 2.0, "reset_noise": 0.12, "cycle_cv": 0.15, "p0": 3.72, "every": 2, "max_div": 12, "max_div_frac": 0.03, "cell_set": "cell", "min_cycle": 4, "max_cycle": 12},
           {"op": "topo_record", "at": "vertex", "every": 1}]
    sched = ["mesh_seed", "cell_geometry", "cell_neighbours", "cell_chem_seed", "cell_chem_diffuse", "cell_chem_react", "cell_grow", "cell_mechanics", "edge_flip", "cell_divide", "topo_record"]
    cfg = {"general": {"name": f"tyssue_{NAME}", "seed": SEED, "n_frames": FR, "dt": 0.02, "record_cap": FR + 2, "boundary": "free", "dim": 3, "world": [10 * R] * 3},
...
try:
    ...
    rec.update(cells_end=int(mtT["nF"]), n_div=int(emesh.get("n_div", 0)), area_cv=round(float(arT.std() / arT.mean()), 3),
               ...
               activator_range=[round(float(aT.min()), 2), round(float(aT.max()), 2)], patterned=bool(aT.std() > 0.1))
