...
Everything else is ready for it: `ecm_from_cell` already carries the reaction force, and the
...
def build(name, tissue_npz, fit=FIT, plate_box=None, ecm_direct_push=True, **ecm):
    ...
    object: `plate_confine` holds the matrix out of them during the run and `ecm_seed` never seeds
    ...
    for o in spec["operators"]:
        ...
        if o["op"] in ("integrin_adhesion", "surface_track", "mpm_boundary",
                       "bm_contact", "adhesion_seed", "adhesion_pull",
        ...
        if o["op"] == "bm_seed":
            ...
            o["scale"] = s
            o["surface"] = tissue_npz
    # NON-PENETRATION, ALWAYS. The penalty in `ecm_from_cell` punishes penetration rather than
    ...
    # surface: the epithelium pushed the stroma DIRECTLY, twice (`ecm_from_cell` as a soft penalty and
    ...
    # two direct paths are both redundant and harmful: `cell_exclude` repositions particles without
    ...
    if ecm_direct_push:
        spec["operators"].append({"op": "cell_exclude", "at": "mpm_particle",
                                  "centre": [0.5, 0.5, 0.5], "surface": tissue_npz, "scale": s,
                                  "skin": 0.004})
        spec["schedule"].append("cell_exclude")
    # THE MEMBRANE GETS THE SAME BACKSTOP THE MATRIX HAS ALWAYS HAD, and the asymmetry it corrects is
    # why the sheet sank. The epithelium repels interstitial fibres twice -- `ecm_from_cell` as a soft
    # penalty and `cell_exclude` as a hard projection -- while NOTHING pushed on the basement
    ...
    # `mpm_boundary` and the strain did not move by one digit (3.8e-4, identical to 88) because
    ...
    if "basement_membrane_particle" in spec["sets"] and membrane_exclude:
        spec["operators"].append({"op": "cell_exclude", "at": "basement_membrane_particle",
                                  "centre": [0.5, 0.5, 0.5], "surface": tissue_npz, "scale": s,
                                  "skin": 0.006})
        spec["schedule"].append("cell_exclude")
    if gap_box is not None and "mpm_block" not in spec["sets"]:
        ...
        # by `plate_confine` AND an elastic block filling the same region. The projection wins every
        ...
        spec["operators"].append({"op": "plate_confine", "at": "mpm_particle", "axis": 2,
                                  "centre": 0.5, "gap_half": gap_box, "stiff": 1.0})
        spec["schedule"].append("plate_confine")
