...
constraint (mesh_ops.py): mesh_seed -> cell_mechanics. A jittered sphere relaxes to a
...
import mesh_ops        # noqa: F401  mesh_seed + cell_mechanics
import t1_ops     # noqa: F401  edge_flip (surface T1 annealer)
from mesh_ops import build_sphere_mesh, face_polygons_3d
...
def make_spec(name, p0, buf, grow_rate, divide, n_cells, frames):
    ops = [
        {"op": "mesh_seed", "at": "vertex", "n_cells": n_cells, "radius": RADIUS,
         "jitter": JITTER, "p0": p0, "seed": SEED, "before_frame": 1},
    ]
    sched = ["mesh_seed"]
    if grow_rate > 0:
        ops.append({"op": "cell_grow", "at": "vertex", "rate": grow_rate, "every": 1, "rho": 1.0, "a_sw": 0.0, "vth_frac": 1e9, "conserve_amount": False})
        sched.append("cell_grow")           # ramp the per-cell TARGETS before the force step reads them
    # Expansion EMERGES from the per-cell volume elasticity: growth only ramps each cell's target volume,
    # and cell_mechanics's force balance inflates every cell locally (no vertex moved by hand). During
    ...
    ops.append({"op": "cell_mechanics", "at": "vertex", "p0": p0, "K_A": 1.0, "K_P": 1.0,
    ...
    if divide:
        ...
        sched.append("edge_flip")          # T1 neighbour exchange -> anneal division defects (regularise)
        mxd = max(10, n_cells // 20)              # divisions/call scale with tissue size (clears each wave)
        ops.append({"op": "cell_divide", "at": "vertex", "factor": 2.0, "reset_noise": 0.12, "p0": p0,
                    "every": 2, "max_div": mxd})  # gradual, staggered volume-doubling cell cycle (Turing fig4)
        sched.append("cell_divide")                # split cells whose wedge volume doubled (edge-midpoint septum)
        sstride = max(1, ((frames or FRAMES) + 300) // 300)   # record/snapshot stride -> bound memory on long runs
        ops.append({"op": "topo_record", "at": "vertex", "every": sstride})
        sched.append("topo_record")         # record the (changing) mesh at the recording stride for rendering
    cfg = {
...
def _draw(ax, pos, mesh, p0, azim, act=None, inner=INNER, Lbox=None,
    ...
    (diag_tools.mesh_faults) and passes them to both viewpoints rather than recomputing.
    """
    ...
    for name, p0, grow_rate, divide, n_cells, frames in presets():
        ...
        try:
            ...
            if hist:                                    # verify the closed surface survived every division (Euler=2)
                from topology_ops import rings_from_flat_3d, _check_closed
