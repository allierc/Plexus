...
```
cells / vertex
  [x] mesh_seed              build the epithelial shell
  [x] cell_geometry          per-cell area / perimeter / volume from the mesh
  [x] cell_mechanics           the 3D AVM: area, perimeter, volume, line tension, radial
  [x] cell_grow       per-cell target growth
  [x] cell_divide                 division on volume doubling                      (structural)
  [x] edge_flip           T1 rearrangement                                (rewire)
  [x] topo_record          record the mesh for analysis and rendering
  [x] ecm_gate_growth        matrix stress suppresses the cell cycle
  [~] ecm_load               matrix pressure pushes the surface inward
...
basement_membrane
  [x] bm_seed    lay the sheet on the epithelium's basal surface
  [x] bm_bond    crosslinks: the collagen IV network
  [x] bm_unbond   fragmentation                              (rewire)
  [x] bm_secrete lay down NEW membrane as the surface grows      (structural)
...
interstitial_ecm
  ...
  [x] ecm_from_cell               the epithelium's contact force on the matrix
  [x] cell_exclude           non-penetration                                 (structural)
...
The epithelium repels interstitial fibres TWICE -- `ecm_from_cell` as a soft penalty and `cell_exclude`
as a hard projection -- and did nothing whatever to the basement membrane. Its only outward force was
the integrin spring. Registering `cell_exclude` at `basement_membrane_particle` too, at the SAME
...
`cell_exclude` now skips massless particles, so `bm_secrete`'s parked reserve is not
...
element's radius by smooth interpolation of the pass-1 map. `bm_seed` and
...
**wedges from the sphere centre** (`mesh_ops.face_geometry_3d`), and the monolayer prism in the
...
* **with it at k = 2e4, critically damped**, nothing broke at all -- but the sheet SANK INTO the
  ...
  end-state strain is ~0.15 in EVERY run regardless of stiffness, because `bm_remodel`
