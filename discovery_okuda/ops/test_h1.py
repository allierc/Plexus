#!/usr/bin/env python
"""H1: vesicle cell non-uniformity comes from growing by INFLATION (cell_grow ramps V0f -> big
cells) rather than PROLIFERATION. Test: short A/B, same seed/length, measure cell-area CV.
  A inflation    = cell_grow (current vesicle_grow_divide)
...
def build(mode):
    ...
    common_seed = {"op": "mesh_seed", "at": "vertex", "n_cells": 150, "radius": R, "jitter": J,
                   "p0": 3.72, "seed": SEED, "before_frame": 1, "vseed_cv": 0.4}
    if mode == "inflation":
        grow = {"op": "cell_grow", "at": "vertex", "rate": 0.003, "every": 1, "rho": 1.0, "a_sw": 0.0, "vth_frac": 15.625, "conserve_amount": False}
        gname = "cell_grow"; pre = ["mesh_seed", "cell_grow"]; extra = [common_seed, grow]
    else:  # proliferation (Okuda uniform): rho=1 + v_eq cap; needs cell set + geometry
        extra = [common_seed, {"op": "cell_geometry", "at": "cell"},
                 {"op": "cell_grow", "at": "vertex", "cell_set": "cell", "rate": 0.03,
                  "a_sw": 0.5, "hill": 4.0, "rho": 1.0, "vth_frac": 1.4}]
        pre = ["mesh_seed", "cell_geometry", "cell_grow"]
    ops = extra + [
        {"op": "cell_mechanics", "at": "vertex", "p0": 3.72, "K_A": 1.0, "K_P": 1.0, "Gamma": 0.1,
        ...
        {"op": "cell_divide", "at": "vertex", "factor": 2.0, "reset_noise": 0.12, "cycle_cv": 0.4, "p0": 3.72,
         "every": 2, "max_div": 12, "max_div_frac": 0.03, "cell_set": "cell"},
        {"op": "topo_record", "at": "vertex", "every": 1}]
    sched = pre + ["cell_mechanics", "edge_flip", "cell_divide", "topo_record"]
