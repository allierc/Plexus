...
LAST one -- a real matrix resists by deforming, and `ecm_load` is that version. Read against these
...
the spec tells cells to stop dividing when squeezed. `cell_divide` fires on volume doubling and
`cell_mechanics` charges a cell for being compressed, so a confined cell reaches its division volume
...
# ceiling arrived at depth 0.22, most of the tissue's radius. `cell_exclude` is the hard backstop
