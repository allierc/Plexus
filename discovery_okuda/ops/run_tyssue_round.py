...
import mesh_ops, chem_ops, t1_ops, monolayer_ops, ckpt  # noqa (registers the monolayer op)
...
# shapes but doesn't BUILD the wall -- new cells must be placed NORMAL to the body. New: cell_divide orient_iface
...
def make(p):
    ...
    sched = ["load_mesh_3d", "cell_geometry"]
    if cones:                                                   # N BIG spots at frame 0 (cones); tip-tracking
        ...
        ops += [{"op": "cell_chem_seed", "at": "cell", "mode": smode, "n_spots": p["spots"], "cone_deg": p["cone_deg"], "seed_dir": p.get("seed_dir", None), "tip_radius": p.get("tip_radius", 2.0), **seed}]
        sched += ["cell_chem_seed"]
        if rd:
            impl = p.get("rd_impl", "brusselator")
            if impl == "gray_scott":                              # stable localized SPOTS (substrate depletion)
                react = {"op": "cell_chem_react", "at": "cell", "model": "gray_scott", "F": p.get("F", 0.055), "kk": p.get("kk", 0.062), "rate": p.get("rd_rate", 1.0)}
            elif impl == "gierer_meinhardt":                      # SELF-ENHANCING a^2/h peak (Okuda's RD, ref 37)
                react = {"op": "cell_chem_react", "at": "cell", "model": "gierer_meinhardt", "gm_rho": p.get("gm_rho", 1.0), "mu_a": p.get("mu_a", 1.0), "mu_h": p.get("mu_h", 1.0), "a0": p.get("a0", 0.01), "rate": p.get("rd_rate", 1.0)}
            else:                                                 # Brusselator (Turing spots); B = activator contrast
                react = {"op": "cell_chem_react", "at": "cell", "model": "brusselator", "gamma": p.get("gamma", 0.3), "A": 1.0, "B": p.get("B", 3.0)}
            ops += [{"op": "cell_neighbours", "at": "cell"},
                    {"op": "cell_chem_diffuse", "at": "cell", "d_a": p.get("d_a", 0.05), "d_h": p.get("d_h", 0.7), "chi": p.get("chi", 4.0)},
                    react]
            sched += ["cell_neighbours", "cell_chem_diffuse", "cell_chem_react"]
    else:                                                       # Brusselator RD (develops from noise -> spots emerge late)
        ops += [{"op": "cell_neighbours", "at": "cell"},
                {"op": "cell_chem_seed", "at": "cell", "seed": 0, "before_frame": 3, "mode": "noise", "A": 1.0, "B": 3.0, "noise": 0.04},
                {"op": "cell_chem_diffuse", "at": "cell", "d_a": 0.05, "d_h": 0.7, "chi": p["chi"]},
                {"op": "cell_chem_react", "at": "cell", "model": "brusselator", "gamma": p["gamma"], "A": 1.0, "B": 3.0}]
        sched += ["cell_neighbours", "cell_chem_seed", "cell_chem_diffuse", "cell_chem_react"]
    ...
    if p.get("monolayer"):                                     # MONOLAYER (apical/basal) energy: growth-driven,
        se = {"op": "cell_mechanics", "model": "monolayer", "at": "vertex",   # quasi-static tube (Okuda)
        ...
        se = {"op": "cell_mechanics", "at": "vertex", "p0": 3.90, "K_A": 1.0, "K_P": 1.0, "Gamma": 0.05, "Lambda": 0.2, "K_V": p.get("K_V", 4.0), "K_R": 0.02, "K_bend": p.get("K_bend", 0.0), "antiinv": p.get("antiinv", 0.0), "mu": 1.0, "dt": dt, "relax_iters": p.get("relax", 30), "eta": 0.08, "cap_frac": 0.12}
    ops += [{"op": "cell_grow", "at": "vertex", "cell_set": "cell", "rate": p["rate"], "a_sw": p["a_sw"], "hill": p.get("hill", 4.0), "rho": p["rho"], "vth_frac": p["vth"], "after_frame": ga, "dt": dt, "conserve_amount": p.get("conserve_amount", True)}, se]
    sched += ["cell_grow", "cell_mechanics"]
    if p.get("K_purse", 0.0) > 0 or p.get("K_extrude", 0.0) > 0:   # RD-INTERFACE tube mechanism (purse-string + red extrusion)
        ...
        ops += [{"op": "interface_tension", "at": "vertex", "cell_set": "cell", "K_purse": p.get("K_purse", 0.0), "a_sw": p.get("iface_asw", p["a_sw"]), "eta": p.get("iface_eta", 0.05), "iters": 4, "after_frame": ga}]
        sched += ["interface_tension"]
        if float(p.get("K_extrude", 0.0)) > 0:
            ops += [{"op": "interface_push", "at": "vertex", "cell_set": "cell", "K_extrude": p.get("K_extrude", 0.0), "a_sw": p.get("iface_asw", p["a_sw"]), "eta": p.get("iface_eta", 0.05), "iters": 4, "after_frame": ga}]
            sched += ["interface_push"]
    ops += [{"op": "edge_flip", "at": "vertex", "l_th_frac": p.get("l_th_frac", 0.28), "every": p.get("t1_every", 1), "max_flips": p.get("max_flips", 300)},
            {"op": "cell_divide", "at": "vertex", "factor": 2.0, "reset_noise": 0.12, "cycle_cv": p.get("cycle_cv", 0.4), "p0": 3.90, "every": 2, "max_div": 120, "max_div_frac": p.get("mdf", 0.03), "vcap": p.get("vcap", 0.0), "cell_set": "cell", "min_cycle": p.get("min_cycle", 4), "max_cycle": p.get("max_cycle", 1000000000), "after_frame": ga, "orient_iface": p.get("orient_iface", False), "orient_asw": p.get("orient_asw", p.get("a_sw", 1.0)), "g1_ramp": p.get("g1_ramp", False)},
            {"op": "topo_record", "at": "vertex", "every": 1}]
    sched += ["edge_flip", "cell_divide", "topo_record"]
