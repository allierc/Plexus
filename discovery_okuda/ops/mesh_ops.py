"""mesh_ops -- the 3D (surface) vertex model: an epithelial VESICLE. A closed half-edge mesh on
...
Operators:
  mesh_seed    (structural) -- build a closed spherical half-edge mesh; stash it + A0/P0/V0
  cell_mechanics (lateral)    -- 3D AVM shape energy + lumen volume; force by autodiff; bounded Euler
"""
...
class Divide3D(Structural):
    ...
    def forward(self, H, mask=None):
        from topology_ops import rings_from_flat_3d, flat_from_rings_3d, divide_face_3d
...
class Apoptosis3D(Structural):
    ...
    def forward(self, H, mask=None):
        ...
        from topology_ops import (rings_from_flat_3d, flat_from_rings_3d,
                                           face_collapse_3d)
        self._k += 1
