...
WHAT IT REPLACES. `ecm_from_cell[replay]` pushes along the radius with a force from a SMOOTHED,
32x64 radius map, and `cell_exclude` then projects out whatever the penalty failed to keep out.
...
def build(name, frames, tissue_npz, scale, n_particles=200000, n_fibres=10000, fibre_len=0.12,
    ...
    return {
        ...
        "operators": [
            ...
            {"op": "ecm_seed", "at": "mpm_particle", "centre": list(CENTRE),
            ...
            {"op": "mesh_inside", "at": "mpm_particle", "n_grid": int(n_grid)},
        ...
        # cached, and `mesh_inside` has to test this frame's positions against THIS frame's
...
def measure(P, vm, per, scale, r_tissue, out_dir, dx):
    ...
    # AND IT IS READ AGAINST ITS OWN FRAME 0, NOT AGAINST 1/3. `ecm_seed` pushes any particle that
...
def main():
    ...
    spec_note = dict(
        ...
        replaces=["ecm_from_cell[replay] -- a radial force from a smoothed radius map, with no "
                  "reaction and no shear",
                  "cell_exclude -- a projection backstop, which makes non-penetration "
                  "unmeasurable"],
