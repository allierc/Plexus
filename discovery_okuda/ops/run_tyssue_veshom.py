...
(cell_grow ramps V0f -> big non-uniform cells); here the shell grows by PROLIFERATION (Okuda) --
cell_grow with rho=1 baseline (all cells, no activator gradient) and v_eq CAPPED at vth*v_ref,
so every cell cycles in [~2/3,vth]*v_ref (uniform) and the sphere grows because there are MORE cells.
cell_divide volume-primary + bounded cell-cycle DURATION (min/max_cycle). Metric = area_cv (cell-size
...
from diag_tools import hollow_flags, hollow_metric
...
def make_spec(name, frames, rate, K_V, cv, vth, min_cyc, max_cyc, buf, cbuf):
    ops = [{"op": "mesh_seed", "at": "vertex", "n_cells": 150, "radius": RADIUS, "jitter": JITTER,
           ...
           {"op": "cell_grow", "at": "vertex", "cell_set": "cell", "rate": rate,
           ...
           {"op": "cell_divide", "at": "vertex", "factor": 2.0, "reset_noise": 0.12, "cycle_cv": cv, "p0": 3.72,
            "every": 2, "max_div": 12, "max_div_frac": 0.03, "cell_set": "cell", "min_cycle": min_cyc, "max_cycle": max_cyc},
           {"op": "topo_record", "at": "vertex", "every": 1}]
    sched = ["mesh_seed", "cell_geometry", "cell_grow", "cell_mechanics",
