...
WHAT WAS MISSING. `ecm_from_cell` computes the force the growing epithelium puts on the matrix. That
...
touched by it. The plates (`plate_confine`) get the ovoid by fiat: they are rigid, so the shape is
...
AN OVERDAMPED DISPLACEMENT, WHICH IS WHAT THE SOLVER ITSELF DOES. `cell_mechanics` owns the vertex
...
it IS the force under the overdamped integration `cell_mechanics` already uses (its own `mu` and `dt`),
