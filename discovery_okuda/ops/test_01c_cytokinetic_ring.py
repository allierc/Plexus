...
# `junction_sync` -- which is model-agnostic by design -- renders a not-yet-stored newborn at
...
def classify(S):
    ...
    split half    -- one endpoint is a vertex `cell_divide` appended: half of a contact it cut
