...
def _matrix_strands(ax, q, band, cmap, zorder, alpha, per, three_d=True, lw=0.55, max_lines=2500,
    ...
    `ecm_seed` lays the particles along segments -- particle `i` belongs to strand `i // per` -- and
...
def _matrix_half(ax, q, band, sel, cmap, zorder, alpha, per=None, gap=None, s_rest=1.1, s_hot=2.4):
    ...
    exactly as `ecm_seed` laid it down -- so it is only ever correct on the full, seeding-order array.
...
def _myo_of(mt, n_half, myo_new=1.0):
    ...
    `junction_myosin` writes `m["myo"]` before `cell_divide` and `edge_flip` run, and division
...
def draw_junctions_3d(ax, mt, pos, cam, L, myo_hi=None, cutaway=True, lw=0.9):
    ...
    # single z-order, and the array order is the half-edge table -- which `cell_divide` and
    # `edge_flip` PERMUTE every frame. So which of 12,000 overlapping lines ends up on top changes
