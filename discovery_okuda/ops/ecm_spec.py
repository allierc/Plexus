...
here lives in the engine; importing `ecm_ops` registers the three new operators.
    ...
    schedule   aggregate -> ecm_seed -> ecm_from_cell -> ecm_stress
...
def build_spec(name, n_frames=320, dt=0.004, substep_dt=2.0e-4, n_grid=64,
               ...
               # ending up inside the lumen, and `cell_exclude` is the hard backstop; this is the soft
               ...
               # to transmit a sustained pressure, with `cell_exclude` guaranteeing the constraint.
    ...
    spec = {
        ...
        "operators": [
            ...
            {"op": "ecm_from_cell", "at": "mpm_particle", "centre": [0.5, 0.5, 0.5],
        ...
        "schedule": [
            ...
            "ecm_from_cell",                   # the ball's push, as an external acceleration
            "ecm_stress",                    # recolour by |J-1| BEFORE the frame is recorded
            {"substep_dt": float(substep_dt),
             "steps": (["mpm_strain", "mpm_scatter", "mpm_grid_update", "mpm_boundary",
    ...
    if membrane is not None:
        ...
        spec["sets"]["basement_membrane_particle"] = {
            ...
            "types": {f"m{i}": {"fraction": 1.0 / len(STRESS_COLORS),
                      ...
                      for i in range(len(STRESS_COLORS))}}
        spec["operators"] += [
            {"op": "bm_seed", "at": "basement_membrane_particle",
            ...
            {"op": "bm_repel", "at": "basement_membrane_particle",
            ...
            {"op": "surface_track", "at": "surface", "centre": [0.5, 0.5, 0.5],
             ...
             "seed": int(seed), "jitter": float(membrane_jitter)},
            {"op": "bm_secrete", "at": "basement_membrane_particle",
            ...
            {"op": "bm_crosslink", "at": "basement_membrane_particle",
             "every": int(membrane_rebond_every), "cutoff": float(membrane_cutoff),
             "max_neighbours": 6},
            {"op": "bm_unbond", "at": "basement_membrane_particle",
        ...
        spec["schedule"].insert(i, "bm_seed")
        if not membrane_springs:
            ...
            if n_adhesions > 0:
                ...
                i2 = spec["schedule"].index("bm_seed") + 1
            ...
            if membrane_contact_k > 0.0:
                spec["operators"].append(
                    {"op": "bm_contact", "at": "basement_membrane_particle",
                     "centre": [0.5, 0.5, 0.5], "surface": str(membrane), "scale": 1.0,
                ...
                for _st in spec["schedule"]:
                    if isinstance(_st, dict) and "substep_dt" in _st:
                        _st["steps"].insert(0, "bm_contact")
                        break
            if membrane_grid_bc:
                spec["operators"].append(
                    {"op": "mpm_boundary", "at": "mpm_grid", "centre": [0.5, 0.5, 0.5],
            ...
            spec["schedule"].insert(j, "bm_strain")
        if membrane_springs:
            ...
            spec["schedule"].insert(i + 1, "bm_bond")
            if membrane_tau and membrane_tau > 0:
                spec["schedule"].insert(i + 2, "bm_remodel")
                spec["schedule"].insert(i + 3, "bm_crosslink")
                spec["schedule"].insert(i + 4, "bm_unbond")
            else:
                spec["operators"] = [o for o in spec["operators"]
                                     if o["op"] != "bm_remodel"]
                spec["schedule"].insert(i + 2, "bm_unbond")
            if membrane_repel_w > 0.0 or membrane_repel_k > 0.0:
                ...
                after = ("bm_remodel" if "bm_remodel" in spec["schedule"]
                         else "bm_bond")
                spec["schedule"].insert(spec["schedule"].index(after) + 1, "bm_repel")
            else:
                spec["operators"] = [o for o in spec["operators"] if o["op"] != "bm_repel"]
        ...
        if membrane_direct_forces:
            for o in spec["operators"]:
                if o["op"] in ("integrin_adhesion", "bm_contact"):
            ...
            for _st in spec["schedule"]:
                if isinstance(_st, dict) and "substep_dt" in _st:
                    _st["steps"] = [x for x in _st["steps"] if x != "bm_contact"]
            ...
            if integrin_bond_k > 0.0:
                ...
                for _st in spec["schedule"]:
                    if isinstance(_st, dict) and "substep_dt" in _st:
                        ...
                        break
            spec["schedule"].insert(spec["schedule"].index("bm_seed") + 1,
                                    "integrin_seed")
            for _st in spec["schedule"]:
                if isinstance(_st, dict) and "substep_dt" in _st:
        ...
        if membrane_impl == "graph":
                ...
                for o in spec["operators"]:
                    if o["op"] in ("bm_bond", "integrin_adhesion",
                                   "bm_repel"):
                        o["emit"] = emit
        ...
        if membrane_surface_level:
            ...
            for o in spec["operators"]:
                if o["op"] in ("integrin_adhesion", "bm_seed"):
        ...
        if membrane_reserve and membrane_reserve > 0:
            # LAST IN THE FRAME. `bm_bond` must have run at least once so
            ...
            spec["schedule"].append("bm_secrete")
        else:
            spec["operators"] = [o for o in spec["operators"]
                                 if o["op"] != "bm_secrete"]
