...
def measure(out, spec, stress=None, seeded=None):
    ...
    T = pos.shape[0]
    op = next(o for o in spec["operators"] if o["op"] == "ecm_from_cell")
    ...
    for t in range(T):
        if S is not None:
            ...
            # moment `cell_exclude` started preventing penetration: a run with a working
...
def render(name, out, spec, out_dir, n_strip=8, movie_frames=None, movie=True, fps=None,
    ...
    # WHICHEVER OPERATOR CARRIES THE TISSUE. `ecm_from_cell[replay]` names the pass-1 cache in
    ...
    op = next((o for o in spec["operators"] if o["op"] in ("ecm_from_cell", "mesh_contact")), None)
    centre = np.asarray(op["centre"], float)
    surf = (op or {}).get("surface") or (op or {}).get("tissue")

    if op is None or surf is None or (op["op"] == "ecm_from_cell"
    ...
    # THE MATRIX AS STRANDS, WHEN IT WAS SEEDED AS STRANDS. `ecm_seed` puts `per_parent // n_fibres`
    ...
    # `ecm_from_cell[replay]` runs pass 2 for the tissue's own 402 frames and reads the surface map at
    ...
    if keep_traj:
        try:
            ...
            if "basement_membrane_particle" in out.get("sets", {}):
                ...
                # THE HOOP MAP, in the shape the growth gate reads. `ecm_gate_growth` takes a
