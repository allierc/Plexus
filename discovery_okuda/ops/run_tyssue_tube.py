...
A localized activator patch (top pole, cell_chem_seed mode=patch) drives cell_grow (grow the
target volume where a is high) and, because those cells inflate past their doubling volume, cell_divide
(which propagates the activator to daughters, so the growing region stays activated and proliferates).
The shell is FLUID (p0 > p0* ~ 3.81) and edge_flip fires every tick so the tissue can flow /
...
import mesh_ops        # noqa: F401
import chem_ops       # noqa: F401  cell_geometry + cell_chem_seed + cell_grow
import t1_ops     # noqa: F401  edge_flip
...
from diag_tools import hollow_flags     # geometric hollow-cell diagnostic (folded/tiny caps -> grey walls)
...
def make_spec(name, n_cells, frames, p0, grow_rate, a_sw, patch_z, l_th_frac, cv, buf, cbuf, g1=False):
    ...
    ops = [
        ...
        {"op": "cell_geometry", "at": "cell"},              # cen (needed by the patch seed) each frame
        {"op": "cell_chem_seed", "at": "cell", "mode": "patch", "patch_z": patch_z},  # RE-SEED EVERY FRAME:
        ...
        {"op": "cell_grow", "at": "vertex", "cell_set": "cell", "rate": grow_rate,
         "a_sw": a_sw, "hill": 4.0, "cap": 4.0},               # grow v_eq where activator high (the mitogen)
        {"op": "cell_mechanics", "at": "vertex", "p0": p0, "K_A": 1.0, "K_P": 1.0, "Gamma": 0.05,
        ...
        {"op": "edge_flip", "at": "vertex", "l_th_frac": l_th_frac, "every": 1,
        ...
        {"op": "topo_record", "at": "vertex", "every": 1},   # record EVERY frame -> posf/hist aligned
    ]
