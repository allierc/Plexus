...
epithelial FACE, which is right for its icosphere and wrong here: `cell_divide` re-indexes faces, so a
...
def main():
    ...
    if n_plq > n_max:
        print(f"[06] {n_plq} plaques asked for; the tissue has {x_ep0.shape[0]} vertices at the "
              ...
              f"turnover operator grows the set as `cell_divide` makes more.", flush=True)
