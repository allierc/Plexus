...
SERIES = {
    ...
    # tau = 0 removes `bm_remodel` entirely: rest lengths are frozen, so stretch
    ...
    # the edges. `bm_crosslink` (rewire) is the missing half of fragmentation.
    ...
    #     18,134 particles per frame are PROJECTED out of the tissue by cell_exclude, a positional
    ...
    #
    #     The membrane receives no force at all: ecm_from_cell is an analytic growing sphere bound to
    ...
    #     grid BC was only half the fix: `cell_exclude` still ran on the membrane and is applied
    ...
    #     `bm_contact` becomes redundant rather than badly tuned.
    ...
    #     epithelium still pushes the stroma DIRECTLY -- `ecm_from_cell` as a soft penalty and
    #     `cell_exclude` as a hard projection -- and both predate a membrane that sits on the surface.
