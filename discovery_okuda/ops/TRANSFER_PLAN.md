...
| `cell_chem_diffuse`, `cell_chem_react` (RD) | `diffuse` / `react` (exist in main registry already) | **do NOT re-promote** | the core already has `diffuse`/`react`; these are forks. Reconcile, don't duplicate. |
| `seed_mesh`, `seed_cell`, `cell_neighbours`, `topo_snapshot` | seed / rewire / tooling | **stay prototype** | `seed_mesh` bootstraps via a Voronoi (prototype convenience); `topo_snapshot` is a render helper. |
