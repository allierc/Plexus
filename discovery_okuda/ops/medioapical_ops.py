...
length, and length is EXTENSIVE: when `cell_divide` inserts a vertex into an existing edge (a,b), the
...
is inherited by copying the parent face's value onto both daughters (`cell_divide`'s `keep` map). For a
