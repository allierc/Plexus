...
spots; cell_grow then grows each cell's target volume where the activator is high, so every
spot sprouts a tube, and cell_divide (propagating chem to daughters) + edge_flip let the tissue
...
import mesh_ops, t1_ops, chem_ops  # noqa
...
def make_spec(name, n_cells, frames, n_spots, cone_deg, grow, cv, rho, vth, K_V, min_cyc, max_cyc, buf, cbuf):
    ...
    ops = [{"op": "mesh_seed", "at": "vertex", "n_cells": n_cells, "radius": RADIUS, "jitter": JITTER,
           ...
           {"op": "cell_geometry", "at": "cell"},                    # per-cell centroid (for the cones)
           {"op": "cell_chem_seed", "at": "cell", "mode": "cones", "n_spots": n_spots, "cone_deg": cone_deg},  # re-seed each frame -> tracks the N growing tips
           ...
           {"op": "edge_flip", "at": "vertex", "l_th_frac": 0.28, "every": 1, "max_flips": max(40, n_cells // 8)},
           ...
           {"op": "topo_record", "at": "vertex", "every": 1}]     # ALIGNED recording
...
def run_all(only=None):
    for name, n_cells, frames, n_spots, cone_deg, grow, cv, rho, vth, K_V, min_cyc, max_cyc in presets():
        ...
        try:
            ...
            from topology_ops import rings_from_flat_3d as _rff
