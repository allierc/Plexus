...
Audited 2026-08-05 by re-reading every `spec_run.yaml` (`discovery_okuda/ops` git log has the reasoning).
...
| cell↔matrix contact | `k=900, a_max=200`, penetration punished not prevented | `k=1200, a_max=300` + `cell_exclude` non-penetration | 01–41 have no exclusion; 42+ do |
...
where to divide (`ecm_gate_growth`). An elastic solid **presses** it to 1.41, indistinguishable.
...
* it cannot currently be tested either way, for three reasons:
  ...
  3. `cell_exclude` makes the boundary impermeable BY CONSTRUCTION, so the fix forbids the
...
* and the epithelium has no basal side to attach a membrane to: cells are wedges from the sphere
  centre (`mesh_ops.face_geometry_3d`), and the monolayer prism is drawn by `_draw`, not
