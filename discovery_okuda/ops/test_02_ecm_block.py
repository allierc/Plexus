...
WHAT IS BEING TESTED, in Plexus2 terms: `ecm_seed` (a Seed operator laying fibres of a given length,
...
import ecm_ops                                        # noqa: F401  registers ecm_seed / ecm_stress
...
def build(name, frames, n_particles=90000, n_grid=64, dt=3.2e-3, sub=2.0e-4, youngs=120.0,
    ...
    cube of homogeneous points, and this is a cube of FIBRES, seeded by `ecm_seed` into the same box.
    ...
    return {
        ...
        "schedule": ["aggregate", "ecm_seed", "gravity", "ecm_stress",
...
def _fibre_lines(P_t, band_t, per, max_fibres=700):
    ...
    `ecm_seed` lays each strand as `per` consecutive particles along one direction, so particle i
...
def main():
    ...
    if "--render-only" in sys.argv:
        ...
        per = max(1, int(sp["sets"]["mpm_particle"]["per_parent"])
                  // int(next(o for o in sp["operators"] if o["op"] == "ecm_seed")["n_fibres"]))
