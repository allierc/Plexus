...
def write(R, spec):
    ...
    w("> **Not comparable to the NeuralGraph campaign's 74%.** That figure was annotated "
      "retrospectively over a different unit of analysis; this one is a prospective commitment "
      "scored automatically. A discovery system that confirms three quarters of its predictions "
      "is probably not posing risky ones. The contrast that IS apples to apples is internal: "
      "below the seed floor against above it.\n")
    ...
    w("\n## Intent and outcome\n")
    w("Reported as INTENT-OUTCOME INVERSION, not as miscalibrated confidence: "
      "`confirmatory`/`adversarial`/`exploratory` are declared experimental intents, not "
      "probabilities, and calling them confidence invites an argument about terminology instead "
      "of about the result.\n")
