...
\multicolumn{4}{@{}l}{\textit{before \op{06}: the sheet on a REAL epithelium} (\op{05j}--\op{05m})}\\
G30& \textbf{$\lambda^{\rm geo}$ does not depend on the adhesion's stiffness} & $\kappa_n$ over
     $8\times$ moves it $<5\%$ & \PASS{$0.0000$} ($4.455$ at every $\kappa_n$)\\
G31& it tracks a real, dividing surface & $\lambda^{\rm geo}$ within $5\%$ of
     $\sqrt{A_{\rm ep}(T)/A_{\rm ep}(0)}$, NOT of the radius ratio & \PEND{05l}\\
G32& the mesh survives it & worst triangle quality $>0.2$ & \FAIL{$0.000$} (\op{05j}, no
     secretion, no refinement)\\
G33& $\rho$ held with \op{bm\_secrete} ON, on the real surface & within $10\%$ & \PEND{05l}\\
G34& mean edge in band with \op{bm\_refine} ON, on the real surface & $[0.8,1.7]\times$ seeded &
     \PEND{05l}\\
G35& all four reported together every frame & $\lambda^{\rm geo}$, $\lambda^{\rm el}$, $\rho$, edge
     & \PEND{05m}\\
