...
\multicolumn{4}{@{}l}{\textit{before \op{06}: the sheet on a REAL epithelium --- numbered from G40 to leave G30--G39 to the adhesion work} (\op{05n}, \op{05p}, \op{05q})}\\
G40& \textbf{$\lambda^{\rm geo}$ does not depend on the adhesion's stiffness} & $\kappa_n$ over
     $8\times$ moves it $<5\%$ & \PASS{$0.0000$} ($4.455$ at every $\kappa_n$)\\
G41& it tracks a real, dividing surface & $\lambda^{\rm geo}$ within $5\%$ of
     $\sqrt{A_{\rm ep}(T)/A_{\rm ep}(0)}$, NOT of the radius ratio & \PEND{05q}\\
G42& the mesh survives it & worst triangle quality $>0.2$ & \FAIL{$0.000$} (\op{05n}, no secretion, no refinement)\\
G43& $\rho$ held with \op{bm\_secrete} ON, on the real surface & within $10\%$ & \PEND{05q}\\
G44& mean edge in band with \op{bm\_refine} ON, on the real surface & $[0.8,1.7]\times$ seeded &
     \PEND{05q}\\
G45& all four reported together every frame & $\lambda^{\rm geo}$, $\lambda^{\rm el}$, $\rho$, edge
     & \PEND{05q}\\
