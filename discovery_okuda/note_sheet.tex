...
\subsection*{3.4\; Proteolysis, and the mechanism the literature forced}
...
\subsection*{3.5\; TIMP-2 is the adaptor as well as the inhibitor, and that predicts a bell}
...
\subsection*{3.6\; What this composition buys, and it is the argument for Plexus2}

It is worth stopping to count what produced the breach, because the list is short. \textbf{One set}
(\code{bm\_face}, carrying mass and a rest metric), \textbf{three fields} on it (proMMP-2, MMP-2,
TIMP-2) with \textbf{one state} beside them (TIMP-3, which does not diffuse), \textbf{two operators}
(a semi-implicit diffusion--reaction step, Eqs.~\eqref{eq:pro}--\eqref{eq:act}, and
\op{bm\_degrade}, Eq.~\eqref{eq:degrade}), and \textbf{one threshold} $\rho_{\rm crit}$. That is the
whole composition. What comes out of it is a \emph{localized, self-arresting perforation of a basement
membrane}: a single hole whose area tracks the enzyme's own footprint over an order of magnitude
($0.8\%$, $1.1\%$, $2.8\%$, $8.4\%$ of the sheet for sources of $15^\circ$, $20^\circ$, $30^\circ$ and
$45^\circ$), which stops growing on its own ($+0.06\%$ of the sheet over the last third of a
$401$-frame run), and which cannot be made arbitrarily small because below $\sqrt{D/k}$ the source can
no longer hold a concentration.

\textbf{Nothing in that list mentions a hole.} There is no perforation operator, no rim, no crack-tip
law, no seeded defect and no rule that says where the sheet may fail --- the rim is simply the set of
faces whose areal density fell through $\rho_{\rm crit}$, and the arrest is the enzyme running out of
substrate against clearance and inhibition. Two of the phenotype's properties are outright
\emph{predictions} of the composition rather than inputs to it: the bell (TIMP at zero and at $10K$
both breach nothing, because the same molecule presents the zymogen and poisons the enzyme) and the
lower bound on hole size (a diffusion length, which is why G42's failure --- $\sqrt{D/k}=0.05$ against
$h=0.076$ --- is a statement about what this model can resolve and not a tuning matter).

\textbf{What is an input, said plainly.} Where the enzyme sits. In the \op{06\_hole\_*} ladder MT1-MMP
is placed as a cap of chosen angular radius, which is a statement about the epithelium and not about
the membrane; in \op{06\_breach\_hole} and \op{06\_hole\_one} it is a smooth random field and even the
location is emergent. Neither run paints a hole; both make an enzyme and let it work.



\textbf{The lever is \emph{where} MT1-MMP is, not how fast it cuts.} MT1-MMP is the membrane-anchored
initiator --- the ternary reaction can only start where its cell has put it --- so replacing a
many-peaked random field by a single cap changes no rate at all: it changes where proteolysis is
\emph{permitted to nucleate}, and the chemistry then amplifies that spatial instruction into a hole of
matching size. That separates the model's two levers cleanly: \textbf{source localization sets the
upper geometric scale of a perforation and reaction--diffusion sets the lower one}, since below
$\sqrt{D/k}$ the same cap, at the same peak concentration, breaches nothing. It is a sharper statement
than ``more protease, bigger hole'', and it is the one that matches the biology, because MT1-MMP's
surface distribution is actively regulated --- shifted basally in polarized epithelia, concentrated at
invadopodia and leading edges --- rather than uniform \cite{sato2010}. It also names the next
experiment rather than leaving it as a slogan: if a localized patch can hold a \emph{stable}
perforation, then the chain \textbf{signalling $\to$ protease localization $\to$ BM remodelling $\to$
altered mechanics $\to$ morphogenesis} becomes testable inside one composition, by asking whether the
hole subsequently selects where the epithelium buds, invades, or changes curvature.
...
G29& no plaque ends up on the wrong side & $0\%$ & \PASS{$0.0\%$} (was $89.6\%$)\\\midrule
\multicolumn{4}{@{}l}{\textit{before \op{06}: the sheet on a REAL epithelium} (\op{06c\_real\_driver},
     \op{05k\_gates}) --- all six re-measured, see the note below the table}\\
G40& $\lambda^{\rm geo}$ does not depend on the adhesion's stiffness & $\kappa_n$ over
     $8\times$ moves it $<5\%$ & \PASS{$4.72\%$} --- but \emph{monotone}, $3.641\to3.817$\\
G41& \textbf{it tracks a real, dividing surface} & $\lambda^{\rm geo}$ within $5\%$ of
     $\sqrt{A_{\rm ep}(T)/A_{\rm ep}(0)}$, NOT of the radius ratio & \PASS{$1.40\%$}
     ($3.713$ vs $3.766$)\\
G42& the mesh survives it & worst triangle quality $>0.2$ & \PASS{$0.587$}\\
G43& $\rho$ held with \op{bm\_secrete} ON, on the real surface & within $10\%$ &
     \PEND{} \op{bm\_secrete} is rig-level in \op{05f}, not a method on \op{Sheet}\\
G44& mean edge in band, no \op{bm\_refine} & $[0.8,1.7]\times$ seeded &
     \FAIL{$3.63\times$} --- the size of the demand for refinement\\
G45& all four reported together every frame & one frame axis, $401$ frames & \PASS{$401$}\\
G46& the sheet stays OUTSIDE the real surface (G28 re-run) & signed gap $>0$ &
     \FAIL{$-2.7\times10^{-3}$} on mean radii --- see the caveat\\
...
G40& receptor conserved: $N_f+\sum N_b$ moves only by $s_i$, $1/\tau_i$ & exact &
     \PEND{05d}\\
G41& bound fraction depends on LOAD, not on the frame & monotone in stress, plateaus &
     \PEND{05d}\\
G42& slip rate monotone in $k_{\rm off}$, zero at $k_{\rm off}=0$ & separation &
     \PEND{05d}\\
G43& the discrete-plaque model converges to the density model at high $\Sigma^{-2}$ & within $5\%$ &
     \PEND{05d}\\
\bottomrule
\end{tabularx}}
\smallskip
...
\smallskip
\noindent\textbf{G40--G46 were certified twice, and the first certification is void.} They were first
measured against a driver in which each plaque bound one tissue \emph{vertex}, carried its own rest
length, and retargeted on turnover --- a scheme built for this note and for nothing else. It failed:
worst triangle quality $0.000$, $\lambda^{\rm geo}$ at $9.46$, the sheet visibly collapsed to a third
of the epithelium's radius. The diagnosis written from that failure --- that a real, dividing,
bumpy surface with a vertex count going $396\to12{,}756$ is a different problem from a dilating
icosphere --- was \emph{wrong}. \op{06c\_real\_driver} holds every value of \op{05b}'s certified rig
and changes one thing, the driver, and the sheet tracks the real epithelium on the first attempt:
$\lambda^{\rm geo}=3.713$ against an apical-area ratio of $3.766$, momentum $1.2\times10^{-16}$,
$100\%$ of bonds held, $401$ frames, no divergence. \textbf{05b's own law --- bind to a \emph{face}
with barycentric weights, one shared rest length --- was never the problem.} The three additions were.
A gate measured against a discarded driver certifies nothing, so all six are re-run in \op{05k\_gates}
and the numbers in the table are those.

\smallskip
\noindent Two of them are worth reading past their verdict. \textbf{G40 passes at $4.72\%$ against a
$5\%$ threshold, but $\lambda^{\rm geo}$ is monotone in $\kappa_n$} ($3.641$, $3.713$, $3.769$,
$3.817$ at $\kappa_n=2.5,5,10,20$): that is not scatter under a bound, it is the bond's compliance
leaking into a number that is supposed to report the tissue's growth. The gate is passed and the
mechanism it was written to exclude is present at $4.7\%$; a tighter threshold, or a stiffer adhesion,
would turn it. \textbf{G46 reads mean radii, not the signed per-node distance to the triangle} --- on
a bumpy surface the mean vertex radius sits outside the mean surface, so the measure is biased toward
reporting the sheet as inside. The $-2.7\times10^{-3}$ is real enough to keep the gate open, but it
is not yet the test G28 was: that one needs \op{05b}'s per-node signed standoff evaluated on the
tissue's faces, which is the first thing \op{05k\_G46\_standoff} should carry.


\begin{figure}[H]\centering
\includegraphics[width=\textwidth]{../log/okuda_ECM/05k_gates/gates.png}
\caption{\op{05k\_gates}: G40--G46 on the real epithelium, \op{05b}'s rig with the driver swapped and
nothing else changed. \textbf{a} \PASS{G41}: $\lambda^{\rm geo}$ (black) on the apical-area ratio
$\sqrt{A_{\rm ep}(t)/A_{\rm ep}(0)}$ (green) to $1.40\%$ over $401$ frames --- the sheet is reporting
the tissue's growth, and against the \emph{area}, since a dividing epithelium is not a sphere and the
radius ratio would have been the wrong reference. \textbf{b} \PASS{G40} at $4.72\%$ over $8\times$ in
$\kappa_n$, and the four points are monotone: passed, but the adhesion's compliance is in there.
\textbf{c} \PASS{G42}: worst triangle quality $0.587$, far above the $0.2$ line, against $0.000$ for
the vertex-plaque driver this replaces. \textbf{d} \FAIL{G44}: with no \op{bm\_refine} the mean edge
reaches $3.63\times$ its seeded length, outside the band (green) --- the failure is expected and its
value is the measurement, i.e. how much refinement a $3.77\times$ dilation of a real surface demands.}
\end{figure}
...
\begin{figure}[H]\centering
\IfFileExists{../log/okuda_ECM/_archive_05def/05e_conserve/remesh_model.png}{\includegraphics[width=\textwidth]{../log/okuda_ECM/_archive_05def/05e_conserve/remesh_model.png}}{\fbox{\parbox{0.6\textwidth}{\footnotesize\textbf{figure missing:} \texttt{../log/okuda\_ECM/\_archive\_05def/05e\_conserve/remesh\_model.png}. Its run folder was deleted from \texttt{log/okuda\_ECM}; the figure is not regenerated here because the trajectory it was drawn from is gone too.}}}
...
\begin{figure}[H]\centering
\IfFileExists{../log/okuda_ECM/_archive_05def/05f_secrete/secrete_model.png}{\includegraphics[width=\textwidth]{../log/okuda_ECM/_archive_05def/05f_secrete/secrete_model.png}}{\fbox{\parbox{0.6\textwidth}{\footnotesize\textbf{figure missing:} \texttt{../log/okuda\_ECM/\_archive\_05def/05f\_secrete/secrete\_model.png}. Its run folder was deleted from \texttt{log/okuda\_ECM}; the figure is not regenerated here because the trajectory it was drawn from is gone too.}}}
...
\begin{figure}[H]\centering
\IfFileExists{../log/okuda_ECM/_archive_05def/05d_tear/metrics.png}{\includegraphics[width=\textwidth]{../log/okuda_ECM/_archive_05def/05d_tear/metrics.png}}{\fbox{\parbox{0.6\textwidth}{\footnotesize\textbf{figure missing:} \texttt{../log/okuda\_ECM/\_archive\_05def/05d\_tear/metrics.png}. Its run folder was deleted from \texttt{log/okuda\_ECM}; the figure is not regenerated here because the trajectory it was drawn from is gone too.}}}
...
\subsection*{4.4\; 07: the adhesion must follow the CELL, and the gates that say so}

\textbf{What 06 measured, and why it forces a redesign rather than a threshold.} The plaques in every
06 run are drawn as the links they are, and \op{07\_plaque\_length} puts numbers on them. On
\op{06\_refine}, the best-behaved run in the ladder: the mean plaque is $11.9\,\mu$m and the longest
$63.5\,\mu$m, against $l_0=0.70\,\mu$m and against the $40$~nm at which an integrin cluster actually
sits below the actin it pulls on \cite{kanchanawong2010}. That is $90\times$ its own rest length and
$1{,}600\times$ the protein. $97.6\%$ of the set is past $3l_0$. The adhesion at frame $400$ is not
an adhesion.

\textbf{And the count grows for the wrong reason.} \op{plaque\_seed} makes one edge per live
\emph{sheet node}, so the set follows the MEMBRANE'S MESH: on \op{06\_refine} it changes at $2$ of
$200$ intervals --- frames $122$ and $340$ --- the second by $+300\%$ in a single frame, while the
tissue divides smoothly from $200$ to $6{,}380$ cells. The measurable consequence is that
\textbf{plaques per cell falls from $12.81$ to $6.42$}: the adhesion density per cell halves over a
run in which nothing about adhesion was changed. An integrin cluster is a structure of the CELL, so
the set should be seeded on the cell, inherited at division, and refined only because the tissue
divided --- not because the sheet's mesh did.

\smallskip
{\footnotesize
\begin{tabularx}{\textwidth}{@{}p{0.9cm}Lp{3.5cm}p{2.6cm}@{}}\toprule
& \textbf{gate} & \textbf{threshold, decided before the run} & \textbf{06\_refine today}\\\midrule
G70 & \textbf{the plaque count follows the tissue, not the mesh}: plaques per cell holds its
      seeded value & within $10\%$ for the whole run & \FAIL{$12.81\to6.42$, $-50\%$}\\
G71 & \textbf{and it grows smoothly}, because cells divide one at a time: no kept-frame interval
      changes the count by more than $5\%$ (at $6{,}380$ cells over $401$ frames a frame divides
      about $0.25\%$ of them) & $\max\Delta n/n<5\%$ & \FAIL{$+300\%$ in one interval; $2$ steps in
      $200$}\\
G72 & \textbf{the length distribution is stationary}: a growing tissue must not stretch its
      adhesions, it must make more of them & median in $[0.5,2]\,l_0$ and p99 $<4\,l_0$ at
      \emph{every} frame & \FAIL{median $17\,l_0$, $97.6\%$ over $3l_0$}\\
G73 & \textbf{a plaque born in a division starts at rest}. Without this G72 can be passed on paper
      while every inherited plaque is still $60\,\mu$m long: division would copy the parent's
      stretch instead of resetting it & length $<1.2\,l_0$ on the frame it first appears &
      \PEND{no such operator exists}\\
G74 & \textbf{the integrin content per plaque is stationary}: growth goes into the NUMBER of
      clusters, not into the occupancy of each. Median $N_b$ per plaque against its seeded value &
      within $[0.5,2]\times$ over the run & \PEND{$N_b$ is not recorded --- see below}\\
G75 & \textbf{and division splits the bonds rather than creating them}: $N_f+\sum N_b$ moves only by
      $s_i$ and $1/\tau_i$, re-run \emph{through} a division & exact, as G30 & \PEND{same}\\
\bottomrule\end{tabularx}}
\smallskip

\textbf{G74 and G75 cannot be run today, and the reason is worth stating rather than working around.}
The clutch's bond number $N_b$ and the free pool $N_f$ live on the rig and are never written to
\code{bm\_frames.npz}, so no diagnosis of this store can say whether the integrins turn over --- only
that the number of PLAQUES does not. Recording them is two lines in each rig's keep-tuple and is the
first thing 07 should commit, before any rupture law is designed: otherwise the law gets tuned
against a quantity nobody measured.

\textbf{What this predicts, which is what makes it a gate and not a preference.} If the set is owned
by the cell and inherited at division, plaque spacing stays tied to CELL size rather than to mesh
size, and a new adhesion is seeded at the current geometry --- unstretched. The length distribution
then has a mechanism to be stationary (G72) instead of being clipped by a threshold, which matters
because \op{plaque\_rupture} was deliberately deleted: rupture is $k_{\rm off}(f)$, a rate, and
re-introducing a length cut-off would undo that decision. \op{07\_plaque\_length} also measures the
second reason a cut-off is wrong --- the two ends SLIDE, $0.31\,\mu$m per kept frame on
\op{06\_refine} --- so a threshold would break a population that is not overstretched but merely
displaced, and each break moves load onto its neighbours.


\subsection*{Exit gates: what must be true before 06}

The sheet is not ready to be joined to the matrix and the vertex model because of what it has passed;
it is ready when the following are closed. These are stated \emph{before} the runs that test them, and
each names the consequence of failing rather than a threshold to be relaxed afterwards. Three of them
have never been run at all, which is a stronger reason to hold than any of the marginal results above.

\smallskip
{\footnotesize
\begin{tabularx}{\textwidth}{@{}p{0.8cm}Lp{3.0cm}p{2.4cm}@{}}\toprule
& \textbf{gate} & \textbf{threshold} & \textbf{status}\\\midrule
\multicolumn{4}{@{}l}{\textit{A --- never run. The scheme rests on these and none has been tested.}}\\
G59 & \textbf{the density adhesion reproduces the discrete one} where the discrete one is valid:
      raise the plaque density until there is more than one cluster per node and the two models must
      agree & standoff and traction within $5\%$ & \PEND{05j}\\
G60 & the field integral $\sum_f c_fA_f$ is invariant across a $1\!\to\!4$ split & exact &
      \PEND{05k}\\
G61 & protease on a torn face is \emph{accounted}, not silently dropped & the balance closes &
      \PEND{05k}\\
G62 & the steady field profile is mesh-independent & $<5\%$ across one refinement & \PEND{05k}\\
\midrule
\multicolumn{4}{@{}l}{\textit{B --- measured and failing. Close or waive explicitly.}}\\
G42 & the field's own length is resolved: $\sqrt{D/k}\ge 2h$ & hard & \FAIL{$0.05$ vs $h=0.076$}\\
G55 & the bell peaks at $c_T=K$ & within $2\times$ & \FAIL{$2.03$} --- marginal\\
G46 & the breach is \emph{larger} than its source, and the excess tracks $\sqrt{D/k}$ &
      radius ratio $>1$ & \PEND{partly: 05h\_3 breaches under a UNIFORM source, so it is not a
      stencil; the radius has never been measured}\\
G63 & the sheet sits \emph{across} the bell, not on its shoulder & band fraction $>25\%$ &
      \FAIL{$0.7$--$2.6\%$ in every 05h run}\\
\midrule
\multicolumn{4}{@{}l}{\textit{C --- required by 06 itself, and never attempted.}}\\
G64 & every operator runs \emph{simultaneously} for the full $401$ frames: elasticity, remodelling,
      contact, adhesion with turnover, secretion, refinement, proteolysis & no divergence, no
      dissolution & \PEND{05m}\\
G65 & momentum still closes with \emph{all} couplings summed (adhesion $+$ contact $+$ fibril) &
      $<10\varepsilon$ & \PEND{05m}\\
G66 & the cost is affordable: wall clock per frame at the final mesh with the matrix present &
      reported, with the dominant term named & \PEND{05m}\\
\bottomrule
\end{tabularx}}
\smallskip

\textbf{G59 carries a stop condition} and it is the one to watch. If a discrete-plaque model at high
density does not reproduce the bond-density model, then the density is not a coarse-graining of the
discrete picture --- it is a different model wearing its name, and the whole of \S2.3 would have to be
withdrawn rather than patched. Every other gate here can be waived with a stated reason; that one
cannot.
