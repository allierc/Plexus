...
\multicolumn{4}{@{}l}{\textit{before \op{06}: the sheet on a REAL epithelium} (\op{06c\_real\_driver},
     \op{05q\_gates}) --- all six re-measured, see the note below the table}\\
...
A gate measured against a discarded driver certifies nothing, so all six are re-run in \op{05q\_gates}
...
tissue's faces, which is the first thing \op{05r\_standoff} should carry.


\begin{figure}[H]\centering
\includegraphics[width=\textwidth]{../log/okuda_ECM/05q_gates/gates.png}
\caption{\op{05q\_gates}: G40--G46 on the real epithelium, \op{05b}'s rig with the driver swapped and
