#!/bin/bash
# Submit one rung's specs to ${CLUSTER_QUEUE_PREFIX}l4, all in parallel.
#     bash jobs/size_cycle_submit.sh <rung> [spec ...]      (run on $CLUSTER_SSH, from the repo root)
...
# hosts to keep jobs off: PG_EXCLUDE_HOSTS, comma-separated -- a local setting, not in the repo
excl=""; for h in ${PG_EXCLUDE_HOSTS//,/ }; do excl="$excl -R hname!=$h"; done
for s in $specs; do
  bsub -n 8 -gpu "num=1" $excl -q ${CLUSTER_QUEUE_PREFIX}l4 -W 240 -J ${rung}_$s \
