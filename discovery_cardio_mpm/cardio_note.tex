...
\begin{phasebox}{Phase 1 --- Freeze the recording, seal the test \hfill \prog\ (4 of 7)}
...
\textbf{4. Does the answer depend on the grid?} \ns\ Finer resolution, more particles, smaller
sub-steps. The real motion is smaller than one grid cell, so this is not a formality. The only
Phase-1 item that needs a GPU.

\textbf{5. Freeze the split, seal the diseased sheet.} \ns\ By content, not by filename --- the
same diseased recording sits on disk under three different names. A run that touches it aborts and
every read is logged. \emph{Irreversible, so it goes last.}

\textbf{Exit gate.} All five above, plus Phase~1b green: the split file exists with disjoint frame
sets and the sealed checksums; the ceiling and the tracker number are in this note (they are); the
resolution check is reported against the noise floor; and a test proves a run reading the sealed
specimen aborts.
\end{phasebox}
