...
\begin{phasebox}{Phase 1 --- Freeze the recording, seal the test \hfill \prog\ (6 of 7)}
...
\textbf{6. And the premises --- what the loop is allowed to call a tissue.} \fl\ Written from the
...
\textbf{Exit gate.} All of the above, plus Phase~1b green: the split file exists with disjoint frame
sets and the sealed checksums; the ceiling, the tracker number and the resolution table are in this
note; a test proves a run reading the sealed specimen aborts; and the premise checks run, with
whatever they refuse recorded rather than waived.
\end{phasebox}

\begin{phasebox}{Phase 1b --- The corpus we already own \hfill \prog}
...
\code{reproduce.py}. The migration is written and \textbf{all eight stale recipes now load}; the
regeneration is running. The four phase-delayed runs go first, because they are the only
simulations in the corpus that carry \emph{coordination} structure, and coordination is precisely
the axis the inherited objective cannot see --- so they are the test set for the measure Phase~2
has to invent.

This is worth the compute rather than just fixing the names, because it asks a question nobody has
asked: \textbf{was that operator merge actually behaviour-preserving?} Two operators were folded
into one and the library moved on. If the regenerated trajectory matches the archive, the corpus is
restored \emph{and} the merge is validated; if it does not, we have found a silent change in the
forward model, which matters far more and which reading the diff would never have revealed.
\emph{Everything is written to a scratch root --- the archive is never the thing being overwritten.}
