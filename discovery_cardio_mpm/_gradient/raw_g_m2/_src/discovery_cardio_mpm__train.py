...
def main():
    ...
    ap.add_argument("--perturb", default="",
                    help="PHASE 3: 'name=value[,name=value]' -- set a scalar learnable before the "
                         "first forward. Used with --loss_probe to get the loss at a displaced "
                         "point THROUGH THE SAME CODE PATH the trainer uses, so the finite "
                         "difference is not being compared against a second implementation.")
    ap.add_argument("--loss_probe", type=int, default=0,
                    help="PHASE 3: print the loss at iteration 0 to full precision and exit, "
    ...
    _SCALARS = {"f_wl": f_wl, "f_ang": f_ang, "f_amp": f_amp, "f_ph": f_ph,
                "raw_g": raw_g, "raw_dur": raw_dur}
    if args.perturb:
        with torch.no_grad():
            for item in args.perturb.split(","):
                k, _, v = item.partition("=")
                k = k.strip()
                if k not in _SCALARS:
                    raise SystemExit(f"--perturb: unknown scalar {k!r}; "
                                     f"known: {sorted(_SCALARS)}")
                _SCALARS[k].copy_(torch.tensor(float(v), device=dev, dtype=_SCALARS[k].dtype))
                print(f"  PERTURB {k} := {float(v):.12g}", flush=True)
    ...
    for it in pbar:
        ...
        if args.loss_probe:
            # full precision, and the components too: a finite difference that moves only because
            # of the amplitude anchor is not evidence about the mechanism
            print(f"LOSSPROBE loss={loss.item():.17g} r2={r2_loss.item():.17g} "
                  f"amp={amp_loss.item():.17g}", flush=True)
            return
        ...
        if args.grad_check:
            ...
            for g in ("stiff", "fibre", "gain", "dur", "residual"):
                for prm in groups.get(g, []):
                    ...
                    if gr is not None and prm.numel() == 1:
                        # signed, full precision -- this is what the finite difference is compared
                        # against, and a norm would have thrown the sign away
                        print(f"GRADSCALAR {nm} {gr.item():.17g} value {prm.item():.17g}")
