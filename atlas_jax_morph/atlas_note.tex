...
is the most useful number the atlas has produced, and \S8 is about it.
...
\begin{phasebox}{Phase 4 --- Differential validation \hfill \done\ (107 min, 14 of 16)}
...
\begin{phasebox}{Phase 5 --- The forward figure \hfill \prog\ (it runs)}
...
\begin{center}
\includegraphics[width=\textwidth]{../log/atlas/jax_morph_proliferation/strip.png}\\
{\small\color{grey}Plexus, running operators normalized out of their code: relax + grow + divide,
their four founders, their parameters, 40 macro-steps. Compare with the reference strip in \S4.}
\end{center}
...
The shape matches; the population does not. That is the honest state, and it is worth more than a
figure that agreed. Two things are already ruled out: it is not an array bound (the buffer is five
times the final count) and it is not an inert operator (every scheduled operator is recorded as
having acted, on every frame it ran).
...
Phase 4 is now running that comparison properly for every mechanism, metric and threshold written
down first.
...
\emph{before} running anything. Fourteen passed; two ran out of time twice and are being retried
with a bigger budget rather than quietly dropped.
