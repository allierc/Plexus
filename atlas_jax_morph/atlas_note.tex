...
\input{_state/phase1_operators.tex}
...
App.~E.1 calls the good one} --- it is the language being reused, not the language being
extended.
...
Level buffer while the spec --- and the growth operator standing next to it --- kept it as a state
block. So it read its own default of zero, and the run completed, and the movie looked plausible.
...
\code{diff/certify.py} runs a real \code{config/atlas/} spec through \code{engine.run(grad=True)},
...
one fit and believed it.

\textbf{What it does and does not settle.} It does \emph{not} yet convict the estimator. The drift
is a bias in \emph{the answer after a fixed 20-step budget}, and a noisy gradient with a truncated
Adam run lags its target for reasons that have nothing to do with the estimator being biased.
Separating the two needs each $K$ run to convergence rather than to a step count, which is cheap
and is the next experiment. So the reference's trace/replay/score contract moves from
\emph{required} to \emph{still open} --- weaker than the note previously assumed, and the honest
position.

\textbf{For Figure 5 the operational conclusion is already usable}: fit with $K$ seeds averaged
per step, never one, and treat any single-seed number as a draw rather than an answer.
...
\textbf{Two honest qualifications, because this is the claim most worth over-selling.}

\begin{enumerate}[leftmargin=1.4em,itemsep=1pt]
\item \textbf{Every link is validated; the loop has never been run.} Each atlas spec exercises one
mechanism in isolation (\code{[seed\_state, regulate]}, \code{[seed\_state, grow\_radius]},
\code{[seed\_state, morphogen]}); the only composition is the proliferation model, which has no
gene network in it. Six differential tests do not add up to a demonstration.
\item \textbf{It is a COMPOSITION, not a fourth contract.} Registering ``gene--growth coupling'' as
new vocabulary would be exactly the inflated \emph{new} the ledger exists to prevent, and the
Plexus thesis is that compositions should not need new words. The claim to make is the narrower
and stronger one: \emph{three new contracts plus the growth the language already had close a
sense $\to$ regulate $\to$ grow loop that Plexus previously could not express at all.}
\end{enumerate}

The experiment is one spec --- \code{[seed\_state, morphogen, regulate, grow\_radius,
cell\_divide, relax]}, six operators, all validated, none new --- and it belongs to Phase 7's
``compositions the authors did not run'', because jax-morph's own worked example does not compose
these either.
