...
\textbf{Where it stands (in flight).} 32 jobs, $K \in \{1,2,4,8\}$ $\times$ 8 replicates, 24
frames, 20 Adam steps. $K=1$ and $K=2$ are complete:
...
\midrule
1 & 8 & 1.2072 & 0.0763 & \\
2 & 8 & 1.2606 & 0.0548 & ratio 0.72, against $1/\sqrt{2} = 0.707$\\
4 & \multicolumn{3}{l}{\emph{running}} & \\
8 & \multicolumn{3}{l}{\emph{running}} & \\
\bottomrule
\end{tabular}
\end{center}

Two seeds' worth of averaging shrinks the spread by very nearly the Monte-Carlo factor, which is
what an unbiased-but-noisy estimator does and \emph{not} what a biased one does. That is
encouraging and it is not yet a result: two points define no trend, and the means drift slightly
upward ($1.2072 \to 1.2606$), which would be the signature of a small low-$K$ bias if it persists.
\textbf{$K=4$ and $K=8$ decide it}, and they are the two that matter --- the conclusion will be
recorded here whichever way it falls.
