...
\textbf{What an agent actually costs --- measured, at last.} The budget column is an
\emph{allowance}, and until 1 August nothing recorded what was spent: nine call sites bypassed the
stopwatch, and a round once reported \code{calls: 0} for a round that made about twenty-five. Every
call now records tokens, turns, seconds and dollars. Six real analyst calls on two runs:

\begin{center}\small
\begin{tabular}{@{}l r l@{}}
\toprule
per analyst call      & \textbf{\$0.41}      & 6 calls, \$2.47 total\\
turns used            & \textbf{7.2}         & against a cap of 60 --- the cap never binds\\
wall clock            & \textbf{21 s}        & three analysts in parallel, 64\,s per run\\
cache hits            & 305\,k vs 172\,k new & 64\% --- why a call is \$0.41 and not \$3\\
\midrule
a five-run round      & \textbf{$\approx$\$11} & 26 calls: 15 analysts, 5 eye-checks, 6 others\\
\bottomrule
\end{tabular}
\end{center}

Three things follow, and they contradict what I told Cedric before measuring. \textbf{Turns are not
the lever}: 7.2 used of 60 allowed, so the cap has never once bound, and tightening it would only
truncate work already paid for. \textbf{Nor are the timeouts}: the allowance for the analyst block
is four minutes a call and it takes twenty-one seconds, so the ``116 minutes worst case'' was an
arithmetic ceiling nothing approaches. \textbf{The lever is the number of calls}, because a call
carries a fixed cost before it does any work --- a call returning the single word ``ok'' still
reports 2,993 input and 28,868 cache-creation tokens. Fifteen analyst calls is the largest line on
the board, and two analysts instead of three is a real third off the bill.
