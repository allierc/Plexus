...
\begin{phasebox}{Phase 4b --- Can the chemistry be simulated at all? \hfill \prog}
...
\textbf{Two corrections to what I had told Cedric an hour earlier.} First, \textbf{Okuda's setting
is reachable}. I had said it was not, and had begun writing that the model might need a different
kind of solver. That came from a short probe on the wrong side of a boundary I had not yet located.
Second, \textbf{one number cannot express the limit}, because there are two ways to fail and it
only knows about one. Running away is the one it forbids. The other is that the chemicals spread so
readily that the pattern smears out and dies: the simulation stays perfectly well behaved and
measures nothing. \textbf{Well behaved is not the same as alive.}

\textbf{The rule is now two conditions, and both were measured rather than derived.} The step size
must be under the limit, \emph{and} the spreading multiplier must stay under three. The second has
no formula behind it --- it came from watching fourteen settings run. It exists because the
...
\textbf{The limit sits below the first failure we saw, which is conservative on purpose.} The
measurements do not fall either side of a clean line: two settings above it were fine and one below
it was not, and the odd one out is the case where the suppressor travels furthest --- so something
beyond the two settings matters, and has not been identified. Until it is, the rule refuses two
combinations known to be perfectly good. Refusing a good proposal costs a proposal. Admitting a
divergent one has cost a batch and a conclusion, twice.
