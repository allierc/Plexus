...
checks}, one of which --- the Collector --- was an agent's job and should never have been.

\begin{figure}[h]\centering
\includegraphics[width=0.88\textwidth]{figures/agents_sheet.png}
\caption*{\small The team, before the 1 August rebuild --- sixteen roles. \textbf{Judge},
\textbf{Referee} and the duplicate check have since been dropped, and the \textbf{Collector} added;
the roster is now \code{ROLES.md}, and the code is checked against it.}
\end{figure}
...
premise, \emph{ambiguous} on a \textbf{usual} one & runs\\
\midrule
...
Analysts ($\times$3) & agent & what happened in this one run? Three independent readings, because
one reading is not reproducible. \textbf{Scale to 5 or 8 only if the measured disagreement demands
it} & \textbf{21 s} each\\
...
Evolution & agent & how would the winner be refined? & runs\\
...
back-propagation & runs\\
Supervisor & \textbf{check} & what runs next, and how much? \textbf{Owns budget, allocation and the
70/30 mixture.} The runtime controller & runs\\
\midrule
...
\vspace{2pt}
\textbf{Dropped: Judge, Referee, the duplicate check.} The Judge existed to settle the Eye-check
against the number, and demoting the Eye-check to observation dissolves the dispute; the Referee
ranked by tournament what a certified metric already ranks. \textbf{Both were called zero times.}
Either returns only on a real measurement ambiguity the metric bank cannot resolve.
...
2.4, Analysts 1.7, and one call each to the Eye-check, Interpreter and Evolution. \textbf{The
...
\code{calls: 0} for a round that made about twenty-five. Six real analyst calls came in at
