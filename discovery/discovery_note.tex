...
\textbf{Still to do}
\begin{itemize}[itemsep=1pt]
\item \textbf{Reachability.} At the corrected bound, Okuda's $\varphi = 10$ needs $\chi \leq 0.08$.
Whether his regime is reachable at all with an explicit step is now an open question, and the
honest answer may be that it needs an implicit one.
\item \textbf{The reaction has no bound at all}, deliberately --- the candidate was measured and
refuted. So a composition can still be refused by the diffusion check and diverge through the
reaction, and only the Biologist will notice.
\item \textbf{One preset does not compile} (\code{round\_44\_base}), for reasons unrelated to
stability. \code{validate\_space} sits at 54/56 on it.
\end{itemize}
