...
analysts' consensus --- a label derived from the same caption it then read --- so it could catch a
...
\begin{phasebox}{Phase 5 --- The first live run, and what it cost to learn \hfill \prog}
\textbf{It ran.} Six rounds were authorised, two completed before the lessons below made
continuing pointless: 23 agent calls, \$11, 29 minutes of agent time, eight simulations. The
loop proposed, reviewed, submitted, refused, scored, and steered itself --- the whole machine
turned over for the first time.

\textbf{And it produced one usable run out of eight.} Map coverage 0\%. Nothing was learned
about the biology. That is the honest headline, and the value of the round is entirely in why.

\textbf{What the loop got right, unprompted}
\begin{itemize}[itemsep=1pt]
\item The Critic refused every bad run and said why: chemistry diverged, buffer saturated. None
of them entered the ledger.
\item The straggler killer cut two runs at 46 minutes against an 11-minute batch median.
\item The Supervisor diagnosed its own sterility --- \emph{``surprise 0.00: the batch is
confirming what we already believe, near-zero information. Push adversarial''} --- and steered
the next round itself.
\item The round that produced nothing returned \emph{no admissible evidence} rather than scoring
what it had.
\end{itemize}

\textbf{Four defects, each invisible until something was looked at}

\textbf{1. The movies were black, and the first frame was black too.} This is the one Cedric
asked about, and the answer is worse than a rendering bug. The colour scale is computed once from
frames sampled across the whole run --- correctly, because a per-frame rescale makes a decaying
pattern and a stable one look identical, which is how an extinct field passed as coral for
months. But it was computed over samples \emph{including} non-finite values, and one such value
makes both bounds non-finite, so every cell in every frame is drawn as nothing. \textbf{Frame~0
was perfectly good and was drawn black because frame~800 diverged.} The artefact reported
``nothing happened'' for a run whose first half was fine --- the opposite of the truth, which is
worse than having no artefact. Fixed: the scale is taken from finite values only, cells that left
the model are drawn grey rather than white, and the run says on its own output how many there
were.

\textbf{2. The trajectory analysis had been dead on every run.} The per-frame series records the
morphology as a word, and the writer that saves the series casts every column to a number. It
raised on the first one, was caught, printed one line among hundreds, and the run carried on
without its per-frame table, its plot, its tube metrics, or --- the one that matters --- its
\emph{evidence horizon}. So peak and final were taken over all frames including any after the
mesh tore, which is exactly what the horizon exists to prevent.

\textbf{3. The reservoir gate fires one step too late.} Three of eight runs died on the cell
buffer. The gate refuses a run whose reservoir cannot hold its stated target, and these runs grew
past a target that was estimated too low. The check is correct and the estimate is not.

\textbf{4. Two files were being written as books.} \code{memory.md} reached 1904 words across six
appended blocks, in a file whose purpose is to hold \emph{state}. Both files now have a fixed
shape, checked: an append-only log with per-round fields, beside a state document with seven
named sections and no eighth.

\textbf{The pattern across all four} --- and across the day --- is one thing said four ways: the
check existed and fired correctly, and what it produced went nowhere. A refusal printed among
hundreds of lines, a horizon silently absent, a flag raised after the compute was spent, a record
grown past reading. \textbf{A check whose output nobody receives is not a check.}

\textbf{What has to be true before it runs again}
\begin{itemize}[itemsep=1pt]
\item A swallowed exception around an artefact must be loud. The rule has been written down since
July and this is the third time it was broken in the same file.
\item The reservoir target must come from the growth the composition actually asks for, not an
estimate that three runs in eight exceed.
\item A round that produces no admissible evidence should say so in the scoreboard, not only in
the terminal.
\end{itemize}
\end{phasebox}
