...
150 cells against a paper that says 200. Evolution was wired in the same pass; only the Referee's tournament remains idle.

\begin{figure}[h]\centering
\includegraphics[width=0.88\textwidth]{figures/agents_sheet.png}
\caption*{\small The team. Every agent now runs: the \textbf{Grounder} and \textbf{Evolution} were
written and never called until 1 August, and the Grounder's absence is why a batch started at 150
cells against a paper that says 2,000. Only the \textbf{Referee} is still idle.}
\end{figure}

\begin{center}\small
\begin{tabular}{@{}l p{5.9cm} l r l@{}}
\toprule
\multicolumn{5}{@{}l}{\textbf{Agents} --- they read evidence and form a judgement}\\
\midrule
Proposer & chooses which mechanism edits to test next; also checks the
batch against what has already been tried & 1  & not yet & runs\\
Peer-review & reads the proposed batch before it costs anything & 1  & not yet & runs\\
Analysts ($\times$3) & read one run each, independently, and say what happened & 15 & \textbf{21 s} each & runs\\
...
\addlinespace
\textbf{Grounder} & \textbf{reads Okuda's paper; sets each batch's starting conditions from it,
and refuses to advise if its own quotes do not check out} & 1  & not yet & runs\\
Evolution & refines the round's winner, rather than the loop only ever picking one
& 1  & not yet & runs\\
\midrule
\multicolumn{5}{@{}l}{\textbf{Checks} --- deterministic code that refuses. No judgement, no cost.}\\
\midrule
...
\midrule
\textbf{one round} & 20 of the 26 calls are the per-run readers, and they are
independent --- run together, the block costs its slowest call & 26 &
\textbf{64 s} per run & \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Only the analysts have been measured, and the gap is large.} Timing was wired on 1 August
--- until then nine call sites bypassed the stopwatch, and a round once reported \code{calls: 0}
for a round that made about twenty-five. Six real analyst calls: \textbf{21 seconds} each against
a four-minute allowance, and \textbf{7.2 turns} against a cap of 60. So neither the timeout nor
the turn cap has ever constrained anything, and the ``two hours a round'' arithmetic I put to
Cedric earlier was a ceiling nothing approaches. The other roles say \emph{not yet} rather than
carrying their allowance, because an allowance printed in a measurement column is how a ceiling
becomes a fact.
...
\textbf{(c) The per-run calls run together} --- \emph{done, and measured}. Three analysts read one
run concurrently, and runs are read concurrently across a batch. They are independent by design:
three readings exist because one is not reproducible, and no analyst's prompt depends on another's
answer. The eye-check still runs \emph{after}, because it is handed their consensus and is not
independent --- parallelising a dependency is not an optimisation, it is a different experiment.
\end{phasebox}
...
\begin{phasebox}{Phase 4b --- Numerical stability: can a spec be integrated at all? \hfill \prog}
\textbf{Why this became a phase, mid-flight.} The pre-launch stopped on a control whose chemistry
went non-finite. Chasing it found a stability bound that had been \textbf{50 times too permissive}
for as long as the campaign has existed --- and every ceiling derived from it inherited the same
factor, so the search box advertised a region the integrator cannot enter.

\textbf{The arithmetic.} An explicit diffusion step is stable while
$\mathrm{dt}\cdot\chi\cdot\max(d_a,d_h)$ stays below one. \code{translate} scales $\chi$ by
$1/\mathrm{dt}$ --- that is the clock fix, which exists because the chemistry was being integrated
on the mechanics substep --- so the engine steps $\mathrm{dt}\cdot(\chi/\mathrm{dt})\cdot d$, and
the $\mathrm{dt}$ cancels. The check multiplied by it anyway, against the \emph{unscaled} $\chi$,
and so reported \textbf{0.056} for a composition whose real number is \textbf{2.8}.

\textbf{Why nobody saw it.} The bound was a \emph{comment}: ``CFL stays satisfied:
$0.02 \times 65 \times 0.16 = 0.21$''. True when written. Then the defaults moved and Phase 2
widened the diffusivity box to reach Okuda's $\varphi = 10$, and the arithmetic in the comment
quietly stopped describing the code. A comment cannot be re-derived --- only re-read, and nobody
had cause to re-read it. \emph{A limit that one phase widens and another relies on has to be a
check.}

\textbf{What it cost.} \code{okuda\_route} --- the recipe named for the target --- sat at 2.8 and
passed every gate. Its chemistry diverged within a hundred frames, and the Biologist refused every
run that used it. That refusal is how this was found at all: the premise gate caught a defect no
metric would have shown.

\textbf{Done}
\begin{itemize}[itemsep=1pt]
\item The bound is computed, not commented: \code{diffusion\_cfl} and both ceilings drop the
cancelled $\mathrm{dt}$, and \code{translate} refuses a spec whose \emph{emitted} values breach it.
\item Defaults moved to \code{coral\_fixed\_ball}'s point --- chosen because it is \emph{measured}
to run, not because it sits inside a box.
\item \code{cfl\_audit.py}: every spec in the repository, on both paths that produce one. 52 of 54
hand-written configs are stable; the two that are not are the two that break, and the one which
actually ran went non-finite exactly as predicted.
\end{itemize}

\textbf{Then it was certified against the engine, and the corrected bound is still wrong.}
Twenty specs on the cluster, 300 frames each, sweeping across the predicted wall. An argument
about where a limit lies is worth nothing until the boundary it predicts is the boundary the
engine has --- and it is not:

\begin{center}\small
\begin{tabular}{@{}r r r l l@{}}
\toprule
\textbf{CFL} & $\chi$ & $d_h$ & \textbf{measured} & \\
\midrule
0.10 & 0.01 & \textbf{10} & finite, patterns & \textbf{Okuda's $\varphi = 10$ IS reachable}\\
0.16 & 0.08 & 2   & finite, patterns & \\
0.21--0.55 & 1.3 & 0.16--0.42 & finite, patterns & the working band\\
0.48 & 3 & 0.16 & finite, patterns & \\
\midrule
0.50 & 0.05 & 10  & \textcolor{bad}{non-finite} & predicted stable\\
0.64 & 4 & 0.16   & \textcolor{warn}{finite but INERT} & the activator dies; P4\\
0.65 & 1.3 & 0.5  & \textcolor{bad}{non-finite} & \\
0.96 & 6 & 0.16   & \textcolor{warn}{finite but INERT} & every cell activated; P4\\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Okuda's $\varphi = 10$ is reachable after all.} At $\chi = 0.01$ it runs finite and
patterns. I had inferred from a short probe that it was not, and said so. The probe was 80 frames
and tested $\chi = 0.05$, which does fail --- the wall for $d_h = 10$ sits between 0.01 and 0.05,
and I had generalised from the wrong side of it.

\textbf{One number cannot express this wall.} At $\chi = 1.3$ the chemistry survives to CFL 0.55
and dies at 0.65; at $\chi = 0.05$ it dies at 0.50. And there is a \emph{second} failure mode the
CFL says nothing about: at $\chi = 4$ and $\chi = 6$ the run stays perfectly finite and the
activator goes to zero. A stability bound does not see that, the Biologist does --- P4, ``the
chemistry was silently extinguished''. \textbf{Finite is not the same as alive}, and a bound that
only forbids divergence admits a whole region of dead runs.

\textbf{The bound is now two clauses, and both are measured.}
$\mathrm{CFL} \leq 0.45$ \emph{and} $\chi \leq 3$. The second could not be derived --- it came from
watching fourteen points on the cluster --- and it exists because there are \emph{two} failure
modes where the formula only knew about one. Divergence is the one a CFL forbids. Extinction is
the other: at $\chi = 4$ and $\chi = 6$ the run stays perfectly finite and the activator goes to
zero. $\chi$ multiplies both diffusivities, so raising it flattens the activator's own gradient as
fast as the inhibitor's, and past about 3 the local peak cannot outrun its own spreading. A stable
run that measures nothing is not caught by a stability bound; only the Biologist sees it, and only
afterwards.

\textbf{The limit sits below the first observed failure, which is conservative on purpose.} The
measurements do not admit a clean threshold: 0.48 and 0.55 were stable, 0.50 was not --- and the
0.50 case is $d_h = 10$, so something beyond the product matters, since a large $d_h$ fails where
a small one survives at the same CFL. Until that term is derived the bound refuses two points known
to be fine. Refusing a good composition costs a proposal; admitting a divergent one has cost a
batch and a conclusion, twice.

Checked against all eight measured points: every bad one refused, no bad one admitted.

\textbf{Still to do}
\begin{itemize}[itemsep=1pt]
\item \textbf{Derive the missing term.} A single product cannot order the failures, and the bound
is conservative until something explains why $d_h = 10$ behaves differently.
\item \textbf{The reaction still has no bound}, deliberately --- the candidate was measured and
refuted --- so a composition can pass the diffusion check and die through the reaction.
\item \textbf{Longer runs may move the wall.} These were judged at 300 frames; a run that survives
300 and dies at 900 is still a run the gate let through.
\item \textbf{One preset sits on the wrong side.} \code{round\_44\_base} ran healthily as a hand
config at CFL 0.056 --- 901 frames, 2983 cells, activator 0.993 --- and compiles to 2.8 as a
composition, because the clock fix scales $\chi$ by 50. Both are true, and they are not the same
experiment: the archived run's chemistry had a fiftieth of the reaction time the corrected clock
gives, which is precisely the defect the fix was written for. Keeping it as a trusted reference
keeps a pre-fix run in the trusted set. \code{validate\_space} is 54/56 on exactly this, and the
decision is Cedric's.
\end{itemize}

\textbf{Whose job this is.} The \textbf{Critic}. It is deterministic, it runs before any compute,
and it already refuses a composition that is illegal, unrunnable or already tried --- ``cannot be
integrated'' is the same kind of statement. It is not the Metrologist's, who certifies that an
\emph{instrument} measures what it claims, and not the Biologist's, who asks whether the
\emph{specimen} is a tissue. This one is about whether the arithmetic can be performed, which is a
property of the proposal, and the proposal is what the Critic reads.
\end{phasebox}

\begin{phasebox}{Phase 4c --- Multi-spot initialisation: several tubes, not one \hfill \prog}
