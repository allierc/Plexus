...
\textbf{Still to do}
\begin{itemize}[itemsep=1pt]
\item \textbf{Replace the single number with the measured region.} The safe band at 300 frames is
roughly CFL $\leq 0.45$ \emph{and} $\chi \leq 3$, and the second clause is not in any formula.
\item \textbf{The reaction still has no bound}, deliberately --- the candidate was measured and
refuted --- so a composition can pass the diffusion check and die through the reaction. Two of the
twenty did exactly that.
\item \textbf{Longer runs may move the wall.} 300 frames is where these were judged; a run that
survives 300 and dies at 900 is still a run the gate let through.
\end{itemize}
...
And the three seeds are \textbf{bit-identical}. \code{general.seed} does not reach the seeding
operator, so ``three independent runs'' are one run repeated. Replication was the entire reason for
running several spots, and it is not currently available --- which means every spread I have quoted
across seeds in this campaign describes nothing.
\end{phasebox}
