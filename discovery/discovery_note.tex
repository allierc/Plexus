...
the run recorded. \textbf{Runs after the batch and BEFORE the Analysts} --- run it after the
...
Analysts ($\times$3) & agent & what happened in this one run? \textbf{They do not measure --- they
label.} All three see identical numbers, already computed by certified instruments, so
$\times$3 measures \textbf{phenotype ambiguity}, not measurement uncertainty. \emph{Not} Robin's
$\times$8: their Finch writes the analysis code and so produces different numbers; we took
measurement away from the Analyst on purpose & \textbf{21 s} each\\
