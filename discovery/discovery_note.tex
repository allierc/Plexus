...
\textbf{Nine agents and five checks.} Seven agents run every round; two are written and never
called, and one of those two is the reason the weekend batch started at the wrong cell count.

\begin{figure}[h]\centering
%% HELD: the Grounder tile in icons.png was damaged by an edit of mine; awaiting a
%% regenerated sheet (friendly Grounder + book, biologist Premise gate, ranking Referee).
%\includegraphics[width=0.86\textwidth]{figures/agents_sheet.png}
\caption*{\small [figure held --- see note] The team. \textbf{Grounder} and \textbf{Evolution} are written and never called ---
that is a wiring fault, not a judgement on the role: the Grounder is the only agent that knows what
the target is, which is exactly why its absence cost us a batch.}
\end{figure}
...
\textbf{Nine agents, five checks --- not fifteen roles.} The old count inflated the roster by listing
deterministic code beside things that reason. The two dedup routines were the clearest case: ``has
this composition been tried'' is ten lines inside the Critic, and ``has this operator been requested
before'' is a dict scan on the escalation path. Neither is an agent, and neither belongs to the
Proposer --- they act on different objects at different points in the round.
...
\textbf{The Grounder is the important one, and I had previously reported it as running.} It is the
only role that reads Okuda's paper and the wider literature --- its own header lists the reference
model, the literature index and the vendored corpus --- and it has \textbf{no call site anywhere in
the pipeline}. Every one of its entry points is exercised only by its own smoke test.

That is not a cosmetic gap. It is why the weekend batch started at 150 cells when Okuda starts at
200, 2000 and 4000: \emph{nothing in the loop ever asks the paper anything}. The fix is to give it
the starting conditions alongside the figure targets it already holds, and to call it when a batch
is built --- so that the faithful share of each batch inherits Okuda's numbers from the paper rather
than from a default buried in a config file.

\textbf{Evolution} can only ever let the loop \emph{pick} from what it happened to propose, never
refine it. \textbf{Referee} would rank a batch by tournament instead of by sorting on one number.
Both are written and idle.
...
\textbf{(a) Wire the Grounder.} It is the only agent that reads the paper, and it has no call site
anywhere. Until a batch is built by \emph{asking it}, the starting conditions come from whatever
default sits in a config --- which is how 27 runs were launched at 150 cells against a paper that
says 200. It needs two things: Okuda's starting conditions added alongside the figure targets it
already holds, and a call at batch-construction time. The faithful share of a batch then inherits
his numbers from the paper; the exploratory share is free to leave them, which is the 70/30 split.

Two rules keep that from becoming a straitjacket. The Grounder \emph{advises} --- it supplies
numbers, it does not forbid others, or the exploratory 30\% would be dead. What is \emph{not}
optional is the reservoir check: a run that cannot reach the cell count it is aiming at measures
the array rather than the tissue, whichever share it belongs to.
