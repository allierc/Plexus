...
Peer-review & judges whether the batch is \emph{worth} the compute: is each prediction
falsifiable, is the claim already settled, is the stated reason a mechanism or a restatement
of the edit. It advises; it cannot refuse & 1  & not yet & runs\\
...
Critic & decides whether a proposal can be \emph{run}: type-legal edits, preconditions met,
parameters in range, not already evaluated, and afterwards whether every scheduled operator
actually acted. Rejects a causal claim with no ablation. Every refusal carries a reason code, and
it cannot be argued with & --- & --- & runs\\
Biologist & refuses a recipe that could not be a tissue, against the premises written in
\code{PREMISES.md} --- which it now \emph{reads}, rather than restating. Its verdict goes to the
Analyst reading that run and its round tally to the Meta-review & --- & --- & runs\\
...
\begin{center}
\includegraphics[width=\linewidth]{figures/agent_figure.png}
\end{center}
\vspace{-6pt}
{\small \textbf{The round, drawn in the order it runs.} Around the rim, each agent hands
its work to the next and the chain closes --- that half was built and it works. The inside of the
ring is the half that was not: the arrows that carry a \emph{finding} back to somebody who can act
on it. There should be a bundle. Three exist, all added this week, and the Supervisor's steer, the
Interpreter's causal descriptions and Evolution's refinement still reach nothing --- so a round
cannot be changed by what the round found. Drawing it is what made the shape obvious: fourteen
agents, and the only thing they shared was the direction of travel. The counts in the middle are
computed from the same list the arrows are drawn from, because the first version of this caption
said ``one of five'' and was wrong within the hour.\par}
...
\emph{the activator had decayed to NaN, the chemistry was extinct} --- and an Analyst read that
run's numbers and named a phenotype. Two hand-offs now exist: the per-run verdict goes to the
\textbf{Analyst} that is about to read it, in the document's own words and with a field it must
