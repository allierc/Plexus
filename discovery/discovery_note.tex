...
\caption*{\small \textbf{The loop after the 1 August rebuild.} Three acts and a cross-run
controller. Blue is an LLM call, green a deterministic check, orange code that is not an agent at
all. The roster is \code{ROLES.md} and \code{roles.py -{}-check} compares it against the code in
both directions. \emph{(The figure still shows Evolution, which has since been removed --- it and
the Meta-review were both answering ``what should change next?''.)}}
\end{figure}
...
\subsection*{What the first cold start found, and it is not good news}

The loop now begins by asking the Archivist where to start, from what is already on disk rather
than from a seed sphere --- and the first thing it did was read \code{mechanics.npz}, which
nothing had ever read. \textbf{Across all 62 finished runs in \code{log/okuda}, \code{n\_protruding}
never once exceeded zero, and \code{protr\_peak} never exceeded 1.13.}

The scalar summary had been reporting ``best sound run: 1.11'', which reads like a result. It is a
sphere with noise on it. \textbf{This project has never produced a protrusion}, and the number that
says so was sitting in every run's mechanics file, unread, for the whole campaign. So the cold
start refuses to name a starting point --- \emph{``there is no measured starting point for a
campaign whose objective is a tube''} --- and the frontier falls back to the reference recipes as a
\emph{recorded choice} rather than as a default nobody noticed.

\subsection*{The launch}
\code{campaign\_loop.py}, unattended, stopping for a closed admission gate, two consecutive aborts,
or the round budget. What to watch: the budget ledger, the Collector's holes, and whether any run
ever reports a protruding cell --- which would be the first in the project's history.
\end{phasebox}
