...
function draw(j){clear();LMATS=[];const T=j.tissue;let center=new THREE.Vector3();const R=$('right');
 ...
 for(const [name,s] of Object.entries(j.sets)){if(!s.pos||(T&&name===T.set))continue;const names=s.type_names||[];const map=[],cols=[],pos=[];
  ...
  const p=new THREE.Points(g,new THREE.PointsMaterial({size:0.2,vertexColors:true}));p.userData={kind:'cluster',set:name,map};scene.add(p);pick.push(p);}
...
async function cpoll(){try{const j=await (await fetch('/api/bio/claude?since='+cseen)).json();if(j.lines&&j.lines.length){const el=$('claude');el.textContent+=j.lines.join('\n')+'\n';el.scrollTop=el.scrollHeight;cseen=j.n;}
 $('cstat').textContent=j.running?'running... '+j.seconds+'s':(j.error?'error: '+j.error.slice(0,200):(j.n?'done in '+j.seconds+'s':''));$('cbtn').disabled=!!j.running;}catch(e){}finally{setTimeout(cpoll,1200);}}
cpoll();
