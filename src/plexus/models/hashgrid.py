...
CARRIED ACROSS FROM `ngp-demo/ngp/hashgrid.py` CHARACTER FOR CHARACTER below this note, for the
same reason `render_vtk` was carried from `discovery_okuda`: an encoder that computes a slightly
different number in two places is an encoder whose two versions cannot be compared, and every
measurement that repo made -- collision gradient splits, level specialisation, the px-per-cell
settling -- was made against THIS arithmetic. Fixes belong upstream and come back by copy.

Five things here that a from-scratch version tends not to have, and each of them is load-bearing
for a spatiotemporal fit rather than a nicety:

  * PER-AXIS base / scale / max resolution. An (x, y, t) field needs its time axis capped at the
    frame spacing independently of space; a scalar cap cannot say that.
  * `smoothstep` interpolation, per axis. With linear weights the encoding is only C^0 and its
    second derivative is zero almost everywhere, which silently kills any Laplacian taken through
    it. And smoothstep is NOT free: its weight derivative vanishes at every cell boundary, so on an
    axis whose cells align with the samples -- a time axis at the frame spacing -- it forces df/dt
    to zero AT every sample. Space smoothstep, time linear.
  * `set_level_window(alpha)`, the BARF/Nerfies coarse-to-fine gate. It is also what every
    per-level PICTURE is built from: "what level k adds" is f(<=k) - f(<=k-1), so without it there
    is no level montage and no level kymograph at all.
  * ONE FLAT TABLE with per-level offsets, dense where the level fits and hashed where it does not.
  * the corner weights as an explicit chain of multiplies rather than `Tensor.prod`, whose double
    backward goes through a division and leaks ~1e-2 of spurious curvature in float32 -- enough to
    swamp the exactly-zero second derivative of a linear interpolant.
