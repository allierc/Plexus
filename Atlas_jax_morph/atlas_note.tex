...
\begin{phasebox}{Phase 5 --- The figure \hfill \todo\ ($\sim$1 day)}
One headline result of the paper, rebuilt in Plexus from the normalized operators. This is the
evidence that the ledger describes something real rather than a filing system.
\end{phasebox}

\begin{phasebox}{Phase 6 --- Promotion, and past the paper \hfill \todo}
...
\begin{enumerate}[leftmargin=1.4em]
\item \textbf{Phase 1 is cheap --- shall I just run it?} Twenty-three excavator calls, about two
and a half hours, no cluster. It produces no verdicts, only readings, so there is little to get
wrong; and Phase 2 cannot start without it.
\item \textbf{Which figure for Phase 5?} The paper's headline is engineered morphogenesis ---
optimising a gene network until a cluster self-organises into a specified shape. Reproducing
\emph{that} exercises the inverse half of Plexus, not just the forward simulator, and is the
stronger result. A forward-only figure --- proliferation plus chemical patterning --- is perhaps a
day's work and proves considerably less.
