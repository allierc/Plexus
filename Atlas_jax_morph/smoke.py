"""smoke -- the first reference artefact, and the check that the oracle is worth diffing against.

A differential test compares our trajectory to theirs. That comparison means nothing unless
*theirs* is reproducible: if the reference gives a different answer on the second call, any
disagreement we later measure could be the reference's own noise. So this script does not just
run the reference -- it runs the same simulation twice from the same PRNG key and asserts the two
histories are bit-identical, before writing anything down as reference.

The model is the authors' own proliferation composition (examples/04_physics_examples.ipynb):
mechanical relaxation under a Morse potential, saturating growth, stochastic division. Three of
the four mechanisms the atlas is here to normalize, in one run.
"""
