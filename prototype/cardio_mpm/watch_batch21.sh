...
SSH="ssh -o BatchMode=yes -o ConnectTimeout=10 -o ServerAliveInterval=10 -o ServerAliveCountMax=3 $CLUSTER_SSH"
while true; do
  out=$(timeout 90 $SSH "bash -l -c 'bjobs -o \"jobid stat\" $JOBS 2>/dev/null'" 2>/dev/null)
