...
A = args.amplify
if args.overlay:
    fig, a0 = plt.subplots(1, 1, figsize=(8.4, 8.9)); ax = [a0, a0]
    s_nodes = ax[0].scatter(ref_nodes[:, 0], ref_nodes[:, 1], s=2.4, c=GREEN, lw=0)
    s_band = ax[1].scatter(X0[band, 0], X0[band, 1], s=0.5, c=DIM, lw=0)
    s_part = ax[1].scatter(X0[~band, 0], X0[~band, 1], s=0.7, c="#7fb4ff", lw=0, alpha=0.9)   # particles ON TOP
    names = (f"green: {ref_nodes.shape[0]:,} tracking nodes (recording); blue: {X0.shape[0]:,} MPM particles (model); "
             f"displacement from rest x{A:g}",) * 2
else:
    ...
    names = (f"recording: {ref_nodes.shape[0]:,} tracking nodes, displacement from rest x{A:g}",
