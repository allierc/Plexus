...
$PY movie.py           --continuous --device cuda:0 $A --params "$FIT" --tag cont --seconds 6
$PY movie_image.py     --continuous --overlay --device cuda:0 $A --params "$FIT" --tag cont --amplify 4 --seconds 6
$PY movie_particles.py --continuous --overlay --device cuda:1 $A --params "$FIT" --tag cont --seconds 6
...
$FF -loglevel error -y -i progress_overlay_cont.mp4 -i progress_particles_cont.mp4 -filter_complex \
  "[0:v]scale=-2:1000[a];[1:v]scale=-2:1000[b];[a][b]hstack=inputs=2[v]" -map "[v]" -r 26 \
