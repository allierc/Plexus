...
for name, (a_, b_), col in zip(PAIRS, PAIRS.values(), (BLUE, RED)):
    ...
    for k, (key, lab_) in enumerate((("g", "g (strain)"), ("g2", "g2 (strain)"),
                                     ("delay", "delay (frames)"), ("phi", "axis (deg)"))):
        ...
        ax[k].set_xlabel(f"seed 0: {lab_}"); ax[k].set_ylabel("seed 1")
    table[name] = row
ax[0].legend(loc="upper left", fontsize=9)
for a, s in zip(ax, "abcd"): a.text(-0.02, 1.01, s, transform=a.transAxes, fontsize=13, va="bottom", ha="right")
fig.savefig(os.path.join(HERE, "out", "figures", _A.out), dpi=130, bbox_inches="tight")
