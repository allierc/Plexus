...
fibre-mediated version -- the matrix itself resisting growth, which is what `ecm_load_3d` does -- is
...
A CLAMP, NOT A FORCE, AND THAT IS A CHOICE WITH A COST. `shape_energy_3d` owns the vertex force loop
...
@register_operator("plate_confine_3d", family="mechanics", set="vertex", kind="structural")
class PlateConfine3D(Structural):
    ...
    def forward(self, H, mask=None):
        ...
        if not self._said:
            print(f"[plate_confine_3d] {self.at}: rigid plates at {self.centre:+.4g} "
