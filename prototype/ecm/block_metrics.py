...
WHY END-TO-END AND NOT CONTOUR LENGTH. `seed_ecm` jitters every particle by 0.004 box units across a
...
THE STRAND IS AN INDEX CONVENTION, NOT DATA. `seed_ecm` lays strand k as particles k*per .. (k+1)*per-1
...
def _spec(d):
    ...
    per = max(1, int(s["sets"]["mpm_particle"]["per_parent"]) // int(ops["seed_ecm"]["n_fibres"]))
    return dict(per=per, n_fibres=int(ops["seed_ecm"]["n_fibres"]),
                fibre_len=float(ops["seed_ecm"]["fibre_len"]),
