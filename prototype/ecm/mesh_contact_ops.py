...
star-shaped about its own centroid -- `cell_to_ecm[replay]` already relies on this and P11 is the
...
the artefact `cell_exclude_3d` was written to sweep up rather than to explain. The grid here gives
...
class MeshContact(Lateral):
    ...
    EMIT = "mpm_acceleration"          # consumed by `mpm_scatter` as a_ext, like `cell_to_ecm`
    SUPPORTED_DIMS = [3]
    ...
    def _build(self, j, dev, dt_, alpha=0.0):
        ...
        the positions of the vertices the two meshes share are blended. `divide_3d` appends vertices
        ...
        # VERTEX VELOCITY, from the next kept mesh. `divide_3d` appends vertices and never renumbers
...
@register_operator("mesh_inside_count", family="hierarchy", set="particle", kind="lateral")
class MeshInsideCount(Lateral):
    """How many matrix particles are behind the surface, and how deep -- measured, not fixed.

    `cell_exclude_3d` answers the same question by PROJECTING the offenders out, which makes the
...
def selftest(tissue, scale=0.00853, dev="cuda:0", n=40000, n_brute=400, meshes=(0, 100, 199)):
    ...
    `cell_exclude_3d` was written to sweep up rather than to explain. Two questions, both of which
    can come back wrong:
