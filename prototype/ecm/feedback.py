...
THE QUESTION. `plate_confine_3d` produces an ovoid by fiat -- the plates are rigid, so the aspect ratio
...
tissue has nowhere to put one. So:
    ...
    STEP 0   an ANISOTROPIC matrix -- a disc-shaped cavity, thin above and below the tissue and roomy
             around its equator -- loaded by the unconfined tissue. `cell_to_ecm` records the contact
    ...
    STEP 2   the tissue re-run WITH that load, and then the matrix re-run against the tissue that
             comes out. One full staggered iteration. `ecm_load_3d`'s docstring states what a single
