...
|---|---|---|
| `seed_ecm`, `seed_basement_membrane` | seed | initial condition, fine |
| `cell_to_ecm` | mechanics | replay of the recorded surface pushing the stroma -- fine |
| `cell_exclude_3d` (on the ECM) | mechanics | **positional projection** -- see 3 below |
| `mpm_strain / scatter / grid_update / gather` | mpm | the solver, fine |
| `mpm_tissue_boundary` | field | **the hack** -- see 1 |
| `basement_membrane_continuum_strain` | lateral | **not a mechanism** -- see 2 |

## 1. `mpm_tissue_boundary` is a numerical device, not a mechanism
...
**Replaced by** `basement_membrane_contact` (runs 110/111): penetration measured per PARTICLE against
...
## 2. `basement_membrane_continuum_strain` is measurement registered as an operator
...
## 3. `cell_exclude_3d` still launders deformation, now only for the stroma
