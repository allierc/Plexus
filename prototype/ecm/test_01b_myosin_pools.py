...
TWO QUESTIONS, ONE OF WHICH THE FOLDER 01 COULD NOT ASK. When `divide_3d` inserts a vertex into an
...
the mesh's description of it. Two things must therefore hold, and they are separate claims:
  ...
  1. THE AMOUNT IS CONSERVED.  N_(a,n) + N_(n,b) = N_(a,b), where N_e = m_e * l_e is the myosin ON a
     junction and m_e -- the multiplier `shape_energy_3d` applies to the line tension -- is its
...
def snapshots(path):
    ...
    misalignment that `junction_myosin_sync` exists to prevent.
    """
    z = np.load(path)
    out = []
    for i in range(len(z["mesh_frames"])):
        ...
        if myo.size != es.size:
            raise RuntimeError(f"{os.path.basename(path)} frame {t}: myosin {myo.size} against "
                               f"{es.size} half-edges -- the cache predates junction_myosin_sync.")
...
def splits(S):
    ...
    A vertex `divide_3d` inserts is APPENDED, so its index is >= the previous snapshot's live vertex
...
def t1_rate(path):
    ...
    cells) and `reconnect_t1_3d` runs every 4 frames, so the denominator is the frame SPAN and not the
