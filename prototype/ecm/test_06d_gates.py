"""
06d_gates -- G40 to G45 re-measured on the driver that actually works.
...
def main():
    ...
    for kn in (2.5, 5.0, 10.0, 20.0):
        ...
        print(f"[06d] kappa_n {kn:5g}  lam_geo {sweep[kn]:.4f}", flush=True)
    ...
    for k, v in gates.items():
        print(f"[06d] {'PASS' if v['pass'] else 'FAIL'}  {k}: {v['value']}", flush=True)
    ...
    print(f"[06d] gates.png -> {d}", flush=True)
