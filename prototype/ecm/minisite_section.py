...
def main():
    ...
    def _cap2(run):
        ...
        extra = {
            "sc_inh_soft": "Nothing dies here: B does not kill the cell, it switches its GROWTH "
                           "off, and teal is a cell whose growth is off. This is the soft end of "
                           "that ladder — growth is half off at 0.65 of the inhibitor's own "
                           "maximum, so only B's peaks stop it — and what it leaves is fine red "
                           f"spots growing inside broad teal regions that do not: "
                           f"{s['n_spots_final']} growing spots, "
                           f"{s['spot_spacing_cells_final']:.1f} cells apart",
            "tsd_max": "Here B kills instead: it marks a cell for apoptosis where it runs LOW, "
                       "from frame 180, up to a quarter of the population at once. That leaves "
                       "under half the cells the inhibited run beside it keeps, and it is not "
                       "the same thing as growth being "
                       "slowed down — the two fields are patterned INDEPENDENTLY, so the second "
                       "one is deciding how much of what the first builds survives",
