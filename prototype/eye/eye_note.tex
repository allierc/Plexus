...
\textbf{What Phase 3 should be.} Not a controller. The gain matrix names its own targets, in order:
lengthen the obliques (the $1.10^\circ$ torsional gain is the binding constraint on the one motion
only they can produce), then raise $A/E$ toward the pulley ceiling bisected in Phase~0, then
re-probe. The same six runs measure whether it worked --- and cost 90 minutes, against the days the
closed-loop campaign spent guessing.
