...
def shape(mX, membrane, nbin=64):
    ...
    # DIRECTION of the m=1 shell dipole (deg): the theta where the boundary bulges furthest out.
    # reconstruction (2/N)|c1|cos(theta + angle(c1)) peaks at theta = -angle(c1). Reproducibility of
    # THIS angle across seeds (not just fourier_m1 magnitude) is the ORI axis test. -y sediment -> ~+90.
    if len(c) > 1:
        out["shape_axis_angle"] = round(float(np.degrees(-np.angle(c[1]))), 2)
...
def partition(aX, occ, at, r0=0.02):
    ...
    # mutual information between type and y-position (symmetric to mi_type_x -> axial order along gravity -y)
    yb = np.clip((P[:, 1] * 8).astype(int), 0, 7)
    miy = 0.0
    for t in np.unique(typ):
        for yi in range(8):
            pxy = np.mean((typ == t) & (yb == yi))
            if pxy > 0:
                miy += pxy * np.log2(pxy / (np.mean(typ == t) * np.mean(yb == yi) + 1e-12) + 1e-12)
    out["mi_type_y"] = round(float(max(miy, 0.0)), 4)
    # DIRECTED type dipole: vector from the minority/other centroid to the type.min() centroid.
    # magnitude = how far the two type-domains' centres separate (world units); angle (deg) = which way
    # the partition points. Reproducibility of THIS angle across seeds = the demix has a set axis (ORI).
    lo = typ == typ.min()
    ca = P[lo].mean(axis=0); cb = P[~lo].mean(axis=0); dvec = ca - cb
    out["type_dipole"] = round(float(np.hypot(dvec[0], dvec[1])), 4)
    out["type_axis_angle"] = round(float(np.degrees(np.arctan2(dvec[1], dvec[0]))), 2)
    return out
