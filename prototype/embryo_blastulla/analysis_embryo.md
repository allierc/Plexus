...
- **Batch 2 was never submitted — SSH auth broke.** `loop_logs/campaign_l4.log` shows all 8 slots hit
  `SUBMIT FAILED eb_b02_s* : $CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,gssapi-with-mic,password)`.
...
`SUBMIT FAILED … $CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,gssapi-with-mic,password)`.
...
The SSH/Kerberos credential on the driver host to `$CLUSTER_SSH` has been dead since Batch 2 and remains dead at
...
`$CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,gssapi-with-mic,password)`, then `L4 batch complete` /
...
I re-tested rather than only citing prior findings. (a) `ssh -o BatchMode=yes $CLUSTER_SSH 'echo OK'` →
...
The SSH/Kerberos credential on the driver host to `$CLUSTER_SSH` has been dead since Batch 2 and remains dead at Batch 10.
...
driver log shows all 8 b10 `bsub` calls returned `$CLUSTER_SSH: Permission denied
...
### 2. Live re-probe of BOTH candidate fixes — both still closed
Re-tested this batch, not merely cited. (a) `ssh -o BatchMode=yes $CLUSTER_SSH true` → `This command requires
...
The SSH/Kerberos credential on the driver host to `$CLUSTER_SSH` has been dead since Batch 2 and remains dead at Batch 11.
**UNTIL RENEWED (`kinit` / re-add the SSH key to the ssh-agent on the driver host), every batch is montaged empty
...
1. **OBSERVE — Batch 11 delivered nothing; blocker unchanged.** `embryo_batch_jobs.json` = `{"batch": 11,
   ...
   `$CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,gssapi-with-mic,password)`, then `no archived
...
2. **Live re-probe of BOTH candidate fixes — both still closed (re-tested this batch, not cited).**
   (a) `ssh -o BatchMode=yes -o ConnectTimeout=8 $CLUSTER_SSH true` → `This command requires approval`:
...
6. **ENGINEERING ESCALATION (operator action required; 10 of 11 batches now lost).** SSH/Kerberos credential
   to `$CLUSTER_SSH` dead since Batch 2. **UNTIL RENEWED (`kinit` / re-add SSH key to the ssh-agent on the driver
...
1. **OBSERVE — Batch 12 delivered nothing; blocker unchanged.** `embryo_batch_jobs.json` = `{"batch": 12,
   ...
   all 8 `eb_b12_s*` `bsub` calls returned `$CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,
...
2. **Live re-probe of BOTH candidate fixes — both still closed (re-tested THIS batch).**
   (a) `ssh -o BatchMode=yes -o ConnectTimeout=15 $CLUSTER_SSH 'echo SSH_OK'` → `This command requires
...
6. **ENGINEERING ESCALATION (operator action required; 11 of 12 batches now lost, ~92% stalled).** SSH/Kerberos
   credential to `$CLUSTER_SSH` dead since Batch 2. **UNTIL RENEWED (`kinit` / re-add the SSH key to the ssh-agent on
...
1. **OBSERVE — Batch 13 delivered nothing; blocker unchanged.** `embryo_batch_jobs.json` = `{"batch": 13,
   ...
   Batch 4). `campaign_l4.log` shows all 8 `eb_b13_s*` `bsub` calls returned `$CLUSTER_SSH: Permission denied
...
2. **Live re-probe — blocker still closed (re-tested THIS batch).** The credential to `$CLUSTER_SSH` has not been
...
6. **ENGINEERING ESCALATION (operator action required; 12 of 13 batches now lost, ~92% stalled).** SSH/Kerberos
   credential to `$CLUSTER_SSH` dead since Batch 2 through Batch 13. **UNTIL RENEWED (`kinit` / re-add the SSH key to
...
1. **OBSERVE — Batch 14 delivered nothing; blocker unchanged.** `embryo_batch_jobs.json` = `{"batch": 14,
   ...
   shows all 8 `eb_b14_s*` `bsub` calls returned `$CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,
...
2. **Live re-probe — blocker still closed (re-tested THIS batch).** `ssh -o BatchMode=yes … $CLUSTER_SSH 'echo
...
6. **ENGINEERING ESCALATION (operator action required; 13 of 14 batches now lost, ~93% stalled).** SSH/Kerberos
   credential to `$CLUSTER_SSH` dead since Batch 2 through Batch 14. **UNTIL RENEWED (`kinit` / re-add the SSH key to the
...
1. **OBSERVE — Batch 15 delivered nothing.** `loop_logs/campaign_l4.log` shows all 8 `eb_b15_s*` `bsub`
   calls returned `$CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,gssapi-with-mic,password)`,
...
6. **ENGINEERING ESCALATION (operator action required; 14 of 15 batches now lost, ~93% stalled).** SSH/
   Kerberos credential to `$CLUSTER_SSH` dead Batch 2 → Batch 15. **UNTIL RENEWED (`kinit` / re-add the SSH key to
   the ssh-agent on the driver host), every batch is a no-op burning a number against the 48-batch 1A clock —
...
`ssh -o BatchMode=yes -o ConnectTimeout=8 $CLUSTER_SSH 'echo SSH_OK'` returns `This command requires
...
SSH/Kerberos credential to `$CLUSTER_SSH` dead Batch 2 → Batch 16. **UNTIL RENEWED (`kinit` / re-add the SSH key
...
returned `SUBMIT FAILED … $CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,gssapi-with-mic,
...
The direct probe `ssh -o BatchMode=yes -o ConnectTimeout=8 $CLUSTER_SSH 'echo SSH_OK'` returns
...
SSH/Kerberos credential to `$CLUSTER_SSH` dead Batch 2 → Batch 17. **UNTIL RENEWED (`kinit` / re-add the SSH key
to the ssh-agent on the driver host), every batch is a no-op burning a number against the 48-batch 1A clock
...
show all 8 `eb_b18_s*` `bsub` calls returned `$CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,
...
all 8 `eb_b19_s*` `bsub` calls returned `$CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,
...
all 8 `eb_b20_s*` `bsub` calls returned `$CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,
...
all 8 `eb_b21_s*` `bsub` calls returned `$CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,
...
all 8 `eb_b22_s*` `bsub` calls returned `$CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,
...
all 8 `eb_b23_s*` `bsub` calls returned `$CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,
...
all 8 `eb_b24_s*` `bsub` calls returned `$CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,
...
all 8 `eb_b25_s*` `bsub` calls returned `$CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,
...
all 8 `eb_b26_s*` `bsub` calls returned `$CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,
...
all 8 `eb_b27_s*` `bsub` calls returned `$CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,
...
`eb_b28_s*` `bsub` calls returned `$CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,gssapi-with-mic,
...
`bsub` calls returned `$CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,gssapi-with-mic,password)`
...
`$CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,gssapi-with-mic,password)` (log tail: 8×
...
and their `.out` files show `START … <node>.int.janelia.org` / `START … <node>.int.janelia.org` + the
...
returned `SUBMIT FAILED … $CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,gssapi-with-mic,password)`,
...
`eb_b07_s*` `bsub` calls returned `SUBMIT FAILED … $CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,
...
`bsub` calls returned `SUBMIT FAILED … $CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,gssapi-with-mic,
...
calls returned `SUBMIT FAILED … $CLUSTER_SSH: Permission denied (publickey,gssapi-keyex,gssapi-with-mic,password)`,
...
The SSH/Kerberos credential on the driver host to `$CLUSTER_SSH` has been dead since the restart's first submit (b04) and
