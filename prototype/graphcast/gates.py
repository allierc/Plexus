...
def write_tex(table: dict[str, Gate], path: str, rel_to: str | None = None) -> str:
    ...
    lines = ["% GENERATED by gates.py -- do not edit",
             ...
             r"\begin{tabularx}{\textwidth}{@{}llLllL@{}}\toprule",
             r"& \textbf{gate} & \textbf{threshold} & \textbf{measured} & \textbf{outcome} & \textbf{figure}\\\midrule"]
    ...
    for tier in TIERS:
        ...
        if not rows:
            continue
        lines.append(r"\multicolumn{6}{@{}l}{\textit{" + label[tier] + r"}}\\")
        for g in rows:
