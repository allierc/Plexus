#!/usr/bin/env python
"""train_08_07 -- from 08_06, four additions to fix the residual failures:

  (1) REST-RETURN regul: after each beat the model now rolls a QUIESCENT tail (zero
      activation) and is penalised for any leftover displacement from rest -- this kills
      the slow cycle-to-cycle drift (nodes never returned to their nominal rest position,
      partly because the learnable k_anchor was free to vanish).
  (2) OPENNESS regul: the per-node trajectory's minor-axis extent is pushed UP toward the
      real beat's, so the model stops collapsing loops into compact lines / strings.
  (3) LEARNABLE FitzHugh-Nagumo: the electrical params (a, b, eps) + the u->force gate
      (theta, eta) are global learnable scalars (0-D synchronous rollout, differentiable).
  (4) LEARNABLE A_ij in [-1, 1]: per-edge mechanical coupling weights (tanh-bounded),
      scatter-applied in forward_beat. Plotted as a per-node mean-coupling map.

Still on top of 08_03's machinery: UNet maps, phase map, boundary anchored (train+render),
cycle-batched RMSE, device/compile/wall-clock-stop, png+model checkpoints. 137^2 by default.

Run:  CARDIO_DEVICE=cuda:1 CARDIO_COMPILE=1 CARDIO_MAX_SECONDS=28800 CARDIO_CKPT_EVERY=5000 \
      PYTHONPATH=src .../python prototype/cardio/cardio_train08_07.py
"""
...
# nodes are pinned to GT (green==red) in BOTH train and render -- as in train_08_06. BWIDTH is
...
LAM_OPEN = float(os.environ.get("CARDIO_LAM_OPEN", "3.0"))    # weight: open the loops
...
class FHNParams(nn.Module):
    def __init__(self):
        ...
        self.eps = nn.Parameter(torch.tensor(0.3))
        self.theta = nn.Parameter(torch.tensor(0.0)); self.eta = nn.Parameter(torch.tensor(0.3))

    def clamp_(self):
        self.a.clamp_(0.3, 1.0); self.b.clamp_(0.4, 1.2); self.eps.clamp_(0.1, 0.6)
        self.theta.clamp_(-0.5, 0.8); self.eta.clamp_(0.1, 0.6)
...
# --------------------------------------------------------------------------- #
#  Multi-cycle render forward (same forward_beat mechanics WITH A_ij, eager/no-grad)
# --------------------------------------------------------------------------- #
def render_forward(sk, gn, fb, sc, fhn, pm, aij, geo, grid, X):
    ...
    pred = R.forward_beat(sk, gn, fb, sc, geo, phased, real_disp=real_disp, aij=aij)  # [Tt,N,2] disp
...
# --------------------------------------------------------------------------- #
def main():
    print(f"=== train_08_07 [{ARCH_NAME}]: +rest +open regul, learnable FHN + A_ij  (dev={DEVICE}) ===")
    ...
    groups = [
        ...
        {"params": grad(list(sc.parameters()) + list(fhn.parameters())), "lr": LR_OP, "name": "operator:scalars+FHN"},
    ]
    ...
    print(f"  GRID={GRID} N={N} edges={E} | k_anchor LEARNABLE | Q={Q_QUIET} "
          f"lam_rest={LAM_REST} lam_open={LAM_OPEN} | N_ITER={N_ITER} max_s={MAX_SECONDS:.0f} ckpt={CKPT_EVERY}")

    def do_ckpt(tag):
        with torch.no_grad():
            sk, gn, fb = maps_from_unet(net(x))
            fp, u_raw, w_raw = render_forward(sk, gn, fb, sc, fhn, pm, aij(), geo, GRID, X)
        ...
        torch.save({"net": net.state_dict(), "pm": pm.state_dict(), "sc": sc.state_dict(),
                    "fhn": fhn.state_dict(), "aij": aij.state_dict()}, os.path.join(ckdir, f"model_{tag}.pt"))
    ...
    while it < N_ITER and (MAX_SECONDS <= 0 or time.time() - t0 < MAX_SECONDS):
        opt.zero_grad()
        sk, gn, fb = maps_from_unet(net(x))
        base_1d, _, _ = fhn_activation(fhn, R.T_FIT, period=1000)
        ...
        for b in idx:
            ...
            pred = forward_beat(sk, gn, fb, sc, geo, base_full, real_disp=disp_full, aij=A)
            ...
            l_open = (open_real[b] - openness(pred_beat)).clamp(min=0).mean()              # open up loops
        ...
        with torch.no_grad():
            ...
            sc.k_anchor.clamp_(0.01, 0.5); fhn.clamp_()
        pbar.set_postfix_str(f"loss={loss.item():.5f}")
        if it % 50 == 0:
            with torch.no_grad():
                fr = torch.stack([rmse(R.resample(forward_beat(sk, gn, fb, sc, geo, base_full,
            ...
            tqdm.write(f"  it {it:5d} [{el/60:.1f}m {el/max(it,1)*1000:.0f}ms/it] loss {loss.item():.5f} fitRMSE {fr:.5f} "
                  f"beta={sc.beta.item():.2f} k_anch={sc.k_anchor.item():.3f} a={fhn.a.item():.2f} "
                  f"eps={fhn.eps.item():.2f} A[{A.min().item():.2f},{A.max().item():.2f}] φ±{pm().abs().max().item():.0f}")
        if it % CKPT_EVERY == 0:
            do_ckpt(f"{it:05d}")
        it += 1
        pbar.update(1)
    ...
    with torch.no_grad():
        sk, gn, fb = maps_from_unet(net(x))
        fit_pos, u_raw, w_raw = render_forward(sk, gn, fb, sc, fhn, pm, aij(), geo, GRID, X)
    np.save(os.path.join(d, "fit_pos.npy"), fit_pos)
    torch.save({"net": net.state_dict(), "pm": pm.state_dict(), "sc": sc.state_dict(),
                "fhn": fhn.state_dict(), "aij": aij.state_dict()}, os.path.join(d, "model_final.pt"))
    ...
    with open(os.path.join(d, "run.log"), "w") as f:
        f.write(f"experiment={ARCH_NAME}: 08_06 + rest+open regul + learnable FHN + A_ij[-1,1]\n")
        f.write(f"grid={GRID} edges={E} Q={Q_QUIET} lam_rest={LAM_REST} lam_open={LAM_OPEN}\n")
        f.write(f"scalars={rec}\nfhn={fhn_rec}\nA_ij range=[{aij().min().item():.3f},{aij().max().item():.3f}]\n")
    print(f"  scalars={rec} fhn={fhn_rec}")
    print(f"  archived -> {d}")
