...
state and geometry and writes on that same set. `shape_to_chem` is already four of these --
...
field. Once it is published, `apoptosis_3d` reads it by name and needs no case for it.
...
PUBLISHED ON THE MESH, NOT INTO A STATE COLUMN, and the precedent is `cell_geometry_3d`. A state
...
reason and this joins them.
    ...
    `apoptosis_3d(mode: field_high, field: elong)` reads. Two operators agreeing on a name is the
