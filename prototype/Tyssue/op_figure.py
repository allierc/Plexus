...
exactly the failures that cost this campaign its rounds:
  ...
  * `shape_to_chem.beta` reads LIVE and is pinned to the F clamp, so -2 and -4 emit the SAME field.
...
TOPOLOGY IS FROZEN BY DEFAULT. With `divide_3d` live, a 0.1% perturbation and a 100% perturbation
...
RUN:  python op_figure.py <spec.yaml> --op shape_to_chem
      python op_figure.py <spec.yaml> --op cell_diffuse --values 7 --out figs/operators
"""
...
def draw(curves, op_name, out_dir=OUT, note=""):
    ...
    for i, k in enumerate(keys):
        ...
        # which is exactly what shape_to_chem.beta turned out to be against the F clamp.
