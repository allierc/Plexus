...
def make():
    ...
    sched = ["seed_mesh_3d", "cell_geometry_3d", "cell_adjacency", "cell_rd_seed", "cell_diffuse", "cell_react", "morphogen_growth_3d", "shape_energy_3d", "reconnect_t1_3d", "divide_3d", "topo_snapshot_3d"]
...
try:
    ...
    figm = plt.figure(figsize=(5.0, 5.2)); figm.patch.set_facecolor("black"); axm = figm.add_subplot(111, projection="3d"); figm.subplots_adjust(0, 0, 1, 1)
    ...
    with wri.saving(figm, os.path.join(OUT, "movie.mp4"), dpi=110):
        for j, t in enumerate(keep):
            mt, pt, a = frame(int(t)); _draw(axm, pt, mt, 3.72, azim=(2 * j) % 360, act=norm(a), Lbox=L3); wri.grab_frame()
    plt.close(figm)
