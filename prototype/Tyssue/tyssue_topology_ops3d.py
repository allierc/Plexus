"""tyssue_topology_ops3d -- 3D face DIVISION on the closed spherical half-edge mesh (the 3D sibling of
tyssue_topology_ops.face_divide_line). A cell divides by a septum: two of its edges are split at their
midpoints (m1, m2) and a new edge m1-m2 cuts the face into two daughters. Because every edge of a
closed surface is shared by exactly two faces, splitting an edge ALSO inserts the new midpoint vertex
into the neighbouring face's ring -- the bookkeeping that keeps the mesh closed (Euler V-E+F=2).

The mesh is manipulated as a list of vertex RINGS (ordered CCW, one per face), the same representation
the 2D topology ops use; it is rebuilt to the flat E_srce/E_trgt/E_face half-edge table afterwards.

Operators here are pure functions; the plexus operator `divide_3d` (in tyssue_ops3d) drives them.
"""
