...
The 2D `apoptosis` (tyssue_topology_ops.py:474) is already decomposed into primitives rather than
being a monolith, and the decomposition is what carries over:

| step | 2D | 3D |
|---|---|---|
| shrink the target | `A0_np[f] *= (1 - shrink_rate)` | `V0f[f] *= ...`, with `A0`/`P0` kept consistent as `grow_3d` does (area ~ s², volume ~ s³) |
| shed neighbours | scheduled `t1_transition` | `reconnect_t1_3d` — **exists** |
| extrude | `face_collapse` on a triangle | **the one new primitive** |
| close the gap | force balance | `shape_energy_3d` — **exists** |
...
**The representation is the good news.** `tyssue_topology_ops3d.py` works on `rings` — a list of
...
here and `_check_closed(rings)` (tyssue_topology_ops3d.py:113) is the assertion for it.
...
2. **Per-cell arrays must retire together.** The 3D mesh carries `V0f`, `Vbirth`, `A0`, `P0`,
   ...
   indexed by face. `divide_3d` already appends to all of them on a split; a collapse must retire
...
`apoptosis` is registered in `tyssue_topology_ops.py`, which **the okuda loop does not import** —
`run_one._lazy_engine` loads `tyssue_ops3d`, `tyssue_rd_ops`, `tyssue_t1_ops3d`, `tyssue_monolayer`,
`ckpt` and `tyssue_shape_to_chem`. So the 3D operator should live in `tyssue_ops3d.py` beside
`divide_3d`, its inverse, and be registered as `apoptosis_3d` — leaving the 2D one untouched for
