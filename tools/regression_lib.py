...
def run_cut(spec: dict, family: str, cut: int, device: str, keep: bool = False, stride: int = 1) -> tuple[str, str]:
    ...
    # DETERMINISTIC, BECAUSE THE ARCHIVES ARE. Every working point on disk was generated by
    # `jobs/*.sh`, which export `PLEXUS_STRICT_DETERMINISM=1`; a rerun without it takes the TF32
    # and compiled paths and lands about 1 % away in median radius over 150 frames -- enough to
    # fail the 1 % band on a run whose physics is identical. Setting it here makes the comparison
    # a comparison of the code and not of the numerics the harness happened to pick.
    env = dict(os.environ, PYTHONPATH=os.path.join(ROOT, "src"), GNN_OUTPUT_ROOT=root,
               PLEXUS_STRICT_DETERMINISM="1")
    try:
