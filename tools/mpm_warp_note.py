...
def tbl_pair(cap_pat, nocap_pat, peak):
    """A100: the two capture settings side by side, so the capture column is a comparison and not
    ...
    def rows(pat):
        ...
        for f in sorted(glob.glob(pat)):
            try:
                ...
                pass
        return {r["particles"]: r for r in out}
    c, n = rows(cap_pat), rows(nocap_pat)
    keys = sorted(set(c) | set(n))
    if not keys:
        return "% no rows"
    L = [r"\begin{tabular}{@{}rrrrrrr@{}}", r"\toprule",
         r"& \multicolumn{3}{c}{capture off} & \multicolumn{3}{c}{\textbf{capture on}} \\",
         r"\cmidrule(lr){2-4}\cmidrule(lr){5-7}",
         r"particles & ms/frame & GB/s & GiB & ms/frame & GB/s & GiB \\", r"\midrule"]
    for k in keys:
        a, b = n.get(k), c.get(k)
        f = lambda r, key, fmt: (fmt.format(r[key]) if r else "---")
        L.append((f"{k:,} & " + f(a, "ms_per_frame", "{:.1f}") + " & "
                  + f(a, "gb_per_s", "{:.1f}") + " & " + f(a, "peak_gib", "{:.2f}") + " & "
                  + r"\textbf{" + f(b, "ms_per_frame", "{:.1f}") + "} & "
                  + f(b, "gb_per_s", "{:.1f}") + " & " + f(b, "peak_gib", "{:.2f}")
                  + r" \\").replace(",", r"\,"))
    L += [r"\bottomrule", r"\end{tabular}"]
    return "\n".join(L)
...
def main():
    ...
    parts.append(r"\newcommand{\tblAhunPair}{%" + "\n" +
                 tbl_pair(os.path.join(ROOT, "log/bench/a100_*_warp_cap.json"),
                          os.path.join(ROOT, "log/bench/a100_*_warp.json"), 1555) + "\n}")
    parts.append(r"\newcommand{\tblAhunDefault}{%" + "\n" +
                 tbl_sweep(os.path.join(ROOT, "log/bench/a100_*_default.json"), 1555, "A100 def")
                 + "\n}")
